...
expected_manifest_hash='compiler host manifest:
  hash='
expected_clock_manifest='act=std.time.clock op=now tier=sync path=std.time.clock.now sig=() -> [std::time::clock] std::time::instant surface=contract column=11 symbol=yu_host_3std4time5clock_3now'
expected_server_accept_manifest='act=std.io.net.server op=accept tier=suspend-multi-shot path=std.io.net.server.accept sig=std::io::net::listener -> [std::io::net::server] std::io::net::request surface=contract column=9 symbol=yu_host_3std2io3net6server_6accept'
