...
\large effect-only skeleton・surface ref・operational thunk/force の形式化（議論稿）}
\author{}
\date{2026年6月21日}
...
項の側では、束縛名を値として使う操作を \(\mathsf{ref}\ x\) と表面言語に明示する。
すなわち通常の変数参照はすべて \(\mathsf{ref}\) を通る。
一方、\(\thunk\) と \(\force\) はユーザが書く表面構文ではなく、型指向 elaboration 後の
動作言語にだけ現れる。effectful computation を値として渡す箇所には \(\thunk\) を挿入し、
effect thunk を実行する箇所には \(\force\) を挿入する。
この分離により、静的 id を発行する場所（表面の \(\mathsf{ref}\)）と effect が実際に流れる場所
（動作言語の \(\force\)）を区別できる。
...
\begin{enumerate}[label=\textbf{\arabic*.}]
  ...
  \item \textbf{\(\thunk\) と \(\force\) は動作言語にだけ現れる。}
  surface には \(\thunk\) も \(\force\) もない。effectful computation を値として渡すときに
  \(\thunk\) を挿入し、effect thunk を実行するときに \(\force\) を挿入する。
  したがって id 発行の候補地点は surface の \(\mathsf{ref}\)、effect が実際に流れる地点は
  動作言語の \(\force\) である。
  ...
  \item \textbf{解決した型から operational core を elaboration する。}
...
次の表面項 \(e_{\mathsf{compose}}\) を考える。
\[
\begin{aligned}
e_{\mathsf{compose}}
  ={}& \lambda(f:\emptyrow\to\starann):\starann.\\
     & \lambda(g:\starann\to\starann):\starann.\\
     & \lambda(x:\emptyrow):\starann.\\
     &\quad (\mathsf{ref}\ g)\bigl((\mathsf{ref}\ f)(\mathsf{ref}\ x)\bigr).
\end{aligned}
\]
これは前節の表面項構文 \(e\) そのものである。すべての値参照は \(\mathsf{ref}\) として明示されているが、
\(\thunk\) と \(\force\) はまだ現れない。
型指向 elaboration 後の動作言語では、概念的に
\[
  \force\Bigl((\mathsf{ref}\ g)\,
    \bigl(\thunk(\force((\mathsf{ref}\ f)(\mathsf{ref}\ x)))\bigr)\Bigr)
\]
のように、effectful な中間計算を渡す場所へ \(\thunk\) が入り、実行点へ \(\force\) が入る。
...
局所補題は証明し、全 solver の停止性・完全性・主型性、および cleanup 後の operational marker 消去は
証明計画を伴う予想として明示する。
\end{statusbox}
...
その slot の row 出現に到達した重みから追加制約を生成し、値型 $A$ の内部へ勝手に再帰しない、
という意味である。
...
\(\thunk\) と \(\force\) は表面言語の構文ではない。
型推論で得た computation slot の情報に従って、elaboration が動作言語へ挿入する。
\[
\begin{array}{rcl}
 m &::=& () \mid \mathsf{ref}\ x \mid \lambda x.\ m \mid m\ m
        \mid \thunk(m) \mid \force(m) \mid \perform_h(m)\\
   && \mid\ \mathsf{handle}\;m\;\mathsf{with}\;h(y,k)\Rightarrow m_h\mid \ret\,x\Rightarrow m_r\\
   && \mid\ \mathsf{let}\ x=m_1\ \mathsf{in}\ m_2.
\end{array}
\]
\(\thunk(m)\) は effectful computation を値として保持するために使う。
典型的には、ある関数が effectful argument slot \(A\eff R\) を受け取るとき、
引数位置の表面項は動作言語で thunk 化される。
\(\force(m)\) はその thunk を実行して effect を実際に流す。
本稿の型推論規則は表面項を対象にし、\(\thunk/\force\) の配置は型解決後の operational elaboration として扱う。
...
という表示型である。動作言語では、effectful computation は必要に応じて \(\thunk(m)\) として値に閉じ込められ、
\(\force\) によって実行される。この operational thunk は、ユーザが書く surface term や
effect-only skeleton の一部ではなく、型指向 elaboration の産物として扱う。
\end{remark}
...
\begin{definition}[共変注釈の制約生成]
共変 slot の注釈は、出力 slot に到達した重みから追加制約を生成する judgment で扱う。
\[
  \Delta\vdash \annemit(H,w)\leadsto \mathcal C
\]
は、出力 slot に到達した重み \(w\) と注釈 \(H\) から、追加制約集合 \(\mathcal C\) を生成する。
この judgment の内部規則は体系のパラメータである。
基本条件として、露出している guard family は注釈が許す family に収まる。
\[
...
\]
返り注釈 $\{\mathsf{io}\}$ に到達したとき、
共変注釈の制約生成は $\{\mathsf{write}\}\subseteq\{\mathsf{io}\}$ を要求する。
\end{example}
...
宣言的な weighted subtyping relation には、比較経路を連結する規則を置く。
\[
\inferrule*[right=W-Refl]
 {}
 {\wsub{T}{L}{L}{T}}
\qquad
\inferrule*[right=W-Trans]
 {\wsub{T}{L_1}{R_1}{U}\\
  \wsub{U}{L_2}{R_2}{V}}
 {\wsub{T}{L_1+L_2}{R_1+R_2}{V}}
\]
ここでの \(W\)-Trans は、比較経路を連結したときに左右の重みをそれぞれ加算する規則である。
実装上の solver は一般の推移律を無差別に列挙しない。
Simple-sub と同じく、構造分解で型変数へ到達したときに lower/upper bound を記録し、
変数を中継点にする場合だけ既存の反対側 bound と replay する。
したがって次節の bounds replay は、変数を中間型に取った \(W\)-Trans のアルゴリズム上の実現である。

\begin{formalbox}[title={項の重みは不等式側へ移る},fonttitle=\bfseries]
制約生成では、\(\mathsf{ref}\) 出現に付いた occurrence weight を比較生成の時点で
\(\wsub{T}{L}{R}{U}\) の左右 \(L,R\) へ移す。
構造分解、関数 domain の反転、bounds replay は、不等式側へ移った重みを運ぶ。
\end{formalbox}
...
\begin{enumerate}
  ...
  $\alpha\subty\row H\beta$ を登録して終了する。
  \item \(w\neq\zeroW\) なら、完全 handler 条件
  \[
    H\subseteq\capW\Delta w
  \]
  を要求する。満たさなければ型エラーである。
  \item \(w\neq\zeroW\) なら fresh な row 変数 $\gamma$ を取り、
  \[
...
\paragraph{共変 slot}
その slot に到達した occurrence weight \(w\) と注釈 \(H\) から、
\[
  \Delta\vdash \annemit(H,w)\leadsto\mathcal C_H
\]
を生成する。値型内部へは再帰しない。
\(q=\starann\) は空の制約集合を生成する。
...
返り注釈から追加制約を生成する補助判断を
\[
  \mathsf{posAnn}(\starann,w)=\emptyset,
  \qquad
  \mathsf{posAnn}(H,w)=\mathcal C_H
  \quad(\Delta\vdash\annemit(H,w)\leadsto\mathcal C_H)
\]
と略記する。
その後、$A_x$ の各 id に対応する paint を exported result occurrence に加える。
\[
\inferrule*[right=I-Lam]
 {\mathsf{neg}(K)=(C_x,\Xi_x,A_x)\\
  ...
  \mathcal C_q=\mathsf{posAnn}(q,w_R)\\
  p_{A_x}=\sum_{a\in A_x}\patom a}
 {\Delta;\Gamma\vdash \lambda(x:K):q.e
  \Rightarrow ((C_x\to B\eff\wocc R{w_R+p_{A_x}})\eff\emptyrow)
  \mid\mathcal C\cup\mathcal C_q;\lambda x.M^\circ}
\]
返り注釈は、lambda 本体の output row 出現に到達した重みから追加制約を生成する。
その後、exported result occurrence に引数境界の paint を加える。

\paragraph{application}
$e_1$、$e_2$ を推論し、fresh な $B,R_i,R_o$ を取る。関数型比較を solver へ渡す。
通常の評価 effect は $\seqeff$ で合流する。
この規則は surface application の型制約だけを生成する。
動作言語で必要になる \(\thunk/\force\) は、解決された \(R_i,R_o\) と computation slot の向きに従って
型解決後の elaboration が挿入する。
\[
...
\]
この表示では左右の重みを \(0\) と略記している。
実際には \(e_1\)、\(e_2\)、期待される result slot に付いた occurrence weight を、
この関数型比較を生成する時点で左辺・右辺の重みへ移す。
構造分解と replay は、この不等式側へ移った重みを運ぶ。
...
lambda (f : {} -> {write}) : *.
  ...
  | return x    -> ()
\end{verbatim}
これは surface の式である。動作言語では scrutinee の実行点に \(\force\) が挿入され、概念的には
\[
  \mathsf{handle}\;
    \force((\mathsf{ref}\ f)\,())
  \;\mathsf{with}\;\cdots
...
そのため surface term を直ちに runtime color 操作へ変換せず、まず静的 marker を持つ中間項 $M^\circ$ を生成する。
solver、coalescing、cleanup の後、残った weight と computation slot の向きに従って operational core $M$ を作る。

\subsection{Operational core marker}
...
\toprule
静的情報 & 挿入する operational marker & 動的意味 \\
\midrule
...
型推論と elaboration が成功した surface term $e$ について、operational core $M$ の任意の値 provenance path $\pi$ に現れる
...
\begin{enumerate}
  ...
  \item 宣言的には \(W\)-Trans が比較経路を連結し、solver では変数 bound replay がその必要分だけを実行する。
  \item bounds replay のラベルは加法で一意に合成される。
  \item weighted row split は fresh tail を一つ導入し、head と tail を直接混ぜないため、最一般 residual を保つ。
  \item annotation は slot に到達した重みから追加制約集合を生成し、自由な row tail を失わない。
...
各比較 path の weight は finite-support integer vector である。
ただし再帰 bounds replay で係数が無限に増える可能性を除くため、
...
\begin{itemize}
  ...
  \item surface では、値参照を必ず \(\mathsf{ref}\ x\) と明示する。
  \item \(\thunk\) と \(\force\) は surface ではなく、operational elaboration が挿入する。
  \item 型変数そのものではなく \(\mathsf{ref}\) 参照出現に重みが付く。
  \item 重みは左右付き不等式、比較経路の結合、solver の bounds replay を通る。
  \item 出力注釈は、slot に到達した重みから追加制約集合を生成する。
...
\begin{enumerate}
  ...
  \item \(\thunk/\force\) の型指向挿入を、どの judgment と補題で形式化するか。
...
本稿の体系では、surface の \(\mathsf{ref}\) 参照出現が static id を発行し、その出現に guard weight を付ける。
\(\thunk\) と \(\force\) は surface には現れず、型解決後の operational elaboration が必要な位置へ挿入する。
型比較は左右の重みを保ったまま構造分解され、宣言的には \(W\)-Trans、solver では Simple-sub の
lower/upper bounds replay によって重みが加算される。
