...
\paragraph{実装との対応範囲.}
この停止性定理は、Section~7 の effect-only skeleton を row-only fact へ落とす段階で、
親の effect sequence を各 component から利用先へ直接 fan-out する正規化を仮定している。
すなわち、annotation 由来の positive protection は、対応する filter / predicate boundary が
直接観測できる形で fact machine へ渡る。

現在の Rust 実装は、application、catch、block、defined lambda skeleton の間に
fresh な effect join variable を置く箇所がある。
この lowering では、例えば
\[
  \rho_{\mathrm{call}}
    \xrightarrow{\takeop{\Empty}{u}}
  \sigma
    \xrightarrow{0}
  \rho_{\lambda}
    \xrightarrow{\popop{u}}
  \rho_{\mathrm{call}}
\]
のように、意味上は一つの predicate boundary で閉じるべき marker が
var-var alias replay の executable cycle として見えることがある。
この cycle は source term の非整合ではなく、fact machine へ渡す前の lowering/provenance 正規化が
Section~7 の抽象機械と一致していないことを示す。

したがって、現行実装では var-var alias replay のうち非空 left pop-only label について、
$(\mathrm{lower},\mathrm{upper},\mathrm{pop\ id\ list},\mathrm{right\ weight})$ を
count 抜き key として登録し、同じ alias pair を周回して pop count だけを増やす work を止める。
これは \texttt{pop(n)=pop(1)} という型等式でも、任意の weighted comparison を同一視する規則でもない。
実際、同じ変数 pair という理由だけで replay を no-op にすると、
unannotated local callee の return-effect marker が落ち、\texttt{ref\_update} のような効果が
principal scheme から具体 row へ漏れる。
最終的には lowering を上の fan-out/provenance 正規形へ寄せ、この実装 key が不要かどうかを
再検証する必要がある。
...
Section~\ref{sec:termination} の canonicalization、effect-sequence fan-out 正規化、exact seen key、self-tail no-op、residual hash-cons を実装する。
...
\begin{enumerate}
  ...
  \item effect sequence / block / handler result の lowering は、positive protection を対応する filter / predicate boundary が観測できる正規形へ落としてから replay graph に渡す。
...
これらは単なる最適化条件ではない。
1、2、7 が code bracketing と cycle balance を、5 と 6 が residual の有限生成を、3 と 4 が exact closure の有限処理を保証する。
