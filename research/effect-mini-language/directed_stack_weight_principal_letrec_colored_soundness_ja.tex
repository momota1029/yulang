...
left weight $L$ を実現する well-bracketed colored boundary の内側で、active boundary の entry-except snapshot によって
まだ隠されていなかった operation family $h$ が全 active id を越えた後にも visible であることと
\[
  h\in\Common(L)
\]
は同値である。
...
最初に $h\notin H_u$ となる guard があれば、その時点ではそれより内側の color は entry-except snapshot に含まれておらず、
仮定により外側 color とも交わっていないので、補題~\ref{lem:one-color-hygiene}から $c_u$ が追加される。
従って operation は invisible になる。
もし外側 color と既に交わっていれば、仮定の「まだ隠されていなかった」に反する。
...
同じ id 寿命を通る residual computation では、$h\in J$ の operation は既に entry-except されているか、少なくとも一つの active color を持ち、
...
$h\in J$ なら全 active id について $h\notin H_u\setminus J$ である。
外側から見て最初に entry-except snapshot と交わらない residual guard は、その color を operation に付ける。
そのような guard が無い場合、operation は既に外側 color により except されている。
いずれの場合も active id が少なくとも一つある限り operation は invisible になる。
