...
使用箇所から境界が十分に得られない fresh 単相変数は、`Any` / `Unknown` で補わない。
同時に、未使用・未到達で mono 生成に影響しない slot まで無理に concrete 型へ落としてはならない。
specialize が concrete 型を必要とするのは、次の二種類だけである。

- runtime へ出す expression / binding / ref signature の境界
- 参照先 instance を materialize するため、変数や引数を再帰的に辿る demand edge

この二種類から到達しない slot は solver-local な証拠であり、substitution table に入らなくてよい。
型が一意に決まらないこと自体は、到達しない slot ではエラーではない。
ただし demand edge 上で、参照先を辿るための関数型・thunk 型・constructor 型などの外形が
読めない場合は、単相化不能として止める。
未解決 `RefId`、未解決 typeclass obligation、kind の分からない free type variable が
demand edge 上に残る場合も、未対応の generic direct ref として止める。後続では、引数 expression
の制約を先に解いてから同じ規則で具体代入を作れる範囲を広げる。
...
8. demand-reachable な単相変数を、上下界から concrete 型へ決める。
9. fresh な単相変数から concrete 型への型代入表を作る。到達しない slot は表に入れなくてよい。
...
7. demand-reachable な境界で、どちらの境界からも一意の concrete 型が得られないなら、
   単相化不能としてエラーにする。
...
demand-reachable な境界で候補が一つも出ない場合も、参照先を辿るための具体型が決まらないため
単相化不能としてエラーにする。
一方で、emit される値にも次の demand edge にも関わらない slot は、この判定の対象ではない。
...
落とさず、demand-reachable な境界でだけ単相化不能として扱う。
...
再利用することではない。runtime へ出す境界や producer-consumer 境界の型は、需要内で concrete に
決まった computation 型であり、`Unknown`、free type variable、未解決 `RefId`、未解決
typeclass obligation を含まない。
一方で、specialize 内部で全 expression slot が concrete 解を持つ必要はない。
emit されない slot や、参照先を再帰的に辿る demand edge に現れない slot は、
solver-local な未解決 slot のまま消えてよい。
