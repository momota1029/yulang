...
- `.yuunit`: compiled-unit artifact
- `.yuir`: principal poly artifact
- `.yuvm`: control-vm artifact

`.yuunit` は syntax / namespace / lowering / typed / runtime surface を保存する。
...
full-source `.yuunit` hit で省略されるのは次である。

- `sources::load`
- `infer`

source-unit / std prefix `.yuunit` hit で省略されるのは、prefix として import された
...
full-source `.yuunit` は同じ source key を使い、stage ごとの format version は
...
source-unit `.yuunit` は source dependency graph の unit ごとの key を使う。unit key は
...
merged prefix `.yuunit` は選択された source-unit artifact 群の manifest / surface hash から
...
source-unit `.yuunit` が prefix arena の def を参照する場合は、raw arena-local ID ではなく
...
`.yuir` が miss した場合は、同じ exact source key の full-source `.yuunit` を見る。
hit した場合は compiled surfaces から `poly::Arena` を復元し、得られた `.yuir` と `.yuvm`
も保存する。

full-source `.yuunit` が miss した場合は、source-unit / std prefix `.yuunit` を探す。
hit した場合は prefix artifact を import し、prefix に含まれない suffix source だけを
fresh に処理する。結果として exact full-source `.yuunit` と `.yuir` を保存し、`run` では
`.yuvm` も保存する。

全て miss した場合は source set から fresh に infer を走らせ、得られた `.yuunit`、`.yuir`、
`.yuvm` を保存する。fresh miss 時には source-unit closure `.yuunit` も保存する。
...
- `std-prefix-hit`: std prefix `.yuunit` hit
- `std-prefix-build`: std prefix `.yuunit` を作ってから suffix を処理
- `source-unit-prefix-hit`: single source-unit prefix `.yuunit` hit
- `merged-source-unit-prefix-hit`: merged source-unit prefix `.yuunit` hit
