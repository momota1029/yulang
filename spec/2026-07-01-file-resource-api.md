...
トランザクション、scoped `text_with` は state-passing protocol として
`str -> [e] ('a, str)` callback を受ける）。
...
同じ意味を closure でも書ける。現行 Contract v1 の `text_with` は
`str -> [e] ('a, str)` protocol であり、ref callback ではない。

```yu
file::text_with path: \text0 ->
  my $text = text0
  ...
  (result, $text)
```
