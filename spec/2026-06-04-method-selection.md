...
これらは`role`の関連型によって解決する可能性があるため、関連型ループを行う。
ループ本体（関連型を含む role を生のまま compact 型として展開 → 解ける role を全て解く →
関連型の変化があれば現状の role を新たに展開して繰り返す）は
`2026-06-02-role-system.md` の「roleの再帰的な解決について」に一本化する。

method 選択側に固有なのは、そのループが SCC に変化を生まなくなったときの終了処理だけである。

- SCC に変化が無くなったら、未解決の method を全て role か record field として解釈し、ループを抜ける。

これにより method は最終的に全て解決される。
