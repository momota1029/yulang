...
つまり、現在の dynamic handler 群から見てまだ読める request のうち、指定された effect family に
属さない request だけを blocked にする。すでに別の guard で blocked になっている request は
書き換えない。`family_prefix(path, r.path)` を満たす request は、その境界で受け取るべき effect として残す。
