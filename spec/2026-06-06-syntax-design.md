...
通常 scanner が keyword として認識する語は次である。
...
文脈によって keyword が atom になったり stop token になったりする。
たとえば式の先頭では `do` / `if` / `case` / `catch` / `rule` は atom 側で扱われるが、
`my` は statement dispatch の stop になる。
...
statement parser は最初に式 parser を走らせる。式 parser が stop token を返した場合、
その stop token の種類で declaration / statement に dispatch する。
