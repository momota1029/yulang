...
- stack entryは常に `pop(p)[H1, ..., Hn]` の形に正規化する。
- `H` はeffect path集合である。
...
```text
Weight(#id) = pop(p)[H1, ..., Hn]
```

空の重みでは、全ての`#id`が`pop(0)[]`である。
...
同じconstraint machine内で、同じ`α`、同じ`β`、同じ`S`、同じ重み`@W`からこの分解を複数回行う場合は、同じ残差変数`γ`を再利用してよい。
これは新しい型規則ではなく、分解結果をhash-consして同じ境界を何度も増やさないための実装上の正規化である。
再利用してよいのは、分解前の情報が同じで、作られる`α upper += ...`と`γ @W <: @0 β`が同一になる場合だけである。
...
つまり、`pop(p)[H1, ..., Hn]`で`n > 0`のentryが一つでも残るなら、量化値の述部側には同じidの`pop(u32::MAX)[]`を一つ付与する。
...
また、Pos/Negへ置き換える時点で、対応する非空stack entryがどちらの極にも存在しないpopだけのentryは消してよい。
