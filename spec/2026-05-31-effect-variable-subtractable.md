...
- floor込みのentry合成は、floor同士を共通部分で畳み、pop/stack部分は従来の規則のまま合成する。
  ```text
  floor[F1] pop(p)[H1, ..., Ha] + floor[F2] pop(q)[K1, ..., Kb]
  = floor[F1 ∩ F2] (pop(p)[H1, ..., Ha] + pop(q)[K1, ..., Kb])
  ```
...
# 新規導入変数について
1. 返りエフェクトが注釈されていない引数の`f`に対して`f x`を推論したときに出てくる`f: α [β] → [γ] δ`について: `γ`は「何も引いてはならない型変数」であるので、新しく`#a`を立て、返りエフェクトを`stack(γ, { #a -> pop(0)[Empty] })`として扱う。これの登場する関数の述部には新しく変数を立て、`stack([ε] ζ, { #a -> pop(1)[] })`を置く。
2. `my f(x: [handled] α)`など注釈された変数について: `x: [stack(β, { #b -> pop(0)[handled] })] α`と登録する。`handled`が`_`だった場合は`stack(β, { #b -> pop(0)[All] })`にする。これの登場する関数の述部には新しく変数を立て、`stack([γ] δ, { #b -> pop(1)[] })`を置く。
3. `my f(g: α [β] -> [handled] γ)`と注釈された関数について: `f: α [β] -> [stack(δ, { #c -> pop(0)[handled] })] γ`として登録する。`β`に`io`などと書いてあった場合はそのまま登録してよく、`[io] <: β`とする。`α [β] -> [handled; δ] γ`と書いてあった場合は`η`を立て、`stack(η, { #c -> pop(0)[handled] })`を使い、`f`の述部には`stack([ζ] η, { #c -> pop(1)[] })`を置く。
4. `xs: list(α [A] → [B] β)`のように不変に注釈された変数について、変数`γ`と`δ`を定める。下からは具体rowで抑えるため`A <: γ`、`B <: δ`を置き、同時に`stack(γ, { #a -> pop(0)[A] })`、`stack(δ, { #b -> pop(0)[B] })`を使う。不変位置に書けるのは具体effectのみであり、effect変数、wildcard、具体effectと変数の混合はここでは受け付けない。
...
body内の`catch x`がscrutinee viewから`pop(0)[handled]`を供給し、関数の返り側述部には`pop(1)`が合成される。
...
- 共変位置では、書かれた具体エフェクトをそのまま境界に置く。ここではstack wrapperを作らない。
- 普通の反変位置では、書かれた具体エフェクトを直接境界に固定せず、新しいエフェクト変数を立て、その変数を`stack(ρ, { #id -> pop(0)[S] })`で包む。
...
```yu
@g1 = { #1 -> pop(0)[[g]] }
(a [b] -> [any] d) -> [e] list(f -> [g; stack('eff1, @g1)] h)
```
...
@c2 = { #2 -> pop(0)[[c]] }
@g1 = { #1 -> pop(0)[[g]] }
(a [any] -> [stack('eff2, @c2)] d) -> [any] list(f -> [g; stack('eff1, @g1)] h)
```
...
普通の反変位置ではstack wrapperを作り、共変位置ではstack wrapperを作らない。
...
述部側の多重popは、`stack(T, { #1 -> pop(1)[], #2 -> pop(1)[] })`のように保存してよい。
