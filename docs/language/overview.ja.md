...
- `impl Display fs_err` — 既定の文字列化（手書きの impl で上書きできる）
...
この関数の推論型は概ね `path -> [fs; fs_err] str` の形になり、エラーが effect row に明示される。
...
my read_text_safe path =
    ...
    of
        result::ok text -> text
        result::err err -> err.show + " (フォールバック)"
```

`fs_err::wrap` は、引数の thunk が起こす `fs_err` effect を捕まえて `result str fs_err` を返す。`err` 側は自動生成された `Display fs_err` 経由で文字列化できる。
