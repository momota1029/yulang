...
- `std::io::file::read_text`, `write_text`, `exists`, `is_file`, `is_dir`,
  ...
  `write_at` interception and a reusable pure-Yulang managed-ref helper shape,
  but production `text_with` still uses private snapshot helpers and is not yet
  fully mockable at a public host-act/session boundary. This is still not the
  full Contract v1 file-resource contract because production pure mock
  resource-lifetime parity, directory listing, portable metadata expansion, and
  lock release remain provisional. The
