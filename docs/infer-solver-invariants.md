...
## Per-Demand Generalization Role Snapshots

generalization loop の exact role-solve snapshot reuse は実装済みだが、Stage 2 gate で
repository-std-only と showcase の固定 overhead が 2% no-regression bound を超えたため、production
default は always-full-solve のままである。reuse は実験・benchmark 用の
`--generalize-role-snapshot-enable-reuse` 明示 opt-in だけで有効になる。default を変えるには、別 project
として cheap-demand admission policy を設計し、correctness と四 fixture の release gate を再実行する。

snapshot は一つの root の generalization lifetime だけに属し、exact `E/A/M/D/C` comparison、live
applied filtering、64-entry / 128-MiB Debug-proxy cap、fail-closed full solve を維持する。independent shadow
oracle、always-solve control、exact comparator、candidate guard は継続的な assurance であり、専用の
redundancy review なしに削除しない。95.3x compact-tree expansion と collect-roles/dominance iteration reuse
は未実装の別 scope である。経緯と gate 数値は
`notes/design/2026-07-16-generalize-role-resolve-snapshot-spec.md` に置く。
