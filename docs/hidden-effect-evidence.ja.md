...
## 例: `compose` の未注釈 callback は hidden evidence を持つ
...
未注釈の `g` は、`g x` 由来の effect を外側の callee / handler が偶然捕捉しないように
`take(Empty)` で保護される。`g: _ -> [_] _` は「surface の effect をそのまま見せる」
契約なので、この保護を入れない。

```text
compose_surface : (α [γ] -> [δ] β) -> (ε -> [γ] α) -> ε -> [δ] β
compose_plain   : (α [γ#u[Empty]] -> [δ] β) -> (ε -> [γ#u[Empty]] α) -> ε -> [δ#u] β#u
```
...
`compose_surface` は「引数 `g` の effect が `f` 側の effect row へ流れる」ことを表し、
`compose_plain` はさらに「その effect をこの boundary では引けない」ことを表す。
...
未注釈引数は `take(Empty)` で保護されるので、handler は引数由来の `out` を捕捉できない。
