...
fn main() {
    ...
    if let Err(error) = write_embedded_full_std_artifact() {
        panic!("failed to build embedded full std artifact: {error}");
    }
}
...
fn write_embedded_full_std_artifact() -> Result<(), String> {
    let files =
        yulang::collect_source_text_with_embedded_std(FULL_STD_ARTIFACT_ENTRY, String::new())
            .map_err(|error| error.to_string())?;
    let loaded = yulang::load_source_text_with_embedded_std(FULL_STD_ARTIFACT_ENTRY, String::new())
        .map_err(|error| error.to_string())?;
    let key = yulang::cache::source_cache_key(&files);
    let artifact =
        yulang::cache::compiled_unit_artifact_from_loaded_files_with_key(&files, &loaded, key)
    ...
    if !artifact.errors.is_empty() {
        return Err(format!(
            "embedded full std artifact has lowering errors: {:?}",
            artifact.errors
        ));
    }
