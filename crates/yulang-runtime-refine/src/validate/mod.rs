...
fn collect_hir_type_var_kinds(
    ...
    match ty {
        RuntimeType::Unknown => {}
        RuntimeType::Value(ty) => collect_core_type_var_kinds(ty, slot, out),
...
fn collect_hir_named_arg_kinds(
    ...
    match ty {
        RuntimeType::Unknown => {}
        RuntimeType::Value(ty) => collect_core_named_arg_kinds(ty, vars, out),
        RuntimeType::Fun { param, ret } => {
...
fn infer_concrete_effect_args_from_hir(ty: &RuntimeType, out: &mut TypeArgKinds) {
    match ty {
        RuntimeType::Unknown => {}
        RuntimeType::Value(ty) => infer_concrete_effect_args_from_core(ty, out),
        RuntimeType::Fun { param, ret } => {
