...
//! FS を辿る orchestration はここに置き、`sources` は in-memory source set の parse/load 層に留める。
...
pub fn dump_poly_from_entry(entry: impl AsRef<FsPath>) -> Result<DumpPolyOutput, RouteError> {
    let collected = collect_local_sources(entry)?;
...
pub fn collect_local_sources(
    ...
    Collector::new().collect(entry.as_ref())
}
...
impl Collector {
    ...
    fn collect(mut self, entry: &FsPath) -> Result<Vec<CollectedSource>, RouteError> {
