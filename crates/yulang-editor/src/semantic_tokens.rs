...
mod tests {
    use super::*;
    use rowan::{NodeOrToken, SyntaxNode};
    use yulang_parser::op::{BpVec, OpDef};
    use yulang_parser::sink::YulangLanguage;
    ...
    #[test]
    fn debug_inflate_call() {
        eprintln!("--- inflate {{ base: 3 }} ---");
        dump_tokens("inflate { base: 3 }");
    }

    fn dump_nodes(source: &str) {
        let green = yulang_parser::parse_module_to_green(source);
        let root = SyntaxNode::<YulangLanguage>::new_root(green);
        fn walk(node: &SyntaxNode<YulangLanguage>, depth: usize) {
            let indent = "  ".repeat(depth);
            eprintln!("{}{:?}", indent, node.kind());
            for child in node.children_with_tokens() {
                match child {
                    NodeOrToken::Node(n) => walk(&n, depth + 1),
                    NodeOrToken::Token(t) => {
                        eprintln!("{}  {:?} {:?}", indent, t.kind(), t.text());
                    }
                }
            }
        }
    ...
    #[test]
    fn debug_inflate_call_tree() {
        eprintln!("--- inflate {{ base: 3 }} tree ---");
        dump_nodes("inflate { base: 3 }");
    }
