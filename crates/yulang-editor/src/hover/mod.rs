...
pub struct DiagnosticHoverTarget {
    ...
    pub related_ranges: Vec<Utf16Range>,
}
    ...
    targets
        ...
        .find_map(|(diagnostic_index, target)| {
            ...
            target
                ...
                .iter()
                .enumerate()
                .find_map(|(related_index, range)| {
                    utf16_range_contains_position(*range, position).then_some(
                        DiagnosticHoverSelection {
                            diagnostic_index,
                            related_index: Some(related_index),
                            range: *range,
                        },
                    )
                })
