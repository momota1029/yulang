...
// Historical CPK-8A fixture-writer classifications remain as retirement evidence. The active
// lexical gates below now protect the CPK-only boundaries and forbid reintroduction of removed
// flat/RCPF storage or authority seams.
//
...
//
// These counts classify the reviewed historical writer sites, not every caller of a shared
// fixture. All category-B comparisons are retired, every surviving correctness fixture is
// CPK-only, and no Legacy-only shared constructor remains.
//
...
//   former RCPF one-way feed. All five CPK-8G-7 sub-slices are complete.
// - CPK-8G-11 removes the complete eight-field flat claim mirror after closing production and
//   fixture readers. ProofOccurrenceStore exclusively owns claim transition/dedup and payloads.
...
// - CPK-8G-10 removes the dead oracle/fault-injection shell. CPK-8G-12 part 1 consolidates the
//   publication-fence failure, sticky drain stop, and whole-attempt discard into the CPK channel;
//   only the final telemetry/name closure remains.
...
// dependents retire. CPK-8G-9-0b later retires the last test-only Factored evaluator itself after
// moving the semantic-snapshot round to the production CPK evaluator.
...
// CPK-8G-9-0b closes the final test-only RCPF consumers before reverse-topological structure
// deletion. These are representation characterizations, not the direct structure tests retained
// below: the CPK evaluator and canonical exact-parent contracts own their surviving semantics.
const CPK8G90B_RETIRED_RCPF_FIXTURES: &[(&str, &str)] = &[
    (
        "rcpf_c2_factored_evaluator_matches_fresh_shared_and_insertion_order_queries",
        "the semantic snapshot and publication rounds now use CpkProjectionEvaluator; CPK cycle-cut, canonical formula-order, and fresh/shared equivalence tests pin the surviving contract",
    ),
    (
        "replay_plan_parent_drafts_match_legacy_parent_order",
        "CPK qualified-parent canonical exact-order and first-witness permutation tests pin the product identity/order contract; ReplayParentDraft identity was only RCPF writer plumbing",
    ),
];
...
const PROOF_STATE_REFERENCE_CENSUS: &[(&str, usize)] = &[
    ...
    // CPK-8G-9a removes ReplayClauseProjection's final scoped attribution store together with its
    // one-way projection feed. The generic TypeBounds attribution mirrors were removed in 8G-8b1.
    ("attributed_claim_supports", 0),
    ...
    // CPK-8G-5 adds the CPK-owned snapshot iterator and one flat-reset independence check;
    // CPK-8G-8c0 removes that final fixture-only mirror access.
    ...
    // removes the final production occurrence reader from premise-dependency traversal. CPK-8G-
    // 9-0b removes the test evaluator, CPK-0b census/reset, and CPK-3 fixture read; only the RCPF
    // writer and structure definition remain. CPK-8G-9a removes the occurrence-store argument
    // from the deleted projection feed, and CPK-8G-9d removes the occurrence store and admission
    // feed completely.
...
const REVIEWED_BOUNDARIES: &[(&str, &str)] = &[
    ...
    (
        "constraints/machine/bounds.rs",
        "register_cpk_replay_claim_parents",
    ),
    (
        "constraints/machine/entry.rs",
...
#[test]
fn cpk_8g_9_0b_rcpf_readers_are_closed_before_structure_removal() {
    let bounds = include_str!("machine/bounds.rs");
    let (bounds_production, bounds_tests) = bounds
        .split_once("#[cfg(test)]\nmod mutation_tests")
        .expect("bounds production/test boundary");
    for field in [".replay_parent_sets"] {
        assert_eq!(
            bounds_production.matches(field).count(),
            0,
            "removed RCPF production surface reappeared: {field}",
        );
        assert_eq!(
            bounds_tests.matches(field).count(),
            0,
            "RCPF test fixture must use CPK state instead of reading {field}",
        );
    }
    ...
    for retired in [
        ...
        "SchemeProjectionEvaluationRound",
    ...
    assert_eq!(
        ...
        &["8G-9-0a", "8G-9-0b"],
        "RCPF structure deletion requires both reader-closure preparation slices",
    );
}

#[test]
fn cpk_8g_9a_replay_clause_projection_is_fully_removed() {
    let removed_type = ["ReplayClause", "Projection"].concat();
    let removed_field = ["replay_clause", "_projection"].concat();
    for source in [
        include_str!("mod.rs"),
        include_str!("machine/entry.rs"),
        include_str!("machine/bounds.rs"),
        ...
        assert!(
            ...
            "the removed RCPF projection type must not reappear",
        );
        assert!(
            !source.contains(&removed_field),
            "the removed RCPF projection field/feed must not reappear",
        );
    }
    assert_eq!(
...
fn cpk_8g_9c_replay_result_summary_is_fully_removed() {
    ...
    assert!(
        CPK8G9_COMPLETED_SUBSLICES.starts_with(&["8G-9a", "8G-9b", "8G-9c"]),
        "RCPF reverse-topological deletion must retain all three completed removals",
    );
}

#[test]
fn cpk_8g_9d_replay_occurrence_store_is_fully_removed() {
    let removed_store = ["ReplayOccurrence", "Store"].concat();
    let removed_id = ["ReplayOccurrence", "Id"].concat();
    let removed_key = ["ReplayOccurrence", "Key"].concat();
    let removed_field = ["replay_occurr", "ences"].concat();
    let removed_attachment = ["ReplayParentAttachment", "Batch"].concat();
    for source in [
        include_str!("mod.rs"),
        include_str!("machine/entry.rs"),
        include_str!("machine/bounds.rs"),
        ...
        for removed in [
            ...
            assert!(
                ...
                "removed RCPF occurrence surface reappeared: {removed}",
            );
        }
    }
    let bounds = include_str!("machine/bounds.rs");
    assert_eq!(
        bounds.matches(".replay_parent_sets").count(),
        0,
...
#[test]
fn cpk_8g_12_part_2_obsolete_payload_seams_are_removed_but_semantic_attribution_remains() {
    let proof_source = include_str!("proof/mod.rs");
    for removed in [
        ["LegacyProof", "Backend"].concat(),
        ["NullProof", "Backend"].concat(),
        ["ConstraintProofPayload", "Ref"].concat(),
        ["BoundProofPayload", "Ref"].concat(),
        ["ProofPayload", "View"].concat(),
        ["proof_payload", "_ref"].concat(),
    ] {
        assert!(
            !proof_source.contains(&removed),
            "obsolete CPK-1 payload seam reappeared after CPK-8G-12 part 2: {removed}",
        );
    }
    for semantic_contract in [
        "ClaimedAttributionSource",
        "FlatRetained",
        "ProjectionLineage::ReplayEvidence",
    ] {
        assert!(
            proof_source.contains(semantic_contract),
...
fn cpk_8g_physical_removal_manifest_is_complete_and_uniquely_classified() {
    ...
    for surviving_writer in [
        ...
        assert!(
            ...
            "authoritative CPK proof writer disappeared: {surviving_writer}",
        );
    }
    for replacement in [
        "canonical_projection_storage_is_invariant_across_all_four_event_permutations",
