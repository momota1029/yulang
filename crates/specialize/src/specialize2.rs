...
fn merge_tuple_type_candidates(
    graph: &TypeGraph<'_>,
    left_items: Vec<Type>,
...
fn stack_handler_marker_path_from_body(body: &Expr) -> Option<Vec<String>> {
    match &body.kind {
...
fn catch_stack_handler_marker_path(body: &Expr, arms: &[CatchArm]) -> Option<Vec<String>> {
...
fn forced_body_effect_marker_path(body: &Expr) -> Option<Vec<String>> {
    match &body.kind {
...
fn stack_handler_marker_path_from_block(block: &Block) -> Option<Vec<String>> {
    block
        .stmts
        .iter()
...
fn stack_handler_marker_path_from_stmt(stmt: &Stmt) -> Option<Vec<String>> {
    match stmt {
...
fn stack_handler_marker_path_from_record_spread(
...
fn wrap_stack_handler_marker_block(path: Vec<String>, block: Block) -> Block {
    Block {
        stmts: block
            .stmts
            .into_iter()
...
fn wrap_stack_handler_marker_stmt(path: Vec<String>, stmt: Stmt) -> Stmt {
    match stmt {
...
fn wrap_stack_handler_marker_case_arm(path: Vec<String>, arm: CaseArm) -> CaseArm {
    CaseArm {
        pat: arm.pat,
        guard: arm
            .guard
...
fn wrap_stack_handler_marker_catch_arm(path: Vec<String>, arm: CatchArm) -> CatchArm {
    CatchArm {
...
fn wrap_stack_handler_marker_record_spread(
    path: Vec<String>,
    spread: RecordSpread<Box<Expr>>,
