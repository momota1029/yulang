...
use crate::parser::{
    LexIn, ParserIn,
    input::item::{Item, ItemExtent, LeadingTrivia, TokenKind},
    output::{ParserOutput, RecoveryDraft},
};

/// Sealed capability for one total malformed run.
///
/// Its private `ParserIn` cannot escape. The body can only perform total
...
pub(in crate::parser) struct ErrorRunOutput<'a, 'source, 'recover, 'operators, 'output, 'frozen> {
    input: ParserIn<'a, 'source, 'recover, 'operators, 'output, 'frozen>,
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(in crate::parser) enum PathSegmentRetryLeadingSeal {
    Ineligible,
    Sealed,
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(in crate::parser) enum CallArgumentRetryLeadingSeal {
    Ineligible,
    Sealed,
}

impl ErrorRunOutput<'_, '_, '_, '_, '_, '_> {
    pub(in crate::parser) fn lexical<O>(&mut self, operation: impl FnOnce(LexIn) -> O) -> O {
    ...
    pub(in crate::parser) fn emit_item_as(
        &mut self,
        item: Item,
    ...
    pub(in crate::parser) fn emit_literal_segment(
        &mut self,
        text: &str,
        range: Range<usize>,
        kind: SyntaxKind,
    ...
    pub(in crate::parser) fn append_unexpected(&mut self, unexpected: UnexpectedSyntax) {
        self.assert_unsealed();
    ...
    pub(in crate::parser) fn seal_record_through_retry_leading(
        &mut self,
        retry: &Item,
        successor_origin: usize,
    ...
    pub(in crate::parser) fn seal_path_segment_retry_leading_prefix(
        &mut self,
        retry: &mut Item,
        successor_origin: usize,
        category: UnexpectedCategory,
    ...
    pub(in crate::parser) fn seal_call_argument_retry_leading_prefix(
        &mut self,
        retry: &mut Item,
        successor_origin: usize,
        category: UnexpectedCategory,
...
pub(in crate::parser) fn emit_recovery_missing(
    mut i: ParserIn,
...
pub(in crate::parser) fn emit_recovery_error_item(
    i: ParserIn,
    item: Item,
    successor_origin: usize,
...
pub(in crate::parser) fn emit_recovery_error_run<R>(
    i: ParserIn,
    ...
    // post-commit contracts; unwinding invalidates this ParserOutput.
...
pub(in crate::parser) fn emit_identifier_core(i: &mut ParserIn, item: Item) {
    debug_assert_eq!(
...
pub(in crate::parser) fn emit_integer_core(i: &mut ParserIn, item: Item) {
...
pub(in crate::parser) fn emit_operator_use(i: &mut ParserIn, item: Item, kind: SyntaxKind) {
...
pub(in crate::parser) fn emit_with_keyword(i: &mut ParserIn, item: Item) {
...
pub(in crate::parser) fn emit_token_item(i: &mut ParserIn, item: Item) {
    let payload = item.payload_view();
...
pub(in crate::parser) fn emit_literal_item(i: &mut ParserIn, item: Item, kind: SyntaxKind) {
...
#[cfg(test)]
pub(in crate::parser) fn emit_fragmented_item(i: &mut ParserIn, item: Item) {
    let payload = item.payload_view();
    let kind = if payload.operator_use().is_some() {
...
/// missing slot.
pub(in crate::parser) fn emit_missing(i: &mut ParserIn, leading: LeadingTrivia) {
...
#[cfg(test)]
pub(in crate::parser) fn emit_end(output: &mut ParserOutput, end: &mut End) {
    end.item.emit_eof_leading(output);
}

fn emit_trivia(i: &mut ParserIn, trivia: &LeadingTrivia) {
    emit_trivia_builder(&mut *i.state, trivia);
}

fn emit_trivia_builder(output: &mut ParserOutput, trivia: &LeadingTrivia) {
    trivia.emit(output);
}

pub(in crate::parser) fn token_syntax_kind(kind: TokenKind) -> SyntaxKind {
    match kind {
