...
impl LeadingTrivia {
    ...
    pub(super) fn emit(&self, output: &mut ParserOutput) {
        for part in &self.0 {
...
impl Item {
    ...
    pub(super) fn emit_all_remaining_leading(&mut self, output: &mut ParserOutput) {
    ...
    pub(super) fn emit_payload(self, output: &mut ParserOutput, kind: SyntaxKind) {
    ...
    pub(super) fn emit_remaining(mut self, output: &mut ParserOutput, payload_kind: SyntaxKind) {
    ...
    pub(super) fn emit_eof_leading(&mut self, output: &mut ParserOutput) {
        assert!(self.payload_view().is_eof());
        self.emit_leading_range(output, self.physical_leading.len(), |_, _| {});
    }

    #[cfg(test)]
    pub(super) fn emit_terminal_boundary(mut self, output: &mut ParserOutput) -> PendingBoundary {
