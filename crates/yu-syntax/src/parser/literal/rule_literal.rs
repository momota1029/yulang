...
#[cfg(test)]
pub(in crate::parser) enum RuleLiteralExit {
    Complete,
    Boundary(Item),
}

pub(in crate::parser) enum NormalizedRuleLiteralExit {
    Complete(LineEntry),
    Boundary(Item, LineEntry),
}

#[cfg(test)]
pub(in crate::parser) enum PatternLiteralOpener {
    Rule(Item),
    String(Item, StringMode),
}

#[cfg(test)]
pub(in crate::parser) fn scan_expression_rule_literal_opener_witness(mut i: LexIn) -> Option<Item> {
    let token = i.token(scan_expression_rule_literal_opener_token)?;
    Some(Item::plain(LeadingTrivia::default(), Payload::Token(token)))
}

pub(in crate::parser) fn scan_expression_rule_literal_opener_token(mut i: LexIn) -> Option<Token> {
...
#[cfg(test)]
pub(in crate::parser) fn scan_pattern_literal_opener_witness(
    mut i: LexIn,
...
pub(in crate::parser) fn scan_pattern_literal_opener_token(mut i: LexIn) -> Option<Token> {
...
#[cfg(test)]
pub(in crate::parser) fn rule_literal_witness(
    i: ParserIn,
    opener: Item,
    part_origin: usize,
    fence: &FenceBoundary,
...
pub(in crate::parser) fn rule_literal_normalized(
    mut i: ParserIn,
    opener: Item,
