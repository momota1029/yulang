...
#[cfg(test)]
pub(super) fn statement(i: ParserIn, baseline: usize, stops: Stops) -> TailExit {
...
#[cfg(test)]
pub(super) fn is_canonical_statement_nud(i: ParserIn, item: &Item, baseline: usize) -> bool {
...
fn emit_indented_missing(i: ParserIn, item: &mut Item, origin: usize, role: GrammarRole) {
...
fn missing_brace_close(mut i: ParserIn, mut item: Item, origin: usize) -> TailExit {
    if item.payload_view().is_eof() {
        item.emit_all_remaining_leading(&mut *i.state);
    }
    emit_braced_missing(
...
fn emit_braced_missing(i: ParserIn, item: &Item, origin: usize, role: GrammarRole) {
    let at = item
        .payload_view()
        .pending_boundary()
