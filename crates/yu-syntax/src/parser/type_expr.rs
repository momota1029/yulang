...
#[cfg(test)]
pub(super) fn type_expr(i: ParserIn) -> Option<TailExit> {
...
/// expose a general `ParserIn` to its body.
...
fn emit_leading_type_head_missing(i: &mut ParserIn, item: &Item, item_origin: usize) {
...
fn emit_bracket_arrow_missing(i: &mut ParserIn, item: &Item, item_origin: usize) {
    let at = item.payload_view().pending_boundary().map_or_else(
        || item.extent(item_origin).recovery_range().start,
        |boundary| boundary.coordinate(),
    );
