...
pub(in crate::parser) fn for_statement_selected(item: &Item) -> bool {
    item_word(item) == Some("for")
}

#[allow(clippy::too_many_arguments)]
pub(in crate::parser) fn for_statement_normalized(
    mut i: ParserIn,
    keyword: Item,
    baseline: usize,
    outer_stops: Stops,
...
fn iterable_from_item_normalized(
    ...
    let exit = if item.payload_view().is_boundary()
        || implicit_delimited_newline(baseline, item.leading_view())
    {
        crate::parser::expression::emit_required_expression_missing(
            &mut i,
            &mut item,
            item_origin,
            iterable_stops(outer_stops),
...
fn outer_boundary(mut i: ParserIn, item: &Item, baseline: usize, outer_stops: Stops) -> bool {
...
fn implicit_gap(baseline: usize, leading: crate::parser::input::item::LeadingView<'_>) -> bool {
...
fn emit_keyword(i: &mut ParserIn, item: Item, kind: SyntaxKind, spelling: &str) {
    debug_assert_eq!(
