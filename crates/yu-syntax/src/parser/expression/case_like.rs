...
#[derive(Clone, Copy)]
pub(in crate::parser) enum CaseLikeFamily {
    Case,
    Catch,
}
...
pub(in crate::parser) fn case_like_nud_normalized(
    mut i: ParserIn,
    family: CaseLikeFamily,
    mut keyword: Item,
...
fn missing_block(mut i: ParserIn, family: CaseLikeFamily, exit: TailExit) -> TailExit {
    i.state.start_node(family.block_node().into());
    let exit = match exit {
...
fn arm_sequence_normalized(
    ...
    let sequence = Some(match policy {
        ArmSequencePolicy::CaseInline => {
            crate::parser::context::sequence::SequenceOwner::CaseInline
        }
        ArmSequencePolicy::CatchInline => {
            crate::parser::context::sequence::SequenceOwner::CatchInline
        }
        ArmSequencePolicy::Indented {
            family: CaseLikeFamily::Case,
            ..
        } => crate::parser::context::sequence::SequenceOwner::CaseIndented,
        ArmSequencePolicy::Indented {
            family: CaseLikeFamily::Catch,
            ..
        } => crate::parser::context::sequence::SequenceOwner::CatchIndented,
        ArmSequencePolicy::CatchBraced { .. } => {
            crate::parser::context::sequence::SequenceOwner::CatchBraced
        }
    });
...
fn finish_absent_arm_normalized(mut i: ParserIn, exit: NormalizedExit) -> NormalizedExit {
...
fn arm_body_boundary(mut i: ParserIn, item: &Item, baseline: usize, stops: Stops) -> bool {
...
fn guard_kind(mut i: ParserIn, item: &Item) -> Option<SyntaxKind> {
    if item.payload_view().is_boundary() {
        None
...
impl ArmSequencePolicy {
    ...
    fn body_stops(self) -> Stops {
        match self {
            ...
            Self::CatchBraced { .. } => {
                STOP_COMMA
                    | STOP_LINE_BREAK
                    | crate::parser::input::operator::stops_for(TokenKind::RBrace)
            }
        }
    }
    ...
    fn accepts_arm_entry(self, mut i: ParserIn, item: &Item, outer_stops: Stops) -> bool {
    ...
    fn boundary_after_separator(self, mut i: ParserIn, item: &Item, outer_stops: Stops) -> bool {
        if sequence_outer_boundary(i.rb(), item, outer_stops) {
            return true;
        }
        match self {
...
fn sequence_outer_boundary(i: ParserIn, item: &Item, outer_stops: Stops) -> bool {
...
fn emit_keyword(i: &mut ParserIn, item: Item, kind: SyntaxKind) {
