...
pub(in crate::parser) fn if_nud_normalized(
    mut i: ParserIn,
    mut keyword: Item,
...
fn arm_keyword(i: ParserIn, item: &Item, if_baseline: usize) -> Option<SyntaxKind> {
...
pub(in crate::parser) enum ActiveStatementCompanion {
    Elsif,
    Else,
}

pub(in crate::parser) fn active_statement_companion(
    mut i: ParserIn,
    item: &Item,
    baseline: usize,
    stops: Stops,
...
fn if_arm_normalized(
    ...
    let sequence = Some(crate::parser::context::sequence::SequenceOwner::If);
...
fn else_arm_normalized(
    ...
    let sequence = Some(crate::parser::context::sequence::SequenceOwner::If);
    #[cfg(test)]
    ambient.observe(crate::parser::context::ambient_claim::ProofSite::ElseBody);
...
fn inline_boundary(mut i: ParserIn, item: &Item, baseline: usize, stops: Stops) -> bool {
    item.payload_view().is_eof()
        || crate::parser::input::observation::is_separator(item)
...
fn emit_inline_missing(i: &mut ParserIn, item: &mut Item, baseline: usize) {
    if !implicit_delimited_newline(baseline, item.leading_view()) {
