...
#[cfg(test)]
pub(in crate::parser) fn struct_declaration_selected_normalized(
    i: ParserIn,
    item: &Item,
    baseline: usize,
    item_origin: usize,
...
pub(in crate::parser) fn struct_declaration_selected_lexical(
    source: &str,
    item: &Item,
    baseline: usize,
    item_origin: usize,
...
pub(in crate::parser) fn struct_declaration_normalized(
    mut i: ParserIn,
    intro: Item,
    baseline: usize,
    stops: Stops,
...
fn header_missing(i: &mut ParserIn, item: &Item, origin: usize, role: StructRole) {
...
pub(in crate::parser) enum FieldList {
    NamedBrace,
    Tuple,
}

#[derive(Clone, Copy, Eq, PartialEq)]
pub(in crate::parser) enum FieldOuterClose {
    Recover,
    Borrow,
}

pub(in crate::parser) struct DeclarationFieldExit {
    pub(in crate::parser) exit: NormalizedExit,
    pub(in crate::parser) item_origin: usize,
}
...
#[allow(clippy::too_many_arguments)]
pub(in crate::parser) fn declaration_fields_normalized(
    mut i: ParserIn,
...
pub(in crate::parser) fn named_declaration_fields_next_field_candidate(
    i: ParserIn,
    item: &Item,
    item_origin: usize,
    line_entry: LineEntry,
...
fn indented_end(mut i: ParserIn, item: &Item, baseline: usize, stops: Stops) -> bool {
    item.payload_view().is_eof()
        || indentation_after_newline(item.leading_view()).is_some_and(|indent| indent < baseline)
        || is_active_stop(i.rb(), item, stops)
}

fn emit_missing_field(i: &mut ParserIn, leading: LeadingTrivia, tuple: bool) {
    i.state.start_node(SyntaxKind::StructField.into());
    if tuple {
...
fn emit_missing_field_item(i: &mut ParserIn, item: &mut Item, tuple: bool) {
    i.state.start_node(SyntaxKind::StructField.into());
    if tuple {
        i.state.start_node(SyntaxKind::TypeExpression.into());
    }
...
fn header_boundary(mut i: ParserIn, item: &Item, baseline: usize, stops: Stops) -> bool {
