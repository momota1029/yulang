...
use crate::{parser::input::operator::OperatorSite, syntax_kind::SyntaxKind};
...
pub(in crate::parser) fn binding_statement_selected_lexical(
    source: &str,
    item: &Item,
    baseline: usize,
    item_origin: usize,
    ...
    if crate::parser::declaration::use_decl::use_declaration_selected_lexical(
        source,
        item,
        item_origin,
        fence,
    ...
    if crate::parser::declaration::mod_decl::mod_declaration_selected_lexical(
        source,
        item,
        baseline,
        item_origin,
    ...
    if crate::parser::declaration::struct_decl::struct_declaration_selected_lexical(
        source,
        item,
        baseline,
        item_origin,
    ...
    if crate::parser::declaration::type_decl::type_declaration_selected_lexical(
        source,
        item,
        baseline,
        item_origin,
...
pub(in crate::parser) fn is_binding_visibility(item: &Item) -> bool {
    visibility_word(item).is_some()
}

#[allow(clippy::too_many_arguments)]
pub(in crate::parser) fn binding_statement_normalized(
    mut i: ParserIn,
    visibility: Item,
    baseline: usize,
    stops: Stops,
...
fn binding_target_normalized(
    ...
    let stops = pattern_stops_from_owner(owner_stops)
        | crate::parser::pattern::PATTERN_STOP_COMMA
        | crate::parser::pattern::PATTERN_STOP_SEMICOLON
        | PATTERN_STOP_EQUALS;
    let entry = suffix_marker(i.rb());
    let CurrentItem {
        mut item,
...
fn binding_body_boundary(mut i: ParserIn, item: &Item, baseline: usize, stops: Stops) -> bool {
...
fn emit_binding_body_eof_leading(i: &mut ParserIn, item: &mut Item, baseline: usize, stops: Stops) {
...
fn emit_binding_missing(i: &mut ParserIn, role: BindingRole, item: &Item, origin: usize) {
