...
#[cfg(test)]
pub(in crate::parser) fn error_declaration_witness(
    mut i: ParserIn,
    baseline: usize,
    stops: Stops,
...
#[cfg(test)]
pub(in crate::parser) fn error_declaration_selected_normalized(
    i: ParserIn,
    item: &Item,
    baseline: usize,
    item_origin: usize,
...
pub(in crate::parser) fn error_declaration_selected_lexical(
    source: &str,
    item: &Item,
    baseline: usize,
    item_origin: usize,
...
#[allow(clippy::too_many_arguments)]
pub(in crate::parser) fn error_declaration_normalized(
    mut i: ParserIn,
    intro: Item,
    baseline: usize,
    stops: Stops,
...
fn required_name_normalized(
    ...
    loop {
        emit_token_item(&mut i, item);
        let (mut next, next_origin, next_entry) = error_item_normalized(
            i.rb(),
            *item_origin,
            *line_entry,
            fence,
...
fn parse_variant_body_normalized(
    ...
    let exit = declaration_variant_sequence_normalized(
        i.rb(),
        crate::parser::declaration::declaration_variant::VariantOwner::Error,
        form,
        introducer,
        yield_with,
        baseline,
...
fn recover_body_introducer_normalized(
    ...
    loop {
        emit_token_item(&mut i, item);
        (item, item_origin, line_entry) = error_item_normalized(
            i.rb(),
            item_origin,
            line_entry,
            fence,
...
fn implicit_bodyless_boundary(mut i: ParserIn, item: &Item, baseline: usize, stops: Stops) -> bool {
    item.payload_view().is_boundary()
...
fn emit_parameter(i: &mut ParserIn, parameter: Item) {
