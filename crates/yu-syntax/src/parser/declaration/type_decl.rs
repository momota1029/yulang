...
pub(in crate::parser) fn type_declaration_selected_lexical(
    source: &str,
    item: &Item,
    baseline: usize,
    item_origin: usize,
...
pub(in crate::parser) fn type_declaration_normalized(
    mut i: ParserIn,
    intro: Item,
    baseline: usize,
    stops: Stops,
...
fn definition_boundary(mut i: ParserIn, item: &Item, baseline: usize, stops: Stops) -> bool {
...
fn emit_intro(i: &mut ParserIn, item: Item, kind: SyntaxKind) {
...
fn emit_item_leading(i: &mut ParserIn, item: &mut Item) {
    item.emit_all_remaining_leading(&mut *i.state);
}
