...
use crate::parser::{
    ...
    output::emit::{emit_recovery_error_run, emit_recovery_missing},
};
...
pub(in crate::parser) fn use_declaration_normalized(
    i: ParserIn,
    intro: Item,
    baseline: usize,
    stops: Stops,
...
#[allow(clippy::too_many_arguments)]
pub(in crate::parser) fn use_declaration_header_normalized(
    i: ParserIn,
    intro: Item,
    baseline: usize,
    stops: Stops,
...
pub(in crate::parser) fn use_declaration_selected_normalized(
    i: ParserIn,
    item: &Item,
    item_origin: usize,
    fence: Option<&FenceBoundary>,
...
pub(in crate::parser) fn use_declaration_selected_lexical(
    source: &str,
    item: &Item,
    item_origin: usize,
    fence: Option<&FenceBoundary>,
...
fn missing(i: ParserIn, item: &Item, origin: usize, role: GrammarRole, expected: ExpectedSyntax) {
...
fn missing_close(i: ParserIn, item: &Item, origin: usize, close: char, owner: ConstructRole) {
    missing(
        i,
        item,
        origin,
...
fn declaration_boundary(i: ParserIn, item: &Item, stops: Stops, newline: bool) -> bool {
    i.map(
...
fn declaration_boundary_lex(mut i: LexIn, item: &Item, stops: Stops, newline: bool) -> bool {
    item.payload_view().is_boundary()
        ...
        || crate::parser::input::observation::is_active_stop_lex(i.rb(), item, stops)
        || matches!(
            token_kind(item),
            Some(
                TokenKind::Comma
...
pub(in crate::parser) fn next_use_item_lex(
    mut i: LexIn,
    item_origin: &mut usize,
    line_entry: &mut LineEntry,
    fence: Option<&FenceBoundary>,
...
fn emit_separator(i: &mut ParserIn, item: Item, separator: Separator) {
