...
pub(super) fn string_literal_witness<'source, 'recover, 'operators, 'output, 'frozen>(
    i: ParserIn<'_, 'source, 'recover, 'operators, 'output, 'frozen>,
    opener: Item,
    mode: StringMode,
    part_origin: usize,
    fence: &FenceBoundary,
    mut interpolation_body: impl for<'a> FnMut(
        ParserIn<'a, 'source, 'recover, 'operators, 'output, 'frozen>,
    ) -> Item,
) -> StringLiteralExit
where
    'operators: 'recover,
...
fn string_literal_with_interpolation_body<'source, 'recover, 'operators, 'output, 'frozen>(
    mut i: ParserIn<'_, 'source, 'recover, 'operators, 'output, 'frozen>,
    opener: Item,
    mode: StringMode,
...
fn emit_literal_missing(i: ParserIn, role: LiteralRole, at: usize) {
