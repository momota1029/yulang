...
use crate::parser::{
    ...
    input::{
        ...
        operator::{STOP_LINE_BREAK, active_stop_item},
    },
};
use reborrow_generic::Reborrow as _;

/// Dynamic word operators retain `elsif?` and `else!` through lexical suffix observation.
pub(in crate::parser) fn is_active_stop(i: ParserIn, item: &Item, stops: Stops) -> bool {
    i.map(
...
pub(in crate::parser) fn is_active_stop_lex(mut i: LexIn, item: &Item, stops: Stops) -> bool {
    if token_kind(item).is_some_and(|kind| active_stop_item(kind, stops)) {
        return true;
    }
    (stops & crate::parser::input::operator::STOP_ELSIF != 0
        && is_contextual_word_lex(i.rb(), item, "elsif"))
        || (stops & crate::parser::input::operator::STOP_ELSE != 0
            && is_contextual_word_lex(i, item, "else"))
}

pub(in crate::parser) fn is_contextual_word(mut i: ParserIn, item: &Item, word: &str) -> bool {
...
pub(in crate::parser) fn token_kind(item: &Item) -> Option<TokenKind> {
    let payload = item.payload_view();
...
pub(in crate::parser) fn is_separator(item: &Item) -> bool {
    matches!(
        token_kind(item),
...
pub(in crate::parser) fn is_close(item: &Item) -> bool {
    matches!(
        token_kind(item),
...
pub(in crate::parser) fn delimited_baseline(incoming: usize, leading: LeadingView<'_>) -> usize {
    indentation_after_newline(leading)
        .filter(|&indentation| indentation > incoming)
        .unwrap_or(incoming)
}

pub(in crate::parser) fn implicit_delimited_newline(
    baseline: usize,
    leading: LeadingView<'_>,
) -> bool {
    indentation_after_newline(leading).is_some_and(|indentation| indentation <= baseline)
}

pub(in crate::parser) fn indentation_after_newline(leading: LeadingView<'_>) -> Option<usize> {
    leading.indentation_after_newline()
}

pub(in crate::parser) fn is_line_stop(item: &Item, stops: Stops) -> bool {
