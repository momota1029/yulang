...
pub(in crate::parser) enum LineEntry {
    PhysicalStart,
    InLine,
}

pub(in crate::parser) enum CurrentPayload {
    Token(crate::parser::input::item::Token),
    Operator(crate::parser::input::item::OperatorToken),
}

pub(in crate::parser) struct AcceptedPayload {
    pub(in crate::parser) payload: CurrentPayload,
    pub(in crate::parser) next_line_entry: LineEntry,
}

pub(in crate::parser) struct CurrentItem {
    pub(in crate::parser) item: Item,
    pub(in crate::parser) next_line_entry: LineEntry,
}
...
pub(in crate::parser) fn current_item<P>(
    mut i: LexIn,
    item_origin: usize,
...
fn boundary_item(
    ...
    item_origin: usize,
    boundary: crate::parser::input::item::PendingBoundary,
) -> CurrentItem {
    let next_line_entry = if matches!(
        boundary.kind(),
        crate::parser::input::item::Boundary::EofAfterTrivia
    ) {
        LineEntry::InLine
    } else {
...
) -> Option<crate::parser::input::item::PendingBoundary> {
    let coordinate = suffix_coordinate(source, item_origin, i.remainder());
...
#[cfg(test)]
pub(in crate::parser) fn scan_identifier_item_witness(
    i: LexIn,
    item_origin: usize,
    line_entry: LineEntry,
    fence: Option<&FenceBoundary>,
    ...
    current_item(i, item_origin, line_entry, fence, |mut i, _, _, _, _| {
        let token = i.token(crate::parser::input::lexer::scan_identifier)?;
