...
use crate::parser::{
    ...
    input::item::{LeadingTrivia, Payload},
};
#[cfg(test)]
...
use crate::parser::input::operator::OperatorSite;
...
#[cfg(test)]
pub(in crate::parser) fn statement_item_after_trivia(
    mut i: ParserIn,
...
#[cfg(test)]
pub(in crate::parser) fn scan_statement_item(
    mut i: LexIn,
    baseline: usize,
    stops: Stops,
) -> Option<Item> {
...
pub(in crate::parser) fn scan_statement_payload(
    mut i: LexIn,
...
/// `current_item`.
pub(in crate::parser) fn scan_expression_payload(
    mut i: LexIn,
    site: OperatorSite,
    has_leading_trivia: bool,
...
#[cfg(test)]
pub(in crate::parser) fn scan_nud_payload(
    mut i: LexIn,
    has_leading_trivia: bool,
    payload_origin: usize,
    fence: Option<&FenceBoundary>,
...
pub(in crate::parser) fn scan_path_segment_payload(
    mut i: LexIn,
    has_leading_trivia: bool,
    payload_origin: usize,
    fence: Option<&FenceBoundary>,
...
pub(in crate::parser) fn contextual_word_suffix_follower(i: LexIn) -> Option<bool> {
    Some(!matches!(i.remainder().chars().next(), Some('?' | '!')))
}

pub(in crate::parser) fn introduced_body_indentation_normalized(
    i: ParserIn,
    item_origin: usize,
    fence: Option<&FenceBoundary>,
) -> Option<usize> {
    i.map(
        |lex: LexIn| {
            Some(
                crate::parser::input::operator::newline_indentation_after_fenced_trivia(
                    lex.remainder(),
                    item_origin,
                    LineEntry::InLine,
                    fence,
...
pub(in crate::parser) fn scan_case_label_payload(mut i: LexIn) -> Option<AcceptedPayload> {
    Some(token_payload(i.token(scan_apostrophe_sigil_identifier)?))
}

/// Raw NUD vocabulary for one normalized TypeExpression Item.
pub(in crate::parser) fn scan_type_nud_payload(
    mut i: LexIn,
    has_leading_trivia: bool,
    payload_origin: usize,
    fence: Option<&FenceBoundary>,
...
pub(in crate::parser) fn scan_type_payload(
    mut i: LexIn,
...
pub(in crate::parser) fn scan_pattern_nud_payload(
    mut i: LexIn,
    has_leading_trivia: bool,
    payload_origin: usize,
    fence: Option<&FenceBoundary>,
...
pub(in crate::parser) fn scan_pattern_payload(
    mut i: LexIn,
    _has_leading_trivia: bool,
    _payload_origin: usize,
    _fence: Option<&FenceBoundary>,
    ...
    if stops & crate::parser::pattern::PATTERN_STOP_ARROW != 0
        && let Some(arrow) = i.token(scan_arm_arrow)
    {
...
pub(in crate::parser) fn scan_operator_shaped_unknown(mut i: LexIn) -> Option<Token> {
...
pub(in crate::parser) fn is_operator_shaped_unknown(item: &Item) -> bool {
...
#[cfg(test)]
pub(in crate::parser) fn scan_trivia<S>(mut i: In<'_, &str, &mut Recover<'_>, S>) -> LeadingTrivia
where
    S: Rb,
{
...
pub(in crate::parser) fn scan_trivia_part(mut i: LexIn) -> Option<Trivia> {
    i.check(choice((
...
pub(in crate::parser) enum FencedBlockComment {
    Complete(Trivia),
    Boundary {
        accepted: Trivia,
...
pub(in crate::parser) fn scan_block_comment_fenced(
    mut i: LexIn,
    part_origin: usize,
    fence: &FenceBoundary,
...
#[cfg(test)]
pub(in crate::parser) fn scan_fenced_prior_trivia_part(mut i: LexIn) -> Option<Trivia> {
    i.check(choice((
        token(scan_horizontal_whitespace),
        token(scan_newline),
        token(scan_line_comment),
...
pub(in crate::parser) fn scan_identifier(mut i: LexIn) -> Option<Token> {
...
pub(in crate::parser) fn source_identifier(source: &str) -> Option<(&str, &str)> {
...
pub(in crate::parser) fn source_declaration_head(source: &str) -> bool {
...
pub(in crate::parser) fn scan_apostrophe_sigil_identifier(mut i: LexIn) -> Option<Token> {
...
pub(in crate::parser) fn scan_arm_arrow(mut i: LexIn) -> Option<Token> {
...
pub(in crate::parser) fn is_exact_arm_arrow(remainder: &str) -> bool {
...
pub(in crate::parser) fn scan_path_segment(mut i: LexIn) -> Option<Token> {
...
pub(in crate::parser) fn scan_declaration_type_parameter(mut i: LexIn) -> Option<Token> {
...
pub(in crate::parser) fn is_declaration_starter_word(word: &str) -> bool {
    matches!(
        word,
        "use"
            | "mod"
...
pub(in crate::parser) fn scan_integer(mut i: LexIn) -> Option<Token> {
...
pub(in crate::parser) fn scan_punctuation(i: LexIn) -> Option<Token> {
...
pub(in crate::parser) fn scan_exact_pipe(mut i: LexIn) -> Option<Token> {
    let (accepted, text) = i
        .rb()
...
pub(in crate::parser) fn scan_exact_equals(i: LexIn) -> Option<Token> {
...
pub(in crate::parser) fn is_exact_equals_source(source: &str) -> bool {
    source.starts_with('=')
        && !source[1..]
            .chars()
            .next()
...
#[cfg(test)]
pub(in crate::parser) fn scan_lbrace(i: LexIn) -> Option<Token> {
    let token = scan_punctuation(i)?;
...
pub(in crate::parser) fn scan_unknown(i: LexIn) -> Option<Token> {
