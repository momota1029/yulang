...
impl ErrorRunOutput<'_, '_, '_, '_, '_, '_> {
    ...
    pub(super) fn seal_path_segment_retry_leading_prefix(
        &mut self,
        retry: &mut Item,
        successor_origin: usize,
        category: UnexpectedCategory,
...
pub(super) fn emit_identifier_core(i: &mut ParserIn, item: Item) {
    debug_assert_eq!(
...
pub(super) fn emit_integer_core(i: &mut ParserIn, item: Item) {
...
pub(super) fn emit_operator_use(i: &mut ParserIn, item: Item, kind: SyntaxKind) {
...
pub(super) fn emit_with_keyword(i: &mut ParserIn, item: Item) {
...
pub(super) fn emit_token_item(i: &mut ParserIn, item: Item) {
    let payload = item.payload_view();
...
pub(super) fn emit_literal_item(i: &mut ParserIn, item: Item, kind: SyntaxKind) {
...
#[cfg(test)]
pub(super) fn emit_fragmented_item(i: &mut ParserIn, item: Item) {
    let payload = item.payload_view();
    let kind = if payload.operator_use().is_some() {
...
/// missing slot.
pub(super) fn emit_missing(i: &mut ParserIn, leading: LeadingTrivia) {
...
#[cfg(test)]
pub(super) fn emit_end(output: &mut ParserOutput, end: &mut End) {
