use crate::parser::declaration::declaration_variant::{
    VariantOwner, declaration_variant_owner_witness,
};
...
fn typed_variant_raw_run_keeps_fence_and_contextual_stops_before_name_retry() {
    for owner in [VariantOwner::Enum, VariantOwner::Error] {
        for (yield_with, stops) in [
            (true, 0),
            (false, crate::parser::input::operator::STOP_WITH),
        ] {
            let (green, records, exit, _) = typed_variant(
                "= @ $ with",
                owner,
