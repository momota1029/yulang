...
fn record_caller_and_inherited_close_boundaries_keep_the_complete_pending_item() {
    ...
    for (prefix, expected) in [
        ...
        let inherited = crate::parser::type_expr::with_type_outer_close(0, TokenKind::RParen);
        assert_pending_record(prefix, " /*é*/) tail", 0, inherited, &expected);
    }
    let inherited = crate::parser::type_expr::with_type_outer_close(0, TokenKind::RBracket);
