...
#[cfg(test)]
pub(super) fn type_expr(i: SyntaxIn) -> Option<TailExit> {
...
fn is_type_caller_boundary_parts(
    ...
    if kind.is_some_and(|kind| crate::lexical::stops::active_stop_item(kind, caller_stops)) {
        return true;
    }
    if kind != Some(TokenKind::Identifier) {
        return false;
    }
    (caller_stops & crate::lexical::stops::STOP_WITH != 0 && text == Some("with"))
        || (caller_stops & crate::lexical::stops::STOP_IN != 0 && text == Some("in"))
        || (caller_stops & crate::lexical::stops::STOP_ELSIF != 0 && text == Some("elsif"))
        || (caller_stops & crate::lexical::stops::STOP_ELSE != 0 && text == Some("else"))
}
