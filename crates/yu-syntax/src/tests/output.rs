...
use crate::tests::pattern::pattern_literal_witness;
...
const OPTION_SYNTAX_IN_AUDIT: [OptionEntryAudit; 23] = [
    OptionEntryAudit {
...
fn option_parser_in_audit_has_exactly_the_reviewed_procedures() {
    assert_eq!(OPTION_SYNTAX_IN_AUDIT.len(), 23);
    for (index, entry) in OPTION_SYNTAX_IN_AUDIT.iter().enumerate() {
        assert!(!entry.source.is_empty());
        assert!(!entry.procedure.is_empty());
        assert!(
            !OPTION_SYNTAX_IN_AUDIT[..index].iter().any(|prior| {
                prior.source == entry.source && prior.procedure == entry.procedure
            }),
            "duplicate Option<...> SyntaxIn audit entry: {entry:?}"
        );
    }
    assert_eq!(
        OPTION_SYNTAX_IN_AUDIT
            .iter()
