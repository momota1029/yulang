...
use crate::{
    ...
    lexical::{
        ...
        operator_scan::scan_operator_fenced,
        trivia::{
...
fn emit_accepted_end(item: Item) -> GreenNode {
    let mut builder = GreenNodeBuilder::new();
    builder.start_node(SyntaxKind::Root.into());
    let mut end = crate::handoff::End { item };
    emit_end(&mut builder, &mut end);
    builder.finish_node();
    builder.finish()
}

fn emit_accepted_boundary(item: Item) -> (GreenNode, crate::lexical::item::PendingBoundary) {
    let mut builder = GreenNodeBuilder::new();
    builder.start_node(SyntaxKind::Root.into());
    ...
    let mut pattern_item = current_item(
        ...
        |i, leading, origin, fence, _| {
            scan_pattern_nud_payload(
                ...
                origin,
                fence,
                crate::pattern::PATTERN_DEFAULT_STOPS,
            )
        },
    )
...
fn current_item_rolls_back_an_optional_payload_after_tentative_leading_scan() {
    ...
    let result = current_item(
        ...
        None,
        |mut payload, _, _, _, _| {
            let _ = payload.token(crate::lexical::lexer::scan_identifier)?;
            None
        },
    );
