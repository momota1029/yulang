...
fn use_schema_glob_full_phase_composition() {
    ...
    for (suffix, tail, pending) in [
        ...
        if qualifier {
            ...
            assert_eq!(
                projection(&anchor),
                [(WithKw, 22..26), (Whitespace, 26..27), (UsePath, 27..33)]
            );
            assert_eq!(
    ...
    for (source, glob_end, end, glob_children) in [
        ...
        assert_eq!(
            ...
            [
                ...
                (UsePath, glob_end + 9..end)
            ]
        );
        let path = anchor.children().next().unwrap();
        assert_eq!(projection(&path), [(Identifier, glob_end + 9..end)]);
        for node in root.descendants() {
            for child in node.children_with_tokens() {
                assert_eq!(child.parent().as_ref(), Some(&node));
                assert_eq!(
...
fn use_schema_qualifier_anchor_path_ownership() {
    ...
    for (source, version, anchor_start, path_children, pending) in [
        ...
        (
            ...
            true,
            Some(9),
            vec![(Identifier, 14..20)],
            None,
        ),
        ("use a with", false, Some(6), vec![(Missing, 10..10)], None),
        ("use a with @", false, Some(6), vec![(Error, 11..12)], None),
        (
            "use a with @ /*é*/ target",
            false,
            Some(6),
        ...
        (
            ...
            false,
            Some(6),
            vec![(Identifier, 11..17), (Slash, 17..18), (Error, 18..19)],
            None,
        ),
        (
        ...
        if let Some(start) = anchor_start {
            ...
            let path_start = path_children[0].1.start;
            let mut expected_anchor = vec![(WithKw, start..start + 4)];
            if path_start > start + 4 {
                expected_anchor.push((Whitespace, start + 4..path_start));
            }
            expected_anchor.push((UsePath, path_start..end));
            assert_eq!(projection(anchor), expected_anchor);
            let path = &paths[1];
...
fn use_schema_glob_repeated_exclusion_episodes() {
    ...
    for (suffix, tail, pending) in [
        ...
        if let Some(qualifiers) = qualifiers.first() {
            ...
            let path = anchor.children().next().unwrap();
            assert_eq!(projection(&path), [(Identifier, 27..33)]);
...
#[test]
fn use_schema_recursive_glob_qualifier_siblings() {
    use SyntaxKind::*;
    ...
    for (source, close) in [
        ...
        let qualifiers = tree
            ...
            .find(|node| node.kind() == UseQualifiers)
            .unwrap();
        let mut expected = vec![(Whitespace, 32..33), (UseVersion, 33..35)];
        if close == 47 {
            expected.extend([(Whitespace, 35..36), (UseAnchor, 36..47)]);
        }
        assert_eq!(projection(&qualifiers), expected);
        for node in [&glob, &qualifiers] {
            assert_eq!(
        ...
        // Check every remaining shell as well as the recursive sibling boundary.
        for node in root.descendants() {
            let start = u32::from(node.text_range().start());
            let expected = match (node.kind(), start) {
                (Root, _) => vec![(Statement, 0..close + 1)],
                (Statement, _) => vec![(UseDeclaration, 0..close + 1)],
                (UseDeclaration, _) => {
                    vec![(UseKw, 0..3), (Whitespace, 3..4), (UseTree, 4..close + 1)]
                }
                (UseTree, 4) => vec![(UsePath, 4..5), (ColonColon, 5..7), (UseGlob, 7..close + 1)],
                (UseGlob, 7) => vec![
                    (Star, 7..8),
                    (Whitespace, 8..9),
                    (WithoutKw, 9..16),
                    (Whitespace, 16..17),
                    (UseExclusion, 17..close + 1),
                ],
                (UseExclusion, 17) => vec![(UseExclusionGroup, 17..close + 1)],
                (UsePath, 4 | 18) => vec![(Identifier, start..start + 1)],
                (UseExclusion, 31) => vec![(Identifier, 31..32)],
                (UseVersion, _) => vec![(Version, 33..35)],
                (UseAnchor, _) => vec![(WithKw, 36..40), (Whitespace, 40..41), (UsePath, 41..47)],
                (UsePath, 41) => vec![(Identifier, 41..47)],
                (UseExclusionGroup, _) | (UseTree, 18) | (UseGlob, 21) | (UseQualifiers, _) => {
                    continue;
                }
                _ => panic!("unexpected node: {node:?}"),
            };
            assert_eq!(projection(&node), expected);
        }
        for node in root.descendants() {
            for child in node.children_with_tokens() {
                assert_eq!(child.parent().as_ref(), Some(&node));
            ...
            for frozen in [false, true] {
                ...
                assert_eq!(
                    alias
                        ...
                        .map(|node| node.kind())
                        .collect::<Vec<_>>(),
                    [UseAlias, UseTree, UseDeclaration, Statement, Root]
                );
                assert_use_composition_children(&alias, 6, &alias_children);
                assert_use_composition_children(
                    &alias.parent().unwrap(),
                    4,
                    &[
                ...
                for child in root.descendants_with_tokens() {
                    ...
                    assert!(!matches!(child.kind(), Invalid | UseGlob | UseGroup));
                    if matches!(child.kind(), Missing | Error) {
                ...
                if boundary.is_empty() {
                    ...
                    let Err(Either::Left(mut pending)) = exit else {
                        panic!("semicolon must remain pending: {source:?}")
                    };
                    let leading_len = boundary.find(';').unwrap();
                    let extent = pending.extent(source.len() - input.len());
                    ...
                    assert_eq!(token_kind(&pending), Some(TokenKind::Semicolon));
                    assert_eq!(pending.payload_view().spelling(), Some(";"));
                    let leading = emit_pending_leading_text(&mut pending);
...
fn use_schema_group_and_glob_alias_direct_recovery_and_handoff() {
    ...
    for (head, parent_kind, terminal_kind, terminal_text) in [
        ("use {p}", UseTree, UseGroup, "{p}"),
        ("use p::*", UseGlob, Star, "*"),
    ] {
        for repeated in [false, true] {
            let earlier = if repeated { " as a" } else { "" };
            let alias_start = head.len() + earlier.len() + 1;
            for (tail, alias_children, range, missing) in [
                ("", vec![(AsKw, "as"), (Missing, "")], 8..8, true),
                (
                    " @ #",
                ...
                for boundary in ["", " ;next", "\r\n;next"] {
                    ...
                    for frozen in [false, true] {
                        ...
                        let alias = root
                            .descendants()
                            .filter(|node| node.kind() == UseAlias)
                            .last()
                            .unwrap();
                        assert_eq!(
                            alias
                                ...
                                .map(|node| node.kind())
                                .collect::<Vec<_>>(),
                            if parent_kind == UseGlob {
                                vec![UseAlias, UseGlob, UseTree, UseDeclaration, Statement, Root]
                            } else {
                                vec![UseAlias, UseTree, UseDeclaration, Statement, Root]
                            }
                        );
                        assert_use_composition_children(
                            &alias,
                        ...
                        let mut parent_children = vec![(terminal_kind, terminal_text)];
                        if repeated {
                            parent_children.extend([(Whitespace, " "), (UseAlias, "as a")]);
                        }
                        let alias_text = format!("as{tail}");
                        parent_children
                            .extend([(Whitespace, " "), (UseAlias, alias_text.as_str())]);
                        assert_use_composition_children(
                            &alias.parent().unwrap(),
                            if parent_kind == UseGlob { 7 } else { 4 },
                            &parent_children,
                        );
                        let aliases = alias
                            .parent()
                            ...
                            .filter(|node| node.kind() == UseAlias)
                            .collect::<Vec<_>>();
                        assert_eq!(aliases.len(), if repeated { 2 } else { 1 });
                        if repeated {
                            assert_use_composition_children(
                                &aliases[0],
                                (head.len() + 1) as u32,
                                &[(AsKw, "as"), (Whitespace, " "), (Identifier, "a")],
                            );
                        }
                        for child in alias.children_with_tokens() {
                            assert_eq!(child.as_node().is_some(), child.kind() == Missing);
