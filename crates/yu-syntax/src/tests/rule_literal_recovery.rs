...
fn parenthesized_required_slots_leave_crlf_to_inner_alternation() {
    ...
    for (source, owner_kind, introducer, at) in [
        ...
        let end = source.len();
        let nodes = root.descendants().collect::<Vec<_>>();
        assert_eq!(nodes.len(), 12, "{source:?}");
        for (node, (kind, node_range, parent, children)) in nodes.iter().zip([
            (Root, 0..end, None, vec![RuleBody]),
            (
                RuleBody,
                0..end,
            ...
            (
                ...
                2..end - 2,
                Some(4),
                vec![RuleSequence, Newline, RuleSequence],
            ),
            (RuleSequence, 2..at, Some(5), vec![RuleItem]),
            (RuleItem, 2..at, Some(6), vec![Identifier, owner_kind]),
            (owner_kind, 3..at, Some(7), vec![introducer, Missing]),
            (Missing, at..at, Some(8), vec![]),
            (RuleSequence, at + 2..at + 6, Some(5), vec![RuleItem]),
            (RuleItem, at + 2..at + 6, Some(10), vec![Identifier]),
        ]) {
        ...
        for (token, (kind, token_range, parent)) in tokens.iter().zip([
            (LBrace, 0..1, 1),
            (LParen, 1..2, 4),
            (Identifier, 2..3, 7),
            (introducer, 3..at, 8),
            (Newline, at..at + 2, 5),
            (Identifier, at + 2..at + 6, 11),
            (RParen, end - 2..end - 1, 4),
            (RBrace, end - 1..end, 1),
        ]) {
            assert_eq!(token.kind(), kind);
            assert_eq!(
...
fn dedicated_rule_slots_are_directly_readable_from_the_rowan_tree() {
    ...
    let field = first.first_child().expect("direct RuleField");
    let missing = field.first_child().expect("direct name Missing");
    ...
    for (node, parent, kind, node_range, text, children) in [
        ...
        (
            ...
            1..3,
            "a.",
            vec![SyntaxKind::Identifier, SyntaxKind::RuleField],
        ),
        (
            &field,
            &first,
            ...
            2..3,
            ".",
            vec![SyntaxKind::Dot, SyntaxKind::Missing],
        ),
        (&missing, &field, SyntaxKind::Missing, 3..3, "", vec![]),
        (
            &sequences[1],
            &alternation,
            SyntaxKind::RuleSequence,
    ...
    for (token, (kind, text, token_range, parent)) in tokens.iter().zip([
        ...
        (SyntaxKind::Dot, ".", 2..3, &field),
        (SyntaxKind::Newline, "\n", 3..4, &alternation),
        (SyntaxKind::Identifier, "next", 4..8, &next),
        (SyntaxKind::RBrace, "}", 8..9, &body),
    ]) {
        assert_eq!(token.kind(), kind);
        assert_eq!(token.text(), text);
    ...
    let owner = missing.parent().expect("required-name owner");
    let introducer = owner
        .first_child_or_token()
    ...
    assert_eq!(sequences.len(), 2);
    let first = sequences[0].first_child().expect("first direct RuleItem");
    let path = first.first_child().expect("direct RulePath");
    let missing = path.first_child().expect("direct name Missing");
    let next = sequences[1].first_child().expect("next direct RuleItem");
    for (node, parent, kind, node_range, text, children) in [
        (
            &body,
        ...
        (&missing, &path, SyntaxKind::Missing, 4..4, "", vec![]),
        (
            &sequences[1],
            &alternation,
            SyntaxKind::RuleSequence,
    ...
    for (token, (kind, text, token_range, parent)) in tokens.iter().zip([
        (SyntaxKind::LBrace, "{", 0..1, &body),
        (SyntaxKind::Identifier, "a", 1..2, &first),
        (SyntaxKind::ColonColon, "::", 2..4, &path),
        (SyntaxKind::Newline, "\n", 4..5, &alternation),
