...
pub(super) use crate::{
    ...
    lexical::{
        ...
        operator_scan::scan_operator,
        stops::{STOP_ARROW, STOP_COLON, STOP_ELSE, Stops, stops_for},
...
pub(super) use crate::tests::yumark_cell_witness::{
...
pub(super) fn run_yumark_cell<'source>(
    ...
    GreenNode,
    Result<crate::lexical::item::PendingBoundary, TailExit>,
    &'source str,
) {
    let operators = OperatorTable::empty();
    let mut input = source;
...
pub(super) fn emit_terminal_leading_text(
    item: Item,
) -> (String, crate::lexical::item::PendingBoundary) {
    let mut builder = GreenNodeBuilder::new();
    builder.start_node(SyntaxKind::Root.into());
