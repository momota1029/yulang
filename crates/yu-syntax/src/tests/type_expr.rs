...
fn pattern_annotation_keeps_caller_missing_distinct_from_t1_error() {
    ...
    assert_eq!(
        ...
        [required_recovery::missing(
            0,
            GrammarRole::Pattern(crate::recovery_record::PatternRole::TypeAnnotation),
            2
        )]
    );
...
fn type_parenthesized_t4p_seeded_context_phase_is_lexical_and_frozen_stable() {
    let inactive = crate::type_expr::TypeMlContext::INACTIVE;
    let outer_active = crate::type_expr::TypeMlContext::outer_active_for_test();
    let outer_dormant = crate::type_expr::TypeMlContext::outer_dormant_for_test();
    let non_type_apply = crate::type_expr::TypeMlContext::non_type_apply_active_for_test();
...
fn type_parenthesized_t4p_seeded_owned_boundaries_preserve_pending_item_frontiers() {
    let outer_rbrace = crate::type_expr::with_type_outer_close(0, TokenKind::RBrace);
...
fn type_parenthesized_t4p_dormant_provenance_crosses_forall_without_sibling_records() {
    let context = crate::type_expr::TypeMlContext::outer_dormant_for_test();
...
fn type_call_t3a_missing_phase_suspends_contextual_but_preserves_caller_boundaries() {
    ...
    let active_close_stops = stops_for(TokenKind::RBracket)
        & !crate::lexical::stops::STOP_COMMA
        ...
        for (slot, fresh_slot) in [("", true), ("F", false), ("F,", true), ("F;", true)] {
            for gap in [" ", " \t "] {
                for (payload, caller_stops, outer_closes) in [
                    ...
                    (":", crate::lexical::stops::STOP_COLON, 0),
                    (
                        ...
                        0,
                        crate::type_expr::with_type_outer_close(0, outer_close),
                    ),
                ] {
...
fn shared_delimited_horizontal_local_closes_and_fresh_else_keep_owner_admission() {
    for (owner, opener, close) in [
        ...
        for slot in ["", "F,", "F;"] {
            ...
            let context = crate::type_expr::TypeMlContext::INACTIVE;
