...
fn safe_point(item: &Item, role: OperatorHeaderRole) -> bool {
    ...
    match role {
        ...
        OperatorHeaderRole::LeftBindingPower | OperatorHeaderRole::RightBindingPower => {
            text == "=" || (!power_shaped(text) && super::driver::is_nud_item(item))
        }
        OperatorHeaderRole::DefinitionIntroducer => super::driver::is_nud_item(item),
...
fn next_slot_item(
    ...
    let CurrentItem {
        ...
        |mut lex, gap, payload_origin, _, _| {
            ...
            super::driver::scan_expression_payload_with_literals(
                lex,
                OperatorSite::Nud,
                gap,
                payload_origin,
...
pub(super) fn operator_header_normalized(
    mut i: RewriteIn,
    mut item: Item,
    mut origin: usize,
    mut line: LineEntry,
