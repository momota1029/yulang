...
fn borrowed_close_item() -> Item {
    Item::plain(
        ...
        Payload::Boundary(borrowed_close_boundary()),
    )
}

fn borrowed_close_boundary() -> PendingBoundary {
    PendingBoundary::new(
        40..46,
...
fn transition_item() -> Item {
    Item::plain(
        ...
        Payload::Boundary(transition_boundary()),
    )
}

fn transition_boundary() -> PendingBoundary {
    PendingBoundary::new(
        80..87,
...
fn eof_boundary_item() -> Item {
    Item::plain(
        ...
        Payload::Boundary(eof_boundary()),
    )
}

fn eof_boundary() -> PendingBoundary {
    PendingBoundary::new(120..120, Boundary::EofAfterTrivia)
}
...
fn isolated_cell_composes_current_root_style_statements_on_one_builder() {
    for (source, statements) in [("", 0), ("x", 1), ("x; y", 2), ("our x", 1)] {
        ...
        assert_eq!(exit, Ok(borrowed_close_boundary()), "{source:?}");
        assert_eq!(green.to_string(), format!("{source}  "), "{source:?}");
...
#[test]
fn isolated_cell_terminal_adapter_emits_body_leading_and_returns_exact_facts() {
    let cases = [
        (borrowed_close_item(), borrowed_close_boundary(), "our x  "),
        (transition_item(), transition_boundary(), "our x"),
        (eof_boundary_item(), eof_boundary(), "our x\n"),
    ];
    for (terminal, expected, emitted) in cases {
        let (green, exit, remaining) = run_yumark_cell("our x", terminal);
        assert_eq!(remaining, "");
        assert_eq!(exit, Ok(expected));
...
fn successor_boundary_is_returned_before_ordinary_token_predicates() {
    ...
    assert_eq!(exit, Ok(borrowed_close_boundary()));
...
fn fragmented_identifier_item() -> Item {
    Item::finish(
        ...
        Some(vec![ForeignSplit::quote_prefix(102, 2)]),
        100,
    )
...
#[test]
fn terminal_adapter_emits_fragmented_body_leading_exactly_once() {
    let terminal = fragmented_terminal();
    let (green, exit, remaining) = run_yumark_cell("", terminal);
    assert_eq!(remaining, "");
    assert_eq!(
        exit,
        Ok(PendingBoundary::new(200..202, Boundary::EofAfterTrivia))
    );
    assert_eq!(green.to_string(), "> ");
    let root = SyntaxNode::new_root(green);
...
fn fragmented_terminal() -> Item {
    Item::finish(
        ...
        Some(vec![ForeignSplit::quote_prefix(200, 2)]),
        200,
    )
