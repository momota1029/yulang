...
pub(super) struct AmbientClaimContext<'frame> {
    pub(super) view: Option<AmbientClaimView<'frame>>,
    #[cfg(test)]
...
impl<'frame> AmbientClaimContext<'frame> {
    ...
    pub(super) fn unavailable(self) -> Self {
        Self { view: None, ..self }
    }

    #[inline]
    pub(super) fn if_outer_tail(self) -> Self {
        #[cfg(test)]
    ...
    #[cfg(test)]
    pub(super) fn assert_shape(self, baseline: Option<usize>, frames: &[usize]) {
    ...
    #[cfg(test)]
    pub(super) fn with_proof(mut self, proof: fn(ProofSite, Option<AmbientClaimView<'_>>)) -> Self {
        self.proof = Some(proof);
        self
    }

    #[cfg(test)]
    pub(super) fn observe(self, site: ProofSite) {
...
#[derive(Clone, Copy)]
pub(super) struct AmbientClaimView<'frame> {
...
pub(super) struct AmbientPositionEvidence<'word> {
    ...
    pub(super) following_word: Option<&'word str>,
}

impl<'frame> AmbientClaimView<'frame> {
    pub(super) fn root_statement(baseline: usize) -> Self {
        Self {
    ...
    pub(super) fn statement(self, baseline: usize) -> Self {
        Self {
            statement_baseline: Some(baseline),
            ..self
        }
    ...
    pub(super) fn if_companion(self, baseline: usize) -> AmbientIfCompanion<'frame> {
        AmbientIfCompanion {
