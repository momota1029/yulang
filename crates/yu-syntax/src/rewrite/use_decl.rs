...
fn missing(i: RewriteIn, item: &Item, origin: usize, role: GrammarRole, expected: ExpectedSyntax) {
...
fn missing_close(i: RewriteIn, item: &Item, origin: usize, close: char, owner: ConstructRole) {
    missing(
        i,
        item,
        origin,
...
fn declaration_boundary(i: RewriteIn, item: &Item, stops: Stops, newline: bool) -> bool {
    i.map(
