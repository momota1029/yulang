...
use crate::{
    ...
    lexical::{
        ...
        stops::{STOP_COMMA, STOP_IN, STOP_SEMICOLON, Stops, stops_for},
...
use crate::pattern::delimited::{list_pattern, parenthesized_pattern, record_pattern};
...
pub(super) fn pattern_stops_from_owner(stops: Stops) -> PatternStops {
    ...
    .into_iter()
    .filter_map(|(kind, stop)| crate::lexical::stops::active_stop_item(kind, stops).then_some(stop))
...
#[cfg(test)]
pub(super) fn pattern_with_stops(i: SyntaxIn, stops: PatternStops) -> TailExit {
...
pub(super) fn is_pattern_nud(item: &Item, stops: PatternStops) -> bool {
    (!is_pattern_word_stop(item, stops)
        ...
        || string_mode_from_opener(item)
            .is_some_and(|mode| matches!(mode, crate::literal::StringMode::Heredoc { .. }))
}
...
fn is_pattern_tail_boundary(mut i: SyntaxIn, item: &Item, stops: PatternStops) -> bool {
    token_kind(item).is_some_and(|kind| pattern_tail_stop_token(kind, stops))
        || (stops & PATTERN_STOP_ARM_GUARD_IF != 0
            && crate::lexical::observation::is_contextual_word(i.rb(), item, "if"))
        || (stops & PATTERN_STOP_ARM_GUARD_WHERE != 0
            && crate::lexical::observation::is_contextual_word(i, item, "where"))
        || is_pattern_word_stop(item, stops)
}
...
fn emit_pattern_alias_keyword(i: &mut SyntaxIn, item: Item) {
