...
mod tests {
    ...
    #[test]
    fn legacy_polymorphic_variant_conditional_payload_admission_is_execution_pinned() {
        use crate::session::{
        ...
        // The malformed boundary is admitted only with a non-ambient retry
        // primary. Spacing instead establishes a complete boundary and assigns
        // the malformed run to the payload's TypeExpression recovery.
        // Payload tuples are (boundary start, expression start, expression end,
        // complete horizontal boundary).
        for (source, name_end, rejected_run, payloads, newline, remainder) in [
            (":{A::T}", 3, false, vec![(3, 5, 6, false)], false, ""),
            (":{A->T}", 3, false, vec![(3, 5, 6, false)], false, ""),
            (":{123::T}", 5, false, vec![(5, 7, 8, false)], false, ""),
            (":{123->T}", 5, false, vec![(5, 7, 8, false)], false, ""),
            (":{A::}", 3, true, vec![], false, ""),
            (":{123::}", 5, true, vec![], false, ""),
            (":{A::else: 0", 3, true, vec![], false, "else: 0"),
            (":{A ::T}", 3, false, vec![(3, 6, 7, true)], false, ""),
            (
                ":{A::T U}",
                3,
                false,
                vec![(3, 5, 6, false), (6, 7, 8, true)],
                false,
                "",
            ),
            (":{A\nB}", 3, false, vec![], true, ""),
            ...
            let ambient = !remainder.is_empty();
            let consumed = source.len() - remainder.len();
            let native_close = source.ends_with('}');
            let close_at = consumed - usize::from(native_close);
            let tag_end = payloads.last().map_or(name_end, |p| p.2);
            let mut source_input = SourceInput::new(source);
            let mut local = ParseLocal::new();
            let scope = ambient.then(|| local.push_root_statement_ambient_scope());
            let companion =
                ambient.then(|| local.push_if_expression_companion(0, &["elsif", "else"]));
            // Visible If companion, deliberately no active Else stop frame.
            ...
            if ambient {
                assert_eq!(
                    local.pop_if_expression_companion().map(|f| f.id()),
                    companion
                );
                assert_eq!(local.pop_ambient_owner_scope(), scope);
            }
            assert_eq!(ast.range, 0..consumed, "{source:?}");
            assert!(
            ...
            assert_eq!(pv.range, 0..consumed);
            assert_eq!(pv.colon, 0..1);
            assert_eq!(pv.open, 1..2);
            assert!(pv.trailing_comma.is_none());
            if native_close {
                assert!(
                    matches!(&pv.close, Recovered::Complete(range) if *range == (close_at..consumed))
                );
            } else {
                assert!(matches!(pv.close, Recovered::Incomplete));
            }
            assert_eq!(pv.tags.len(), 1 + usize::from(rejected_run || newline));
            let Recovered::Complete(tag) = &pv.tags[0] else {
                panic!("first tag: {source:?}")
            };
            assert_eq!(tag.range, 2..tag_end);
            if name_end == 5 {
            ...
            assert_eq!(tag.payloads.len(), payloads.len());
            for (payload, &(start, expr_start, end, horizontal)) in
                tag.payloads.iter().zip(&payloads)
            {
                let Recovered::Complete(payload) = payload else {
                    panic!("payload: {source:?}")
                };
                assert_eq!(payload.range, start..end);
                if horizontal {
                    assert!(
                        matches!(&payload.boundary, Recovered::Complete(range) if *range == (start..start + 1))
                    );
                } else {
                ...
                assert_eq!(expr.range, expr_start..end);
                assert!(
                ...
                assert!(
                    matches!(expr.complete_primary(), TypePrimary::Atom(TypeAtom::Identifier(word)) if word.range() == (expr_start..end) && word.text() == &source[expr_start..end])
                );
            }
            if rejected_run {
                assert!(matches!(pv.tags[1], Recovered::Incomplete));
            } else if newline {
                let Recovered::Complete(tag) = &pv.tags[1] else {
                    panic!("newline tag")
                };
                assert_eq!(tag.range, 4..5);
                assert!(tag.payloads.is_empty());
                assert!(
                    matches!(&tag.name, Recovered::Complete(word) if word.text() == "B" && word.range() == (4..5))
                );
            }
            ...
            let output = committed.into_output();
            let records = output.committed_recoveries().to_vec();
            let root = SyntaxNode::new_root(if ambient {
                output.finish_prefix()
            } else {
                output.finish_complete()
            });
            assert_eq!(local.type_expression_episode_depth(), 0);
            assert_eq!(local.type_expression_scoped_stop_frames().count(), 0);
            if ambient {
            ...
            assert_eq!(root.to_string(), source[..consumed]);
            assert_eq!(format!("{root}{remainder}"), source);

            // Pin every node/token in preorder, including its ancestry depth.
            let mut shape = vec![
                (0, SyntaxKind::Root, 0..consumed),
                (1, SyntaxKind::TypeExpression, 0..consumed),
                (2, SyntaxKind::PolymorphicVariantType, 0..consumed),
                (3, SyntaxKind::Colon, 0..1),
                (3, SyntaxKind::LBrace, 1..2),
                (3, SyntaxKind::PolymorphicVariantTag, 2..tag_end),
            ];
            let mut sites = vec![];
            if name_end == 5 {
            ...
            for &(start, expr_start, end, horizontal) in &payloads {
                shape.push((4, SyntaxKind::PolymorphicVariantPayload, start..end));
                let error_start = start + usize::from(horizontal);
                if horizontal {
                    shape.push((5, SyntaxKind::Whitespace, start..start + 1));
                }
                if error_start < expr_start {
                    shape.extend([
                        (5, SyntaxKind::Error, error_start..expr_start),
                        (6, SyntaxKind::Unknown, error_start..expr_start),
                    ]);
                    sites.push((
                        GrammarRole::Type(if horizontal {
                            TypeRole::PolymorphicVariantPayload
                        } else {
                            TypeRole::PolymorphicVariantPayloadBoundary
                        }),
                        RecoveryKind::Error,
                        error_start..expr_start,
                        if horizontal {
                            ExpectedSyntax::TypeExpression
                        } else {
                ...
                shape.extend([
                    (5, SyntaxKind::TypeExpression, expr_start..end),
                    (6, SyntaxKind::Identifier, expr_start..end),
                ]);
            }
            if rejected_run {
                shape.extend([
                    (3, SyntaxKind::PolymorphicVariantTag, name_end..name_end + 2),
                    (4, SyntaxKind::Error, name_end..name_end + 2),
                    (5, SyntaxKind::Unknown, name_end..name_end + 2),
                ]);
                sites.push((
                ...
                shape.extend([
                    (3, SyntaxKind::Newline, 3..4),
                    (3, SyntaxKind::PolymorphicVariantTag, 4..5),
                    (4, SyntaxKind::Identifier, 4..5),
                ]);
            }
            if native_close {
                shape.push((3, SyntaxKind::RBrace, close_at..consumed));
            } else {
                shape.push((3, SyntaxKind::Missing, close_at..close_at));
                sites.push((
                    GrammarRole::ClosingDelimiter {
                        owner: ConstructRole::PolymorphicVariantType,
                        delimiter: Delimiter::Brace,
            ...
            let expected_records = sites
                ...
                .enumerate()
                .map(
                    |(id, (role, kind, range, expected))| CommittedRecoveryRecord {
    ...
    fn legacy_polymorphic_variant_payload_scalar_run_boundaries_are_execution_pinned() {
        ...
        // The same scalar runs begin after the complete wrong-kind head, but
        // the malformed name remains its own recovery before the payload
        // boundary retries the nested PV.  These are direct legacy facts, not
        // a successor classifier or a general admission rule.
        for (source, error_end, nested_start, outer_close) in [
            (":{123+:{B}}", 6, 6, 10),
            (":{123@@:{B}}", 7, 7, 11),
            (":{123::/*c*/:{B}}", 12, 12, 16),
        ] {
            ...
            assert!(pv.trailing_comma.is_none());
            assert!(
                matches!(&pv.close, Recovered::Complete(range) if *range == (outer_close..end))
            );
            let [Recovered::Complete(tag)] = pv.tags.as_slice() else {
                panic!("outer tag: {source:?}")
            };
            assert_eq!(tag.range, 2..outer_close);
            assert!(matches!(tag.name, Recovered::Incomplete));
            let [Recovered::Complete(payload)] = tag.payloads.as_slice() else {
                panic!("payload: {source:?}")
            };
            assert_eq!(payload.range, 5..outer_close);
            assert!(matches!(payload.boundary, Recovered::Incomplete));
            let Recovered::Complete(expr) = &payload.type_expr else {
                panic!("payload expression: {source:?}")
            };
            assert_eq!(expr.range, nested_start..outer_close);
            assert!(
                expr.leading_effect_row.is_none()
                    && expr.postfix.is_empty()
                    && expr.arrow.is_none()
            ...
            assert_eq!(nested.range, nested_start..outer_close);
            assert_eq!(nested.colon, nested_start..nested_start + 1);
            assert_eq!(nested.open, nested_start + 1..nested_start + 2);
            assert!(nested.trailing_comma.is_none());
            assert!(
                matches!(&nested.close, Recovered::Complete(range) if *range == (outer_close - 1..outer_close))
            );
            let [Recovered::Complete(nested_tag)] = nested.tags.as_slice() else {
                panic!("nested tag: {source:?}")
            };
            ...
            assert_eq!(
                ...
                vec![
                    ...
                    (3, SyntaxKind::Colon, 0..1),
                    (3, SyntaxKind::LBrace, 1..2),
                    (3, SyntaxKind::PolymorphicVariantTag, 2..outer_close),
                    (4, SyntaxKind::Error, 2..5),
                    (5, SyntaxKind::Unknown, 2..5),
                    (4, SyntaxKind::PolymorphicVariantPayload, 5..outer_close),
                    (5, SyntaxKind::Error, 5..error_end),
                    (6, SyntaxKind::Unknown, 5..error_end),
                    (5, SyntaxKind::TypeExpression, nested_start..outer_close),
                    (
                        6,
                        SyntaxKind::PolymorphicVariantType,
                        nested_start..outer_close,
                    ),
                    (7, SyntaxKind::Colon, nested_start..nested_start + 1),
                    (7, SyntaxKind::LBrace, nested_start + 1..nested_start + 2),
                    (
                    ...
                    (7, SyntaxKind::RBrace, outer_close - 1..outer_close),
                    (3, SyntaxKind::RBrace, outer_close..end),
                ],
                "{source:?}",
            );
            assert_eq!(
        ...
        // A wrong-kind head has already consumed its own malformed name when
        // CRLF ends the scalar run.  The following malformed nested-PV
        // surface remains a third tag-loop recovery, not a payload retry.
        let source = ":{123::\r\n:{B}}";
        let end = source.len();
        let ast = parse(source);
        assert_eq!(ast.range, 0..end);
        ...
        assert_eq!(pv.open, 1..2);
        assert!(pv.trailing_comma.is_none());
        assert!(matches!(&pv.close, Recovered::Complete(range) if *range == (13..14)));
        let [
            Recovered::Complete(first),
            Recovered::Incomplete,
            Recovered::Complete(last),
            ...
            panic!("wrong-head CRLF tag sequence: {pv:#?}")
        };
        ...
        assert_eq!(
            shape,
            vec![
                (0, SyntaxKind::Root, 0..14),
                (1, SyntaxKind::TypeExpression, 0..14),
                (2, SyntaxKind::PolymorphicVariantType, 0..14),
                (3, SyntaxKind::Colon, 0..1),
                (3, SyntaxKind::LBrace, 1..2),
        ...
        // A block comment after the spaced `::` is not part of that malformed
        // payload run.  With a native close it is PV-local close trivia; at
        // EOF it remains outside the parsed prefix after the payload close
        // slot emits its Missing brace.
        let source = ":{123 ::/*c*/}";
        let ast = parse(source);
        assert_eq!(ast.range, 0..14);
        assert!(ast.leading_effect_row.is_none() && ast.postfix.is_empty() && ast.arrow.is_none());
        let TypePrimary::PolymorphicVariant(pv) = ast.complete_primary() else {
            panic!("native comment PV")
        };
        assert_eq!(pv.range, 0..14);
        assert_eq!(pv.colon, 0..1);
        assert_eq!(pv.open, 1..2);
        assert!(pv.trailing_comma.is_none());
        assert!(matches!(&pv.close, Recovered::Complete(range) if *range == (13..14)));
        let [Recovered::Complete(tag)] = pv.tags.as_slice() else {
            panic!("native comment tag")
        };
        assert_eq!(tag.range, 2..8);
        assert!(matches!(tag.name, Recovered::Incomplete));
        let [Recovered::Complete(payload)] = tag.payloads.as_slice() else {
            panic!("native comment payload")
        };
        assert_eq!(payload.range, 5..8);
        assert!(matches!(&payload.boundary, Recovered::Complete(range) if *range == (5..6)));
        assert!(matches!(payload.type_expr, Recovered::Incomplete));
        ...
        assert_eq!(
            ...
            vec![
                ...
                (3, SyntaxKind::Colon, 0..1),
                (3, SyntaxKind::LBrace, 1..2),
                (3, SyntaxKind::PolymorphicVariantTag, 2..8),
                (4, SyntaxKind::Error, 2..5),
                (5, SyntaxKind::Unknown, 2..5),
                (4, SyntaxKind::PolymorphicVariantPayload, 5..8),
                (5, SyntaxKind::Whitespace, 5..6),
                (5, SyntaxKind::Error, 6..8),
                (6, SyntaxKind::Unknown, 6..8),
                (3, SyntaxKind::BlockComment, 8..13),
                (3, SyntaxKind::RBrace, 13..14),
            ],
        );
        assert_eq!(
        ...
        let ast = i
            .run(from_fn(parse_type_expression))
            .expect("comment AST prefix");
        assert_eq!(i.input.remainder(), "/*c*/");
        drop(i);
        assert_eq!(local.type_expression_episode_depth(), 0);
        assert_eq!(local.type_expression_episode_policy(), None);
        assert_eq!(local.type_expression_scoped_stop_frames().count(), 0);
        assert_eq!(ast.range, 0..8);
        assert!(ast.leading_effect_row.is_none() && ast.postfix.is_empty() && ast.arrow.is_none());
        let TypePrimary::PolymorphicVariant(pv) = ast.complete_primary() else {
            panic!("EOF comment PV")
        };
        assert_eq!(pv.range, 0..8);
        assert_eq!(pv.colon, 0..1);
        assert_eq!(pv.open, 1..2);
        assert!(pv.trailing_comma.is_none());
        assert!(matches!(pv.close, Recovered::Incomplete));
        let [Recovered::Complete(tag)] = pv.tags.as_slice() else {
            panic!("EOF comment tag")
        };
        assert_eq!(tag.range, 2..8);
        assert!(matches!(tag.name, Recovered::Incomplete));
        let [Recovered::Complete(payload)] = tag.payloads.as_slice() else {
            panic!("EOF comment payload")
        };
        assert_eq!(payload.range, 5..8);
        assert!(matches!(&payload.boundary, Recovered::Complete(range) if *range == (5..6)));
        assert!(matches!(payload.type_expr, Recovered::Incomplete));
        ...
        let mut committed = crate::session::Probe::new(i).commit(FullCstOutput::new(source));
        committed.start_node(SyntaxKind::Root);
        commit_direct_type_expression(&mut committed).expect("comment direct prefix");
        assert_eq!(
            committed.probe(|probe| probe.input().input.remainder()),
            "/*c*/",
        );
        ...
        assert_eq!(
            shape,
            vec![
                (0, SyntaxKind::Root, 0..8),
                (1, SyntaxKind::TypeExpression, 0..8),
                (2, SyntaxKind::PolymorphicVariantType, 0..8),
                (3, SyntaxKind::Colon, 0..1),
                (3, SyntaxKind::LBrace, 1..2),
                (3, SyntaxKind::PolymorphicVariantTag, 2..8),
                (4, SyntaxKind::Error, 2..5),
    ...
    #[test]
    fn legacy_polymorphic_variant_payload_scalar_fence_termination_is_execution_pinned() {
        use crate::session::{
            CommittedRecoveryRecord, DiagnosticId, RecoverySiteKey, SyntaxExpectation,
        ...
        for (source, fence, valid_name, tag_error, newline, last_tag) in [
            ...
            let TypePrimary::PolymorphicVariant(pv) = ast.complete_primary() else {
                panic!("fence-qualified outer PV: {source:?}")
            };
            ...
            let [
                ...
                panic!("fence-qualified tag sequence: {pv:#?}")
            };
            assert!(
            ...
            committed.start_node(SyntaxKind::Root);
            commit_direct_type_expression(&mut committed).expect("fence-qualified direct PV");
            committed.probe(|probe| {
            ...
            let mut expected_shape = vec![
                ...
                (3, SyntaxKind::Colon, 0..1),
                (3, SyntaxKind::LBrace, 1..2),
                (3, SyntaxKind::PolymorphicVariantTag, tag_one.clone()),
            ];
            if valid_name {
    ...
    #[test]
    fn legacy_polymorphic_variant_payload_comment_fence_overwrite_is_execution_pinned() {
        use crate::session::{
            CommittedRecoveryRecord, DiagnosticId, RecoverySiteKey, SyntaxExpectation,
            TypeMalformedCallerBoundaryFence, UnexpectedCategory, UnexpectedSyntax,
        };
        ...
        // Unlike the `+`/`@@` cells, this exact comment-bearing physical
        // newline overwrites the incoming fence with the start of its whole
        // comment/trivia run.  This records only that observed legacy fact.
        for (
            source,
            incoming_fence,
            emitted_fence,
            ...
            let mut local = ParseLocal::new();
            local.push_stop_set(stops);
            local.set_type_malformed_caller_boundary(Some(incoming_fence));
            let mut expectations = chasa::LatestSink::new();
            let mut is_cut = false;
            let mut i = In::new(
                &mut source_input,
            ...
            let TypePrimary::PolymorphicVariant(pv) = ast.complete_primary() else {
                panic!("comment-fence outer PV: {source:?}")
            };
            assert_eq!(pv.range, 0..end, "{source:?}");
            assert_eq!(pv.colon, 0..1, "{source:?}");
            assert_eq!(pv.open, 1..2, "{source:?}");
            ...
            let [
                ...
                panic!("comment-fence tag sequence: {pv:#?}")
            };
            assert!(
                first.payloads.is_empty() && last.payloads.is_empty(),
                "{source:?}"
            ...
            committed.start_node(SyntaxKind::Root);
            commit_direct_type_expression(&mut committed).expect("comment-fence direct PV");
            committed.probe(|probe| {
                assert_eq!(probe.input().input.remainder(), "", "{source:?}");
                let local = &mut probe.input().local;
                assert_eq!(local.type_expression_episode_depth(), 0, "{source:?}");
            ...
            expected_shape.extend([
                ...
                (4, SyntaxKind::Error, tag_error.clone()),
                (5, SyntaxKind::Unknown, tag_error.clone()),
                (3, SyntaxKind::BlockComment, comment),
                (3, SyntaxKind::Newline, newline),
                (3, SyntaxKind::PolymorphicVariantTag, last_tag.clone()),
                (4, SyntaxKind::Error, last_tag.clone()),
                (5, SyntaxKind::Unknown, last_tag.clone()),
    ...
    #[test]
    fn legacy_polymorphic_variant_parenthesized_vocabulary_is_execution_pinned() {
        use crate::session::DiagnosticId;

        for (source, wrong, boundary, group, item, recovery) in [
            (
                ":{123(F)}",
                true,
                None,
            ...
            (":{A (F)}", false, Some(3..4), 4..7, 5..6, vec![]),
        ] {
            let end = source.len();
            let ast = parse(source);
            assert_eq!(ast.range, 0..end, "{source:?}");
            ...
            assert_eq!(
                (pv.range.clone(), pv.colon.clone(), pv.open.clone()),
                (0..end, 0..1, 1..2),
                "{source:?}"
            );
            assert!(
                pv.trailing_comma.is_none()
                    && matches!(&pv.close, Recovered::Complete(range) if *range == (end - 1..end)),
                "{source:?}"
            );
            let [Recovered::Complete(tag)] = pv.tags.as_slice() else {
            ...
            assert!(
                if wrong {
                    matches!(tag.name, Recovered::Incomplete)
                } else {
                    matches!(&tag.name, Recovered::Complete(word) if word.text() == "A" && word.range() == (2..3))
                },
                "{source:?}"
            ...
            assert_eq!(
                payload.range,
                boundary.clone().map_or(group.start, |r| r.start)..group.end,
                "{source:?}"
            );
            match (&payload.boundary, &boundary) {
                (Recovered::Incomplete, None) => {}
            ...
            let TypePrimary::Parenthesized(parsed) = expr.complete_primary() else {
                panic!("group: {source:?}")
            };
            assert_eq!(
                (
                ...
                (group.clone(), group.clone(), group.start..group.start + 1),
                "{source:?}"
            );
            assert!(
                matches!(&parsed.close, Recovered::Complete(range) if *range == (group.end - 1..group.end))
                    && parsed.trailing_explicit_separator.is_none(),
                "{source:?}"
            );
            let [Recovered::Complete(inner)] = parsed.elements.as_slice() else {
                panic!("item: {source:?}")
            };
            assert_eq!(inner.range, item.clone(), "{source:?}");
            assert!(
                inner.leading_effect_row.is_none()
                    && inner.postfix.is_empty()
                    && inner.arrow.is_none(),
            ...
            assert!(
                matches!(inner.complete_primary(), TypePrimary::Atom(TypeAtom::Identifier(word)) if word.text() == "F" && word.range() == item),
                "{source:?}"
            );
            ...
            let shape = root
                ...
                .map(|part| {
                    ...
                    (
                        depth,
                        part.kind(),
                        usize::from(part.text_range().start())
                            ..usize::from(part.text_range().end()),
                    )
                })
                .collect::<Vec<_>>();
            let name_end = if wrong { 5 } else { 3 };
            let mut expected = vec![
                (0, SyntaxKind::Root, 0..end),
                (1, SyntaxKind::TypeExpression, 0..end),
                (2, SyntaxKind::PolymorphicVariantType, 0..end),
            ...
            expected.extend([
                (5, SyntaxKind::TypeExpression, group.clone()),
                (6, SyntaxKind::ParenthesizedTypeGroup, group.clone()),
                (7, SyntaxKind::LParen, group.start..group.start + 1),
                (7, SyntaxKind::TypeExpression, item.clone()),
                (8, SyntaxKind::Identifier, item),
                (7, SyntaxKind::RParen, group.end - 1..group.end),
                (3, SyntaxKind::RBrace, end - 1..end),
            ]);
            assert_eq!(shape, expected, "{source:?}");
            let records = parse_direct_recovered(source);
            assert_eq!(records.len(), recovery.len(), "{source:?}");
            for (id, (record, (role, kind, range))) in records.iter().zip(recovery).enumerate() {
                let grammar_role = GrammarRole::Type(role);
                assert_eq!(
                    (
                        record.id,
                ...
                assert_eq!(
                    (
                        ...
                        record.expectations[0].expected,
                        record.expectations[0].range.clone(),
                        record.expectations[0].sources
                    ),
                    (
                        grammar_role,
    ...
    fn legacy_polymorphic_variant_effect_and_call_gap_carriers_are_execution_pinned() {
        ...
        // An apparent numeric Call is a malformed numeric tag name followed
        // by a separate Parenthesized payload in legacy, not a TypeCall.
        // Effect rows and nested PV primaries instead flatten inside Error.
        for (base, prefix_error, name_range, payload) in [
            (":{'[ }", false, 2..5, None),
            (":{'[F, }", false, 2..7, None),
            (":{'[F; }", false, 2..7, None),
            (":{'[F }", false, 2..6, None),
            (":{'[F}", false, 2..5, None),
            (":{'[F ]}", false, 2..7, None),
            (":{123( }", false, 2..5, Some((vec![None], None))),
            (
                ":{123(F, }",
                false,
                2..5,
                Some((vec![Some(6..7), None], None)),
            ),
            (
                ":{123(F; }",
                false,
            ...
            (":{123(F }", false, 2..5, Some((vec![Some(6..7)], None))),
            (":{123(F}", false, 2..5, Some((vec![Some(6..7)], None))),
            (
                ":{123(F )}",
                false,
                2..5,
                Some((vec![Some(6..7)], Some(8..9))),
            ),
            (":{@ '[F }", true, 4..8, None),
            (":{@ 123(F }", true, 4..7, Some((vec![Some(8..9)], None))),
            (":{:{'[F }}", false, 2..9, None),
            (":{:{123(F }}", false, 2..11, None),
        ] {
            for continuation in ["", "::Next"] {
                let source = format!("{base}{continuation}");
                let (remainder, ast) = parse_prefix(&source);
                ...
                assert!(variant.trailing_comma.is_none());
                let [Recovered::Complete(tag)] = variant.tags.as_slice() else {
                    panic!("one recovered tag: {source:?}")
                };
                assert!(matches!(tag.name, Recovered::Incomplete));
                assert_eq!(tag.range, 2..at);
                if let Some((slots, close)) = &payload {
                    let [Recovered::Complete(parsed_payload)] = tag.payloads.as_slice() else {
                        panic!("separate Parenthesized payload: {source:?}")
                    };
                    assert!(matches!(parsed_payload.boundary, Recovered::Incomplete));
                    assert_eq!(parsed_payload.range, name_range.end..at);
                    let Recovered::Complete(expression) = &parsed_payload.type_expr else {
                        panic!("complete payload type: {source:?}")
                    };
                    assert_eq!(expression.range, name_range.end..at);
                    assert!(expression.leading_effect_row.is_none());
                    assert!(expression.postfix.is_empty());
                    assert!(expression.arrow.is_none());
                    let TypePrimary::Parenthesized(group) = expression.complete_primary() else {
                        panic!("legacy admits Parenthesized, not Call: {source:?}")
                    };
                    assert_eq!(group.open, name_range.end..name_range.end + 1);
                    assert_eq!(group.range, name_range.end..at);
                    assert!(group.trailing_explicit_separator.is_none());
                    match (&group.close, close) {
                        (Recovered::Incomplete, None) => {}
                        (Recovered::Complete(actual), Some(expected)) => {
                            assert_eq!(actual, expected)
                        }
                        _ => panic!("local close disposition: {source:?}"),
                    }
                    assert_eq!(group.elements.len(), slots.len());
                    for (element, expected) in group.elements.iter().zip(slots) {
                        match (element, expected) {
                            ...
                            (Recovered::Complete(expression), Some(range)) => {
                                ...
                                assert!(expression.arrow.is_none());
                                assert!(
                                    matches!(expression.complete_primary(), TypePrimary::Atom(TypeAtom::Identifier(word)) if word.text() == "F")
                                );
                            }
                            _ => panic!("payload slot: {source:?}"),
                        }
                    }
                } else {
                    assert!(tag.payloads.is_empty());
                ...
                assert_eq!(
                    variants.len(),
                    1,
                    "nested malformed PV is flattened: {source:?}"
                );
                let children = variants[0].children_with_tokens().collect::<Vec<_>>();
                assert_eq!(
                    children
                        .iter()
                        .map(|child| child.kind())
                ...
                let close = children[3].as_token().expect("actual outer PV close");
                assert_eq!(close.text(), "}");
                assert_eq!(usize::from(close.text_range().start()), at);
                assert_eq!(usize::from(close.text_range().end()), at + 1);
                assert_eq!(close.parent().as_ref(), Some(&variants[0]));
                let tag_node = children[2].as_node().unwrap();
                assert_eq!(usize::from(tag_node.text_range().start()), 2);
                assert_eq!(usize::from(tag_node.text_range().end()), at);
                let tag_children = tag_node.children_with_tokens().collect::<Vec<_>>();
                let mut expected_records = vec![];
                if prefix_error {
                ...
                let errors = root
                    ...
                    .filter(|node| node.kind() == SyntaxKind::Error)
                    .collect::<Vec<_>>();
                assert_eq!(errors.len(), expected_records.len());
                assert_eq!(
                    tag_children.len(),
                    errors.len() + usize::from(payload.is_some())
                );
                for ((error, expected), child) in
                    errors.iter().zip(&expected_records).zip(&tag_children)
                {
                    assert_eq!(child.as_node(), Some(error));
                    assert_eq!(
                        usize::from(error.text_range().start()),
                        expected.site.range.start
                    );
                    assert_eq!(
                        usize::from(error.text_range().end()),
                        expected.site.range.end
                    );
                    let contents = error.children_with_tokens().collect::<Vec<_>>();
                    assert_eq!(contents.len(), 1);
                    let unknown = contents[0].as_token().expect("flattened malformed primary");
                    assert_eq!(unknown.kind(), SyntaxKind::Unknown);
                    assert_eq!(unknown.text_range(), error.text_range());
                    assert_eq!(unknown.text(), &base[expected.site.range.clone()]);
                }
                assert!(!root.descendants().any(|node| matches!(
                    node.kind(),
                    SyntaxKind::TypeCallTail | SyntaxKind::EffectRowType
                )));
                if let Some((slots, close)) = &payload {
                    let payload_node = tag_children.last().unwrap().as_node().unwrap();
                    assert_eq!(payload_node.kind(), SyntaxKind::PolymorphicVariantPayload);
                    assert_eq!(payload_node.text().to_string(), base[name_range.end..at]);
                    expected_records.push(record(
                        expected_records.len(),
                    ...
                    let missing_items = slots.iter().filter(|slot| slot.is_none()).count();
                    if missing_items != 0 {
                        expected_records.push(record(
                            expected_records.len(),
                            GrammarRole::Type(TypeRole::ParenthesizedItem),
                    ...
                    let missing = group
                        ...
                        .collect::<Vec<_>>();
                    assert_eq!(missing.len(), missing_items + usize::from(close.is_none()));
                    ...
                    let gaps = group
                        ...
                        .collect::<Vec<_>>();
                    if let Some(offset) = base[name_range.end..at].find(' ') {
                        assert_eq!(gaps.len(), 1);
                        assert_eq!(gaps[0].to_string(), " ");
                        assert_eq!(
                            usize::from(gaps[0].text_range().start()),
                            name_range.end + offset
                        );
                        assert_eq!(
                            usize::from(gaps[0].text_range().end()),
                            name_range.end + offset + 1
                        );
                    } else {
                    ...
                    let closes = group
                        .children_with_tokens()
                        .filter(|child| child.kind() == SyntaxKind::RParen)
                        .collect::<Vec<_>>();
                    if let Some(range) = close {
                        assert_eq!(closes.len(), 1);
                        assert_eq!(usize::from(closes[0].text_range().start()), range.start);
                        assert_eq!(usize::from(closes[0].text_range().end()), range.end);
                    } else {
    ...
    #[test]
    fn legacy_polymorphic_variant_named_record_vocabulary_is_execution_pinned() {
        use crate::session::DiagnosticId;

        for (source, wrong, boundary, record, field, name, colon, value, recovery) in [
            (
                ":{123{a: F}}",
                true,
                None,
            ...
            assert_eq!(
                payload.range,
                boundary.clone().map_or(record.start, |range| range.start)..record.end,
                "{source:?}"
            );
            match (&payload.boundary, &boundary) {
                (Recovered::Incomplete, None) => {}
            ...
            assert_eq!(expr.range, record, "payload record expression: {source:?}");
            assert!(
                expr.leading_effect_row.is_none()
                    && expr.postfix.is_empty()
                    && expr.arrow.is_none(),
            ...
            let TypePrimary::Record(parsed) = expr.complete_primary() else {
                panic!("record: {source:?}")
            };
            assert_eq!(
                (parsed.range.clone(), parsed.open.clone()),
                (record.clone(), record.start..record.start + 1),
                "{source:?}"
            );
            assert!(
                parsed.trailing_comma.is_none()
                    && matches!(&parsed.close, Recovered::Complete(range) if *range == (record.end - 1..record.end)),
                "{source:?}"
            );
            let [Recovered::Complete(parsed_field)] = parsed.fields.as_slice() else {
                panic!("field: {source:?}")
            };
            assert_eq!(parsed_field.range, field, "{source:?}");
            assert!(
                matches!(&parsed_field.name, Recovered::Complete(word) if word.text() == "a" && word.range() == name)
                    && matches!(&parsed_field.colon, Recovered::Complete(range) if *range == colon),
                "{source:?}"
            );
            let Recovered::Complete(field_expr) = &parsed_field.type_expr else {
                panic!("field expression: {source:?}")
            };
            assert_eq!(field_expr.range, value, "{source:?}");
            assert!(
                field_expr.leading_effect_row.is_none()
                    && field_expr.postfix.is_empty()
                    && field_expr.arrow.is_none(),
                "{source:?}"
            );
            assert!(
                matches!(field_expr.complete_primary(), TypePrimary::Atom(TypeAtom::Identifier(word)) if word.text() == "F" && word.range() == value),
                "{source:?}"
            );
            ...
            expected.extend([
                (5, SyntaxKind::TypeExpression, record.clone()),
                (6, SyntaxKind::NamedRecordType, record.clone()),
                (7, SyntaxKind::LBrace, record.start..record.start + 1),
                (7, SyntaxKind::TypeRecordField, field.clone()),
                (8, SyntaxKind::Identifier, name.clone()),
                (8, SyntaxKind::Colon, colon.clone()),
                (8, SyntaxKind::Whitespace, colon.end..value.start),
                (8, SyntaxKind::TypeExpression, value.clone()),
                (9, SyntaxKind::Identifier, value),
                (7, SyntaxKind::RBrace, record.end - 1..record.end),
                (3, SyntaxKind::RBrace, end - 1..end),
            ]);
            assert_eq!(shape, expected, "{source:?}");
            let records = parse_direct_recovered(source);
            assert_eq!(records.len(), recovery.len(), "{source:?}");
            for (id, (actual, (role, kind, range))) in records.iter().zip(recovery).enumerate() {
                let grammar_role = GrammarRole::Type(role);
                assert_eq!(
                    (
                        actual.id,
                        actual.site.role,
                        actual.site.range.clone(),
                        actual.kind,
                        actual.primary_expectation
                    ),
                    (
                        DiagnosticId(id as u32),
                        grammar_role,
                ...
                assert_eq!(actual.expectations.len(), 1, "{source:?}");
                assert_eq!(
                    (
                        actual.expectations[0].role,
                        actual.expectations[0].expected,
                        actual.expectations[0].range.clone(),
                        actual.expectations[0].sources
                    ),
                    (
                        grammar_role,
                        expected,
                ...
                if kind == RecoveryKind::Missing {
                    assert!(actual.unexpected.is_empty(), "{source:?}");
                } else {
                    assert_eq!(
                        actual.unexpected.as_ref(),
                        [UnexpectedSyntax::Token {
                            range,
                            category: UnexpectedCategory::OtherCharacter
                        }],
