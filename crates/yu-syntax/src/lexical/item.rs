...
impl LeadingTrivia {
    pub(crate) fn ordinary(parts: Box<[Trivia]>) -> Self {
        Self(parts)
    }

    #[cfg(test)]
    pub(crate) fn view(&self) -> LeadingView<'_> {
        LeadingView {
            physical: &self.0,
            first_unemitted: 0,
        }
...
#[derive(Default)]
pub(crate) struct PhysicalLeadingTrivia(Vec<Trivia>);

impl PhysicalLeadingTrivia {
    #[cfg(test)]
    pub(crate) fn from_ordinary(leading: LeadingTrivia) -> Self {
        Self(leading.0.into_vec())
    }

    pub(crate) fn push_ordinary(&mut self, trivia: Trivia) {
        self.0.push(trivia);
    }

    pub(crate) fn push_quote_prefix(&mut self, text: Box<str>) {
        self.0.push(Trivia {
    ...
    pub(crate) fn into_ordinary(self) -> LeadingTrivia {
...
#[derive(Clone, Copy)]
pub(crate) struct LeadingView<'item> {
...
impl<'item> LeadingView<'item> {
    ...
    pub(crate) fn has_ordinary_trivia(self) -> bool {
        self.remaining_physical()
            .any(|part| part.kind != TriviaKind::YmQuotePrefix)
    }

    pub(crate) fn is_grammar_empty(self) -> bool {
    ...
    pub(crate) fn has_ordinary_newline(self) -> bool {
        self.remaining_physical()
            .any(|part| part.kind == TriviaKind::Newline)
    }

    pub(crate) fn has_ordinary_horizontal_gap(self) -> bool {
        self.remaining_physical()
            .any(|part| part.kind == TriviaKind::Whitespace)
    }

    pub(crate) fn indentation_after_newline(self) -> Option<usize> {
    ...
    pub(crate) fn contains_line_break(self) -> bool {
        self.remaining_physical()
            .filter(|part| part.kind != TriviaKind::YmQuotePrefix)
    ...
    pub(crate) fn cut_after_last_ordinary_newline(self) -> Option<usize> {
        self.remaining_physical()
            .enumerate()
    ...
    pub(crate) fn cut_after_first_ordinary_newline(self) -> Option<usize> {
        self.remaining_physical()
            .enumerate()
...
impl Trivia {
    pub(crate) fn whitespace(text: Box<str>) -> Self {
        Self {
    ...
    pub(crate) fn newline(text: Box<str>) -> Self {
        Self {
    ...
    pub(crate) fn line_comment(text: Box<str>) -> Self {
        Self {
    ...
    pub(crate) fn block_comment(text: Box<str>) -> Self {
        Self {
    ...
    pub(crate) fn has_line_feed(&self) -> bool {
...
pub(crate) struct OperatorToken {
    pub(crate) text: Box<str>,
    pub(crate) use_: OperatorUse,
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub(crate) enum OperatorUse {
    Prefix(BindingPower),
    Infix {
        left: BindingPower,
        right: BindingPower,
...
#[derive(Clone, Copy)]
pub(crate) struct PayloadView<'item> {
...
impl<'item> PayloadView<'item> {
    ...
    pub(crate) fn operator_use(self) -> Option<&'item OperatorUse> {
        match self.payload {
    ...
    pub(crate) fn spelling(self) -> Option<&'item str> {
        match self.payload {
    ...
    pub(crate) fn is_eof_after_trivia_boundary(self) -> bool {
        matches!(
            self.payload,
    ...
    pub(crate) fn pending_boundary(self) -> Option<&'item PendingBoundary> {
        match self.payload {
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub(crate) struct PendingBoundary {
    inspected: std::ops::Range<usize>,
    kind: Boundary,
}

impl PendingBoundary {
    pub(crate) fn new(inspected: std::ops::Range<usize>, kind: Boundary) -> Self {
    ...
    #[cfg(test)]
    pub(crate) fn inspected(&self) -> &std::ops::Range<usize> {
        &self.inspected
    }

    pub(crate) fn kind(&self) -> &Boundary {
        &self.kind
    }

    #[cfg(test)]
    pub(crate) fn into_kind(self) -> Boundary {
        self.kind
    }
}
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(crate) struct LayoutEvidence {
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub(crate) enum BorrowedTarget {
    #[allow(
        dead_code,
        reason = "protected pending-boundary vocabulary exercised by recovery/output proofs"
    )]
    Delimiter(Delimiter),
    YumarkFence(Box<crate::lexical::yumark::FenceCloseFacts>),
}
...
pub(crate) enum StopKind {
    ...
    RightBrace,
    YumarkFence(Box<crate::lexical::yumark::YumarkFenceTransition>),
}
...
impl ForeignSplit {
    pub(crate) fn quote_prefix(offset: usize, length: usize) -> Self {
        Self {
            offset,
            length,
...
pub(crate) struct PathSegmentRetryLeadingPrefix {
    end_part: usize,
    range: std::ops::Range<usize>,
}

pub(crate) struct CallArgumentRetryLeadingPrefix {
    end_part: usize,
    range: std::ops::Range<usize>,
}
...
impl ItemExtent {
    pub(crate) fn physical(&self) -> std::ops::Range<usize> {
        self.physical.clone()
    }

    #[cfg(test)]
    pub(crate) fn leading(&self) -> std::ops::Range<usize> {
        self.leading.clone()
    }
    ...
    pub(crate) fn payload(&self) -> std::ops::Range<usize> {
        self.payload.clone()
    }

    pub(crate) fn recovery_range(&self) -> std::ops::Range<usize> {
        self.remaining.start..self.payload.end
    }
}

impl Item {
    pub(crate) fn plain(leading: LeadingTrivia, payload: Payload) -> Self {
        Self {
    ...
    pub(crate) fn leading_view(&self) -> LeadingView<'_> {
        LeadingView {
    ...
    pub(crate) fn extent(&self, successor_origin: usize) -> ItemExtent {
    ...
    pub(crate) fn path_segment_retry_leading_prefix(
        &self,
        successor_origin: usize,
    ...
    pub(crate) fn call_argument_retry_leading_prefix(
        &self,
        successor_origin: usize,
