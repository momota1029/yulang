...
#[cfg(test)]
pub(crate) fn act_declaration_witness(
    mut i: SyntaxIn,
    baseline: usize,
    stops: Stops,
...
#[cfg(test)]
pub(crate) fn act_declaration_selected_normalized(
    i: SyntaxIn,
    item: &Item,
    baseline: usize,
    item_origin: usize,
...
pub(crate) fn act_declaration_selected_lexical(
    source: &str,
    item: &Item,
    baseline: usize,
    item_origin: usize,
...
#[allow(clippy::too_many_arguments)]
pub(crate) fn act_declaration_normalized(
    mut i: SyntaxIn,
    intro: Item,
    baseline: usize,
    stops: Stops,
...
fn recover_body_introducer_normalized(
    ...
    (item, item_origin, line_entry) = act_error_run(
        i.rb(),
        item,
        ActBodyRecoveryPhase::Introducer,
        baseline,
        stops,
        item_origin,
        line_entry,
...
fn inline_body_from_item_normalized(
    ...
    if inline_body_boundary(i.rb(), &item, baseline, stops) {
        act_missing(&mut i, &item, item_origin);
        return complete(handoff(item), line_entry);
    }
...
fn act_missing(i: &mut SyntaxIn, item: &Item, origin: usize) {
...
fn act_error_run(
    ...
    emit_recovery_error_run(i.rb(), |run| {
        loop {
            ...
            let starter = phase == ActBodyRecoveryPhase::Introducer && body_starter(&item);
            ...
            let retry = !boundary
                && (starter
                    || (phase == ActBodyRecoveryPhase::Body
                        && run
                            .lexical(|lex| {
