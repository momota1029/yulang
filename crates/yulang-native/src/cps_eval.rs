        ...
        CpsRuntimeValue::ScopeReturn {
            ...
            if let Some(index) = active_handlers.iter().rposition(|frame| {
                ...
                trace_cps(
                    "ScopeReturnDispatch",
                    format!(
                        "fn={} eval={} prompt={} target={:?} matched=yes install_eval={} owner={} owner_match=yes truncate_at={} action=JumpOrExit",
                        current_function,
                        current_eval_id.0,
                        prompt.0,
                        target,
...
fn capture_continuation_inside_prompt(
    ...
    let return_frames = return_frames[start..]
        .iter()
        .cloned()
        .map(|frame| rebase_captured_return_frame(frame, start, handled))
        .collect();
    CapturedPromptContinuation {
        handlers: handlers
            .iter()
            .cloned()
            .map(|handler| rebase_captured_handler_frame(handler, start, handled))
            .collect(),
...
fn rebase_captured_return_frame(
    ...
    frame.active_handlers = frame
        .active_handlers
        .into_iter()
        .map(|handler| rebase_captured_handler_frame(handler, dropped_frames, handled))
        .collect();
    frame
}
...
fn try_route_scope_return_through_return_frames(
    ...
    for frame_index in (0..return_frames.len()).rev() {
        ...
        let truncate_at = matched_handler.return_frame_threshold;
        ...
        trace_cps(
            "FrameWalkMatch",
            format!(
                "frame_owner={} frame_owner_eval={} handler_prompt={} handler_install_eval={} handlers_before={} handlers_after={} rest_before={} truncate_at={} rest_after={}",
                frame.owner_function,
                frame.owner_eval_id.0,
        ...
        if frame_index < initial_frame_count {
            return Ok(Some(CpsRuntimeValue::RoutedJump(Box::new(routed_jump))));
        }
        let owner = function_by_name(module, &routed_jump.owner_function)?;
        let result = resume_continuation(
            module,
            owner,
            routed_jump.target,
