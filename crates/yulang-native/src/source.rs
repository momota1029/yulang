...
"#,
    ...
    #[test]
    fn evals_list_index_range_raw_source_through_cranelift_value_lane() {
        let items = run_with_large_stack(|| {
            let values = eval_source_value_lane("std::list::index_range_raw [1, 2, 3, 4] 1 3")
                .expect("native value jit eval");
            let [runtime::VmValue::List(list)] = values.as_slice() else {
                panic!("expected one list value");
            };
...
case std::list::view_raw [1]:
    ...
    #[test]
    fn evals_literal_match_source_through_cranelift_value_lane() {
        let values = run_with_large_stack(|| {
            eval_source_value_lane(
                r#"case 2:
    1 -> 9
...
std::str::splice_raw "aあ🙂z" 1 3 "bc""#,
    ...
    #[test]
    fn emits_list_index_range_raw_source_value_object() {
        let object = run_with_large_stack(|| {
...
case std::list::view_raw [1]:
    ...
    #[test]
    fn emits_literal_match_source_value_object() {
        let object = run_with_large_stack(|| {
            compile_source_value_object(
                r#"case 2:
    1 -> 9
