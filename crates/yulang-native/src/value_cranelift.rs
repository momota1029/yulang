...
use crate::native_runtime::{
    ...
    NATIVE_PRIMITIVE_INT_LE, NATIVE_PRIMITIVE_INT_LT, NATIVE_PRIMITIVE_INT_MOD,
    NATIVE_PRIMITIVE_INT_MUL, NATIVE_PRIMITIVE_INT_SUB, NATIVE_PRIMITIVE_INT_TO_HEX,
    NATIVE_PRIMITIVE_INT_TO_STRING, NATIVE_PRIMITIVE_INT_TO_UPPER_HEX, NATIVE_PRIMITIVE_STRING_EQ,
    NATIVE_PRIMITIVE_STRING_INDEX, NATIVE_PRIMITIVE_STRING_LEN, NativeRuntimeContext,
    yulang_native_bool_and, yulang_native_bool_is_true, yulang_native_closure_env_get,
    yulang_native_closure_new, yulang_native_closure_push_env, yulang_native_closure_target_id,
    yulang_native_concat_string, yulang_native_list_empty, yulang_native_list_index,
    yulang_native_list_index_range, yulang_native_list_index_range_raw, yulang_native_list_len,
    yulang_native_list_merge, yulang_native_list_singleton, yulang_native_list_splice,
    yulang_native_list_splice_raw, yulang_native_list_view_raw, yulang_native_make_bool,
    ...
    yulang_native_record_without_field, yulang_native_string_index_range,
...
pub fn compile_value_abi_module(
    ...
    builder.symbol(
        ...
        yulang_native_bool_and as *const u8,
    );
    builder.symbol(
...
fn declare_helpers<M: Module>(module_backend: &mut M) -> NativeValueCraneliftResult<ValueHelpers> {
    Ok(ValueHelpers {
        ...
        bool_and: declare_bool_and(module_backend)?,
...
fn declare_bool_and<M: Module>(module_backend: &mut M) -> NativeValueCraneliftResult<FuncId> {
    let mut sig = module_backend.make_signature();
    sig.params.push(AbiParam::new(types::I64));
    sig.params.push(AbiParam::new(types::I64));
    sig.params.push(AbiParam::new(types::I64));
    sig.returns.push(AbiParam::new(types::I64));
    module_backend
        .declare_function("yulang_native_bool_and", Linkage::Import, &sig)
        .map_err(cranelift_error)
}
...
fn lower_value_stmt<M: Module, L: ValueLiteralStore>(
    ...
    match stmt {
        ...
        NativeAbiStmt::BoolAnd { dest, left, right } => {
            ...
            let left = builder.use_var(variable(*left));
            let right = builder.use_var(variable(*right));
            let callee = module_backend.declare_func_in_func(helpers.bool_and, builder.func);
            let call = builder.ins().call(callee, &[context, left, right]);
            let result = single_call_result(builder, call, "yulang_native_bool_and")?;
            builder.def_var(variable(*dest), result);
            Ok(*dest)
        }
...
fn primitive_binary_code(op: yulang_typed_ir::PrimitiveOp) -> Option<i64> {
    match op {
        ...
        yulang_typed_ir::PrimitiveOp::IntMod => Some(NATIVE_PRIMITIVE_INT_MOD),
        ...
        yulang_typed_ir::PrimitiveOp::StringEq => Some(NATIVE_PRIMITIVE_STRING_EQ),
        _ => None,
    }
}
