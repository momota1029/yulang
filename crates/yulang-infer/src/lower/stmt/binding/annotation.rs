...
pub(crate) fn connect_pattern_sig_annotation(
    ...
    if let Some(fun) = crate::lower::signature::lower_function_sig_shape(state, &sig, &mut vars) {
        ...
        let non_subtract_targets = function_sig_ret_effect_vars(state, &fun);
...
fn function_sig_ret_effect_vars(
    state: &LowerState,
    fun: &crate::lower::signature::LoweredFunctionSigShape,
) -> Vec<TypeVar> {
    let mut vars = collect_pos_free_vars(&state.infer, fun.ret_eff_pos);
    vars.extend(collect_neg_free_vars(&state.infer, fun.ret_eff_neg));
...
fn binding_type_annotation_expr(header: &SyntaxNode) -> Option<SyntaxNode> {
    let pattern = super::super::child_node(header, SyntaxKind::Pattern)?;
    super::super::child_node(&pattern, SyntaxKind::TypeAnn)
        .and_then(|ann| super::super::child_node(&ann, SyntaxKind::TypeExpr))
}
