...
pub(crate) fn lower_synthetic_error_display(
    ...
    let role = display_role_path(state);
    ...
    let show_name = Name("show".to_string());
    ...
    state.register_def_name(method_def, show_name.clone());

    let helper_module = state.ctx.modules.new_module();
    state.insert_module(
        state.ctx.current_module,
        Name(format!("&display#{}", impl_def.0)),
        helper_module,
    );
    state.insert_value(helper_module, show_name.clone(), method_def);

    state.with_type_scope(impl_scope.clone(), |state| {
        state.with_owner(impl_def, |state| {
            lower_synthetic_error_display_body(
                state,
                method_def,
                method_tv,
                error_sig,
...
fn lower_synthetic_error_display_body(
    state: &mut LowerState,
    ...
    let match_eff = state.fresh_exact_pure_eff_tv();
    constrain_cast_result_target(state, match_tv, &str_sig(error_sig.span()), impl_scope);
    ...
    let arms = variants
        .iter()
        .map(|variant| synthetic_error_display_arm(state, variant, arg_tv, match_tv, match_eff))
        .collect::<Vec<_>>();
    let body = TypedExpr {
        tv: match_tv,
        eff: match_eff,
    ...
    store_synthetic_error_display_scheme(state, method_def, error_sig, impl_scope);
    state.runtime_export_schemes.insert(
        method_def,
        synthetic_error_display_runtime_scheme(state, error_sig),
    );
}

fn synthetic_error_display_arm(
    state: &mut LowerState,
...
fn variant_display_text(state: &LowerState, variant: &ErrorThrowVariant) -> String {
    state
        .ctx
        .canonical_value_paths()
        .get(&variant.constructor_def)
        .and_then(|path| path.segments.last())
        .map(|name| name.0.clone())
        .unwrap_or_else(|| "error".to_string())
}

fn store_synthetic_error_display_scheme(
    state: &mut LowerState,
    method_def: crate::ids::DefId,
    error_sig: &SigType,
    impl_scope: &HashMap<String, TypeVar>,
    ...
    let ret = lower_pure_sig_pos_id(state, &str_sig(error_sig.span()), &mut pos_scope);
...
fn synthetic_error_display_runtime_scheme(
    state: &LowerState,
    error_sig: &SigType,
) -> typed_ir::Scheme {
    typed_ir::Scheme {
        ...
        body: typed_ir::Type::Fun {
            ...
            ret: Box::new(export_runtime_sig_type(state, &str_sig(error_sig.span()))),
        },
    }
}

fn str_sig(span: rowan::TextRange) -> SigType {
    SigType::Prim {
        path: Path {
            segments: vec![
                Name("std".to_string()),
...
fn display_role_path(state: &LowerState) -> Path {
    canonical_role_path(
        state,
        &Path {
            segments: vec![Name("Display".to_string())],
        },
    )
}
