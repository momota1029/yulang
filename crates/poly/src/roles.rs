//! Role impl metadata retained for later compiler stages.
//!
//! Inference owns the mutable role solving machinery, but the final `poly::Arena` also needs the
//! resolved source-level impl candidates so downstream stages can instantiate role-constrained
...
impl RoleImplTable {
    ...
    pub fn iter(&self) -> impl Iterator<Item = &RoleImplCandidate> {
        self.candidates
            .values()
...
impl RoleImplCandidate {
    pub fn as_constraint(&self) -> RoleConstraint {
        RoleConstraint {
...
impl RoleConstraint {
    pub fn raw_vars(&self, types: &TypeArena) -> FxHashSet<TypeVar> {
