...
impl DemandEvidenceProfileCounters {
    ...
    pub(super) fn fetch_add_apply_arg_signature_calls(&self) {
        self.apply_arg_signature_calls.fetch_add(1, Ordering::Relaxed);
    }
    pub(super) fn fetch_add_expected_arg_hint_disabled(&self) {
        self.expected_arg_hint_disabled.fetch_add(1, Ordering::Relaxed);
    }
    pub(super) fn fetch_add_expected_arg_hint_present(&self) {
        self.expected_arg_hint_present.fetch_add(1, Ordering::Relaxed);
    }
    pub(super) fn fetch_add_expected_arg_hint_converted(&self) {
        self.expected_arg_hint_converted.fetch_add(1, Ordering::Relaxed);
    }
    ...
    pub(super) fn fetch_add_expected_arg_hint_changed_signature(&self) {
        self.expected_arg_hint_changed_signature.fetch_add(1, Ordering::Relaxed);
    }
    pub(super) fn fetch_add_expected_arg_hint_same_signature(&self) {
        self.expected_arg_hint_same_signature.fetch_add(1, Ordering::Relaxed);
    }
    pub(super) fn fetch_add_expected_arg_hint_rejected_open(&self) {
        self.expected_arg_hint_rejected_open.fetch_add(1, Ordering::Relaxed);
    }
    pub(super) fn fetch_add_apply_callee_signature_calls(&self) {
        self.apply_callee_signature_calls.fetch_add(1, Ordering::Relaxed);
    }
    pub(super) fn fetch_add_expected_callee_hint_disabled(&self) {
        self.expected_callee_hint_disabled.fetch_add(1, Ordering::Relaxed);
    }
    pub(super) fn fetch_add_expected_callee_hint_present(&self) {
        self.expected_callee_hint_present.fetch_add(1, Ordering::Relaxed);
    }
    pub(super) fn fetch_add_expected_callee_hint_converted(&self) {
        self.expected_callee_hint_converted.fetch_add(1, Ordering::Relaxed);
    }
    pub(super) fn fetch_add_expected_callee_hint_used(&self) {
        self.expected_callee_hint_used.fetch_add(1, Ordering::Relaxed);
    }
    pub(super) fn fetch_add_expected_callee_hint_changed_param_signature(&self) {
        self.expected_callee_hint_changed_param_signature.fetch_add(1, Ordering::Relaxed);
    }
    pub(super) fn fetch_add_expected_callee_hint_same_param_signature(&self) {
        self.expected_callee_hint_same_param_signature.fetch_add(1, Ordering::Relaxed);
    }
    pub(super) fn fetch_add_expected_callee_hint_rejected_open(&self) {
        self.expected_callee_hint_rejected_open.fetch_add(1, Ordering::Relaxed);
    }
    pub(super) fn fetch_add_expected_callee_hint_rejected_non_function(&self) {
        self.expected_callee_hint_rejected_non_function.fetch_add(1, Ordering::Relaxed);
    }
}
