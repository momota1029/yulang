...
pub struct ApplyShape {
    ...
    pub target: Option<typed_ir::Path>,
    pub callee_kind: ApplyHeadKind,
    pub callee_intrinsic: Option<typed_ir::TypeBounds>,
    pub callee_contextual: Option<typed_ir::TypeBounds>,
    pub arg_intrinsic: Option<typed_ir::TypeBounds>,
    pub arg_contextual: Option<typed_ir::TypeBounds>,
    pub result_intrinsic: Option<typed_ir::TypeBounds>,
    pub callee_source_edge: Option<u32>,
    pub arg_source_edge: Option<u32>,
    pub principal_callee: Option<typed_ir::Type>,
    pub substitutions: Vec<typed_ir::TypeSubstitution>,
    pub substitution_candidates: Vec<typed_ir::PrincipalSubstitutionCandidate>,
    pub principal_elaboration: Option<typed_ir::PrincipalElaborationPlan>,
...
pub struct ValueShape {
    pub intrinsic: Option<typed_ir::TypeBounds>,
    pub contextual: Option<typed_ir::TypeBounds>,
}
...
pub(super) fn profile_core_program(program: &typed_ir::CoreProgram) -> CoreShapeProfile {
...
pub(super) fn collect_core_shape_table(program: &typed_ir::CoreProgram) -> ShapeTable {
...
impl ShapeCollector {
    fn walk_expr(&mut self, expr: &typed_ir::Expr, path: ExprPath) {
    ...
    fn walk_stmt(&mut self, stmt: &typed_ir::Stmt, path: ExprPath) {
        match stmt {
            typed_ir::Stmt::Let { pattern, value } => {
                self.walk_pattern_defaults(pattern, &path);
                self.walk_expr(value, path);
            }
            typed_ir::Stmt::Expr(expr) => self.walk_expr(expr, path),
    ...
    fn walk_record_spread(&mut self, spread: &typed_ir::RecordSpreadExpr, path: ExprPath) {
        match spread {
    ...
    fn walk_pattern_defaults(&mut self, pattern: &typed_ir::Pattern, path: &ExprPath) {
        match pattern {
            typed_ir::Pattern::Wildcard
            | typed_ir::Pattern::Bind(_)
            | typed_ir::Pattern::Lit(_) => {}
...
fn expr_shape(expr: &typed_ir::Expr, path: ExprPath) -> ExprShape {
...
fn value_shape(expr: &typed_ir::Expr) -> ValueShape {
    match expr {
        typed_ir::Expr::Apply { evidence, .. } => ValueShape {
            intrinsic: evidence.as_ref().map(|evidence| evidence.result.clone()),
            contextual: None,
        },
        typed_ir::Expr::If { evidence, .. }
        | typed_ir::Expr::Match { evidence, .. }
        | typed_ir::Expr::Handle { evidence, .. } => ValueShape {
            intrinsic: evidence.as_ref().map(|evidence| evidence.result.clone()),
            contextual: None,
        },
        typed_ir::Expr::Coerce { evidence, .. } => ValueShape {
...
fn apply_shape_status(evidence: Option<&typed_ir::ApplyEvidence>) -> ApplyShapeStatus {
...
fn bounds_present(bounds: &typed_ir::TypeBounds) -> bool {
...
fn expr_shape_kind(expr: &typed_ir::Expr) -> ExprShapeKind {
    match expr {
        typed_ir::Expr::Var(_) => ExprShapeKind::Var,
        typed_ir::Expr::PrimitiveOp(_) => ExprShapeKind::PrimitiveOp,
        typed_ir::Expr::Lit(_) => ExprShapeKind::Lit,
        typed_ir::Expr::Lambda { .. } => ExprShapeKind::Lambda,
        typed_ir::Expr::Apply { .. } => ExprShapeKind::Apply,
        typed_ir::Expr::If { .. } => ExprShapeKind::If,
        typed_ir::Expr::Tuple(_) => ExprShapeKind::Tuple,
        typed_ir::Expr::Record { .. } => ExprShapeKind::Record,
        typed_ir::Expr::Variant { .. } => ExprShapeKind::Variant,
        typed_ir::Expr::Select { .. } => ExprShapeKind::Select,
        typed_ir::Expr::Match { .. } => ExprShapeKind::Match,
        typed_ir::Expr::Block { .. } => ExprShapeKind::Block,
        typed_ir::Expr::Handle { .. } => ExprShapeKind::Handle,
        typed_ir::Expr::Coerce { .. } => ExprShapeKind::Coerce,
        typed_ir::Expr::BindHere { .. } => ExprShapeKind::BindHere,
        typed_ir::Expr::Pack { .. } => ExprShapeKind::Pack,
    }
}
...
fn apply_head_kind(expr: &typed_ir::Expr) -> ApplyHeadKind {
    match expr {
        typed_ir::Expr::Var(path) => ApplyHeadKind::Path(path.clone()),
        typed_ir::Expr::PrimitiveOp(op) => ApplyHeadKind::Primitive(*op),
        typed_ir::Expr::Apply { callee, .. } => apply_head_kind(callee),
...
impl ExprPath {
    fn owner(&self) -> Option<typed_ir::Path> {
...
mod tests {
    ...
    fn apply_with_evidence_fills_apply_shape_slots() {
        let evidence = typed_ir::ApplyEvidence {
            ...
            substitution_candidates: vec![typed_ir::PrincipalSubstitutionCandidate {
                var: typed_ir::TypeVar("b".to_string()),
        ...
        let program = program_with_root(typed_ir::Expr::Apply {
            callee: Box::new(typed_ir::Expr::Var(path("f"))),
            ...
            program: typed_ir::PrincipalModule {
                ...
                bindings: vec![typed_ir::PrincipalBinding {
                    ...
                    body: typed_ir::Expr::Lambda {
                        ...
                        body: Box::new(typed_ir::Expr::Var(binding_path)),
                    },
                }],
    ...
    fn program_with_root(expr: typed_ir::Expr) -> typed_ir::CoreProgram {
    ...
    fn bounds(ty: typed_ir::Type) -> typed_ir::TypeBounds {
        typed_ir::TypeBounds::exact(ty)
    }
