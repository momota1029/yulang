...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
struct EvidenceProviderEnvLaterGrantPlacement {
    depth: Option<usize>,
    under_then_first: bool,
    under_then_second: bool,
...
impl EvidenceProviderEnvLaterGrantPlacement {
    fn none() -> Self {
        Self {
            depth: None,
            under_then_first: false,
            under_then_second: false,
            in_provider_env_next: false,
        }
    }
...
struct EvidenceResumePlanPlannedCandidate {
    ...
    boundary_lane: EvidenceScopePlanBoundaryLane,
...
impl EvidenceScopePlanBoundaryLane {
    fn from_shadow_profile(profile: EvidenceResumePlanShadowProfile) -> Self {
        if profile.ref_set_boundaries > 0 {
            return Self::RefSet {
                boundaries: profile.ref_set_boundaries,
            };
        }
    ...
    fn has_ref_set(self) -> bool {
        matches!(self, Self::RefSet { boundaries } if boundaries > 0)
    }
}
...
#[derive(Debug, Clone, Copy, Default, PartialEq, Eq)]
struct EvidenceSidecarPlanShadowProfile {
    marker_frames: usize,
    marker_active_add_id_ops: usize,
    marker_handler_boundary_ops: usize,
    provider_frames: usize,
    provider_slots: usize,
    provider_handler_candidates: usize,
    child_marker_frames: usize,
    legacy_frame_avoid_estimate: usize,
}
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum EvidenceSidecarPlanShadow {
    Planned(EvidenceSidecarPlanShadowProfile),
    Rejected(EvidenceSidecarPlanShadowReject),
}
...
impl EvidenceResumePlanTracePlan {
    fn from_continuation(
        ...
        if let Some(frame) = continuation.frame() {
            frame.append_resume_plan_trace(&mut steps, &mut cursor, routed_yield_handler, true);
        }
        Self {
...
impl EvidenceResumeScopedTracePlan {
    ...
    fn sidecar_plan_shadow(
        &self,
        input: EvidenceSidecarPlanShadowInput,
    ) -> EvidenceSidecarPlanShadow {
        if self.root_marker_count() != input.marker_frames {
            return EvidenceSidecarPlanShadow::Rejected(
                EvidenceSidecarPlanShadowReject::RootMarkerMismatch,
            );
        }
        if self.provider_child_scope_count() != input.provider_frames {
            return EvidenceSidecarPlanShadow::Rejected(
                EvidenceSidecarPlanShadowReject::ProviderScopeMismatch,
            );
        }
        EvidenceSidecarPlanShadow::Planned(EvidenceSidecarPlanShadowProfile {
            marker_frames: input.marker_frames,
            marker_active_add_id_ops: input.marker_active_add_id_ops,
            marker_handler_boundary_ops: input.marker_handler_boundary_ops,
            provider_frames: input.provider_frames,
            provider_slots: input.provider_slots,
            provider_handler_candidates: input.provider_handler_candidates,
            child_marker_frames: self.child_marker_count(),
            legacy_frame_avoid_estimate: input.marker_frames + input.provider_frames,
        })
    }
}
    ...
    fn append_frame(
        ...
        match frame {
            ...
            EvidenceContinuationFrame::ScopePlanForeignCatchBoundary {
                scope: scope_continuation,
                boundaries,
                next,
                ..
            } => {
                if let Some(frame) = scope_continuation.frame() {
                    self.append_frame(scope, frame, routed_yield_handler);
                }
                for _ in boundaries.iter() {
                    let index = self.cursors[scope].request;
...
impl EvidenceEvalDeltaFramePlan {
    ...
    fn from_continuation_frame(frame: &EvidenceContinuationFrame) -> Option<Self> {
        match frame {
            EvidenceContinuationFrame::ForceThunk {
                target_value_is_thunk,
                ..
            } => Some(Self::ForceThunk {
                target_value_is_thunk: *target_value_is_thunk,
            }),
            EvidenceContinuationFrame::ForceValueIfThunk { .. } => Some(Self::ForceValueIfThunk),
            EvidenceContinuationFrame::ApplyCallee { site, arg, env, .. } => {
                Some(Self::ApplyCallee {
                    site: *site,
                    arg: *arg,
                    env: env.clone(),
                })
            }
            EvidenceContinuationFrame::ApplyArg { site, callee, .. } => Some(Self::ApplyArg {
                site: *site,
                callee: callee.clone(),
            }),
            EvidenceContinuationFrame::ApplyForcedCallee { site, arg, .. } => {
                Some(Self::ApplyForcedCallee {
                    site: *site,
                    arg: arg.clone(),
                })
            }
            EvidenceContinuationFrame::AdaptValue { source, target, .. } => {
                Some(Self::AdaptValue {
                    source: source.clone(),
                    target: target.clone(),
                })
            }
            ...
            EvidenceContinuationFrame::ApplyAdapterResult {
                ...
                target_ret,
                ..
            } => Some(Self::ApplyAdapterResult {
                ret_markers: ret_markers.clone(),
                source_ret: source_ret.clone(),
                target_ret: target_ret.clone(),
            }),
            EvidenceContinuationFrame::CaseScrutinee { arms, env, .. } => {
                Some(Self::CaseScrutinee {
                    arms: arms.clone(),
                    env: env.clone(),
                })
            }
            EvidenceContinuationFrame::TupleItems {
                values, rest, env, ..
            } => Some(Self::TupleItems {
                values: values.clone(),
                rest: rest.clone(),
                env: env.clone(),
            }),
            EvidenceContinuationFrame::RecordSpread { fields, env, .. } => {
                Some(Self::RecordSpread {
                    fields: fields.clone(),
                    env: env.clone(),
                })
            }
            ...
            EvidenceContinuationFrame::RefSetReference { value, env, .. } => {
                Some(Self::RefSetReference {
                    value: *value,
                    env: env.clone(),
                })
            }
            EvidenceContinuationFrame::RefSetForcedReference { value, env, .. } => {
                Some(Self::RefSetForcedReference {
                    value: *value,
                    env: env.clone(),
                })
            }
            EvidenceContinuationFrame::RefSetValue { reference, .. } => Some(Self::RefSetValue {
                reference: reference.clone(),
            }),
            EvidenceContinuationFrame::RefSetForcedValue { reference, .. } => {
                Some(Self::RefSetForcedValue {
                    reference: reference.clone(),
                })
            }
            EvidenceContinuationFrame::RefSetResolvedUnit { .. } => Some(Self::RefSetResolvedUnit),
            EvidenceContinuationFrame::RefSetEmitResolvedRequest {
                request,
                assigned,
                mode,
                ..
            } => Some(Self::RefSetEmitResolvedRequest {
    ...
    fn to_continuation(&self, next: EvidenceContinuation) -> EvidenceContinuation {
        match self {
            Self::ForceThunk {
                target_value_is_thunk,
            } => EvidenceContinuation::force_thunk(*target_value_is_thunk, next),
            Self::ForceValueIfThunk => EvidenceContinuation::force_value_if_thunk(next),
            Self::ApplyCallee { site, arg, env } => {
                EvidenceContinuation::apply_callee(*site, *arg, env.clone(), next)
            }
            Self::ApplyArg { site, callee } => {
                EvidenceContinuation::apply_arg(*site, callee.clone(), next)
            }
            Self::ApplyForcedCallee { site, arg } => {
                EvidenceContinuation::apply_forced_callee(*site, arg.clone(), next)
            }
            ...
            Self::CaseScrutinee { arms, env } => {
                EvidenceContinuation::case_scrutinee(arms.clone(), env.clone(), next)
            }
            Self::TupleItems { values, rest, env } => {
                EvidenceContinuation::tuple_items(values.clone(), rest.clone(), env.clone(), next)
            }
            Self::RecordSpread { fields, env } => {
                EvidenceContinuation::record_spread(fields.clone(), env.clone(), next)
            }
            Self::RecordFields { values, rest, env } => {
                EvidenceContinuation::record_fields(values.clone(), rest.clone(), env.clone(), next)
            }
            ...
            Self::SelectBase { name, resolution } => {
                EvidenceContinuation::select_base(name.clone(), resolution.clone(), next)
            }
            Self::BlockStmt {
                resume,
                rest,
            ...
            Self::RefSetReference { value, env } => {
                EvidenceContinuation::ref_set_reference(*value, env.clone(), next)
            }
            Self::RefSetForcedReference { value, env } => {
                EvidenceContinuation::ref_set_forced_reference(*value, env.clone(), next)
            }
            Self::RefSetValue { reference } => {
                EvidenceContinuation::ref_set_value(reference.clone(), next)
            }
            Self::RefSetForcedValue { reference } => {
                EvidenceContinuation::ref_set_forced_value(reference.clone(), next)
            }
            Self::RefSetResolvedUnit => EvidenceContinuation::ref_set_resolved_unit(next),
            Self::RefSetEmitResolvedRequest {
                request,
                assigned,
                mode,
...
fn catch_boundaries_pass_direct_tail_signal(
    boundaries: &[EvidenceCatchBoundaryDeltaPlan],
    handler: ExprId,
) -> bool {
    boundaries
        .iter()
        .all(|boundary| boundary.catch_expr != handler)
}

fn catch_boundaries_are_foreign_pass_through(
    boundaries: &[EvidenceCatchBoundaryDeltaPlan],
) -> bool {
    !boundaries.is_empty()
        && boundaries
            .iter()
            .all(|boundary| boundary.is_signal_passthrough() && boundary.has_value_resume_payload())
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum EvidenceScopePlanDirectTailSignalReject {
    ScopeHasRequestBoundary,
    SameHandlerBoundary,
}

fn scope_plan_direct_tail_signal_compact_reject(
    scope_direct_tail_boundary_free: bool,
    boundaries: &[EvidenceCatchBoundaryDeltaPlan],
    handler: ExprId,
) -> Option<EvidenceScopePlanDirectTailSignalReject> {
    if !scope_direct_tail_boundary_free {
        return Some(EvidenceScopePlanDirectTailSignalReject::ScopeHasRequestBoundary);
    }
    if !catch_boundaries_pass_direct_tail_signal(boundaries, handler) {
        return Some(EvidenceScopePlanDirectTailSignalReject::SameHandlerBoundary);
    }
    None
}
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
struct EvidenceProviderDirtyScopeCert {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
struct EvidenceForeignCatchPassThroughCert {
...
struct EvidenceProviderDirtyForeignCatchScopeCert {
    provider: EvidenceProviderDirtyScopeCert,
    catch: EvidenceForeignCatchPassThroughCert,
}
...
impl EvidenceResumePlan {
    fn provider_dirty_foreign_catch_scope(
        ...
        Self {
            ...
            trace: cert.provider.trace,
            eval_delta: cert.provider.eval_delta,
            request_delta: cert.catch.request_delta,
            marker_delta: cert.provider.marker_delta,
            provider_delta: cert.provider.provider_delta,
        }
    }
}
...
fn resume_plan_candidate_from_shadow(
    ...
    let boundary_lane = EvidenceScopePlanBoundaryLane::from_shadow_profile(profile);
    if boundary_lane.has_ref_set() {
...
fn resume_plan_execution_cert_from_candidate(
    ...
    let EvidenceScopePlanBoundaryLane::Catch {
        same_handler,
        no_routed_handler,
        foreign_handler,
    } = candidate.boundary_lane
    else {
        return EvidenceResumePlanExecutionCert::Rejected(
...
#[derive(Debug, Clone, PartialEq)]
enum EvidenceScopeSidecarAction {
    Eval(EvidenceEvalDeltaFramePlan),
    CatchForeign {
        boundaries: Rc<[EvidenceCatchBoundaryDeltaPlan]>,
    },
    Marker {
        frame: EvidenceMarkerDeltaFramePlan,
        child: EvidenceScopeSidecarPlan,
    },
    Provider {
        frame: EvidenceProviderDeltaFramePlan,
        child: EvidenceScopeSidecarPlan,
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum EvidenceScopeLocalPlanReject {
    RequestBoundary,
    RefSetBoundary,
}
...
impl EvidenceScopeLocalPlan {
    fn from_continuation(
        ...
        let mut builder = EvidenceScopeLocalPlanBuilder::default();
        builder.append_continuation(continuation)?;
        Ok(builder.finish())
    }
    ...
    fn materialize_actions(
        actions: &[EvidenceScopeLocalAction],
...
impl EvidenceScopeSidecarPlan {
    fn from_continuation(
        continuation: &EvidenceContinuation,
    ) -> Result<Self, EvidenceScopeSidecarPlanReject> {
        let mut builder = EvidenceScopeSidecarPlanBuilder::default();
        builder.append_continuation(continuation)?;
        Ok(builder.finish())
    }
...
impl EvidenceScopeLocalAction {
    fn materialize_before(&self, tail: EvidenceContinuation) -> EvidenceContinuation {
        match self {
            ...
            Self::Marker { frame, child } => {
                let scoped = EvidenceContinuation::marker_frame(
                    frame.markers.clone(),
                    ...
                    child.materialize(EvidenceContinuation::identity()),
                );
                if tail.is_identity() {
                    scoped
                } else {
            ...
            Self::Provider { frame, child } => {
                let scoped = EvidenceContinuation::provider_env(
    ...
    fn materialize_before(&self, tail: EvidenceContinuation) -> EvidenceContinuation {
        match self {
            ...
            Self::Marker { frame, child } => {
                ...
                append_continuation_unprofiled(scoped, tail)
            }
            Self::Provider { frame, child } => {
                let scoped = EvidenceContinuation::provider_env(
                    frame.provider_env.clone(),
...
fn append_continuation_unprofiled(
    head: EvidenceContinuation,
    tail: EvidenceContinuation,
) -> EvidenceContinuation {
    if tail.is_identity() {
        return head;
...
impl EvidenceScopeLocalPlanBuilder {
    fn finish(self) -> EvidenceScopeLocalPlan {
        EvidenceScopeLocalPlan {
            actions: Rc::from(self.actions.into_boxed_slice()),
        }
    }
    ...
    fn append_frame(
        ...
        match frame {
            ...
            EvidenceContinuationFrame::MarkerFrame {
                ...
                let child = EvidenceScopeLocalPlan::from_continuation(next)?;
                self.actions.push(EvidenceScopeLocalAction::Marker {
                    frame: EvidenceMarkerDeltaFramePlan {
                        markers: markers.clone(),
                        activate_add_ids: *activate_add_ids,
                        handler_path: handler_path
            ...
            EvidenceContinuationFrame::ProviderEnv { provider_env, next } => {
                let child = EvidenceScopeLocalPlan::from_continuation(next)?;
                self.actions.push(EvidenceScopeLocalAction::Provider {
                    frame: EvidenceProviderDeltaFramePlan {
                        provider_env: provider_env.clone(),
                    },
                    child,
            ...
            EvidenceContinuationFrame::CatchForeignBoundarySegment { boundaries, next } => {
                ...
                self.actions.push(EvidenceScopeLocalAction::CatchForeign {
                    boundaries: boundaries.clone(),
                });
                self.append_continuation(next)
            }
            ...
            _ => {
                let frame_plan = EvidenceEvalDeltaFramePlan::from_continuation_frame(frame)
                    .ok_or(EvidenceScopeLocalPlanReject::RequestBoundary)?;
                self.actions
                    .push(EvidenceScopeLocalAction::Eval(frame_plan));
                if let Some(next) = frame.tail_ref() {
                    self.append_continuation(next)?;
                }
                Ok(())
...
impl EvidenceScopeSidecarPlanBuilder {
    fn finish(self) -> EvidenceScopeSidecarPlan {
        EvidenceScopeSidecarPlan {
            actions: Rc::from(self.actions.into_boxed_slice()),
        }
    }
    ...
    fn append_continuation(
        &mut self,
        continuation: &EvidenceContinuation,
    ) -> Result<(), EvidenceScopeSidecarPlanReject> {
        let Some(frame) = continuation.frame() else {
            return Ok(());
        };
        self.append_frame(frame)
    ...
    fn append_frame(
        ...
        match frame {
            ...
            EvidenceContinuationFrame::MarkerFrame {
                ...
                let child = EvidenceScopeSidecarPlan::from_continuation(next)?;
                self.actions.push(EvidenceScopeSidecarAction::Marker {
                    frame: EvidenceMarkerDeltaFramePlan {
                        markers: markers.clone(),
                        activate_add_ids: *activate_add_ids,
                        handler_path: handler_path
            ...
            EvidenceContinuationFrame::ProviderEnv { provider_env, next } => {
                let child = EvidenceScopeSidecarPlan::from_continuation(next)?;
                self.actions.push(EvidenceScopeSidecarAction::Provider {
                    frame: EvidenceProviderDeltaFramePlan {
                        provider_env: provider_env.clone(),
                    },
                    child,
            ...
            EvidenceContinuationFrame::CatchForeignBoundarySegment { boundaries, next } => {
                if !catch_boundaries_are_foreign_pass_through(boundaries.as_ref()) {
                    return Err(EvidenceScopeSidecarPlanReject::RequestBoundary);
                }
                self.actions.push(EvidenceScopeSidecarAction::CatchForeign {
                    boundaries: boundaries.clone(),
                });
                self.append_continuation(next)
            }
            ...
            EvidenceContinuationFrame::RefSetHandleResult { .. }
            | EvidenceContinuationFrame::RefSetHandleValueResult { .. } => {
                Err(EvidenceScopeSidecarPlanReject::RefSetBoundary)
            }
            _ => {
                let frame_plan = EvidenceEvalDeltaFramePlan::from_continuation_frame(frame)
                    .ok_or(EvidenceScopeSidecarPlanReject::RequestBoundary)?;
                self.actions
                    .push(EvidenceScopeSidecarAction::Eval(frame_plan));
                if let Some(next) = frame.tail_ref() {
                    self.append_continuation(next)?;
                }
                Ok(())
...
struct EvidenceSignalHygiene {
    ...
    #[cfg(debug_assertions)]
    provider_env_foreign_later_grant_shadow: Option<EvidenceProviderEnvLaterGrantShadow>,
}
...
impl EvidenceSignalHygiene {
    ...
    fn set_provider_env_foreign_boundary_shadow(
        &mut self,
        cert: EvidenceProviderEnvForeignBoundaryGrantCert,
    ) {
        self.provider_env_foreign_boundary_shadow = Some(cert);
    }
    ...
    #[cfg(debug_assertions)]
    fn set_provider_env_foreign_later_grant_shadow(
        &mut self,
        shadow: EvidenceProviderEnvLaterGrantShadow,
    ) {
        self.provider_env_foreign_later_grant_shadow = Some(shadow);
    }

    #[cfg(debug_assertions)]
    fn provider_env_foreign_later_grant_shadow(
        &self,
    ) -> Option<EvidenceProviderEnvLaterGrantShadow> {
        self.provider_env_foreign_later_grant_shadow
    }
...
enum EvidenceMarkerCloseDecision {
    ...
    NativeProviderEnvForeignBoundary(EvidenceProviderEnvForeignBoundaryGrantCert),
    Legacy,
}
...
impl EvidenceMarkerCloseDecision {
    fn is_native(self) -> bool {
        matches!(
            self,
            Self::NativeProviderPair(_)
                | Self::NativeProviderPrefixBoundary(_)
                | Self::NativeProviderEnvForeignBoundary(_)
        )
    }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum EvidenceProviderEnvForeignBoundaryCloseDecision {
    Native(EvidenceProviderEnvForeignBoundaryGrantCert),
    Legacy,
    NotForeign,
}
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum EvidenceProviderEnvForeignBoundaryCloseReject {
    NearestMisses,
    NoProviderEnv,
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
struct EvidenceProviderEnvForeignBoundaryGrantCert {
    permission: RuntimeEvidenceProviderGrantPermission,
...
#[cfg(debug_assertions)]
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
struct EvidenceProviderEnvLaterGrantShadow {
    permission: RuntimeEvidenceProviderGrantPermission,
    later_grant_depth: usize,
}

#[cfg(debug_assertions)]
impl EvidenceProviderEnvLaterGrantShadow {
    fn new(permission: RuntimeEvidenceProviderGrantPermission, later_grant_depth: usize) -> Self {
        Self {
            permission,
            later_grant_depth,
        }
    ...
    fn later_grant_depth(self) -> usize {
        self.later_grant_depth
    }
}
...
const SCOPE_PLAN_FOREIGN_CATCH_SIDECAR_VALUE_EXECUTOR_ENABLED: bool = false;
...
impl EvidenceContinuation {
    ...
    fn scope_plan_foreign_catch_boundary(
        scope: Self,
        boundaries: Rc<[EvidenceCatchBoundaryDeltaPlan]>,
        next: Self,
    ) -> Self {
        let scope_direct_tail_boundary_free =
            scope.direct_tail_segment_profile().request_boundaries == 0;
        Self::scope_plan_foreign_catch_boundary_with_scope_profile(
            scope,
    ...
    fn scope_plan_foreign_catch_boundary_with_scope_profile(
        scope: Self,
        scope_direct_tail_boundary_free: bool,
        boundaries: Rc<[EvidenceCatchBoundaryDeltaPlan]>,
        next: Self,
    ...
    fn provider_env_later_grant_placement_at(
        ...
        frame.provider_env_later_grant_placement_at(
            handler_id,
            depth,
            under_then_first,
            under_then_second,
    ...
    fn provider_env_grant_placement_at(
        ...
        frame.provider_env_grant_placement_at(
            handler_id,
            depth,
            under_then_first,
            under_then_second,
...
impl EvidenceContinuationFrame {
    fn starts_with_foreign_catch_boundary(&self, routed_yield_handler: Option<ExprId>) -> bool {
        match (self, routed_yield_handler) {
            ...
            (
                Self::ScopePlanForeignCatchBoundary {
                    boundaries, scope, ..
                },
                Some(handler),
            ) if scope.is_identity() => boundaries
                .first()
                .is_some_and(|boundary| boundary.catch_expr != handler),
            _ => false,
        }
    ...
    fn resume_plan_shadow_profile(
        ...
        match self {
            ...
            Self::ScopePlanForeignCatchBoundary {
                ...
                let mut profile = merge_resume_plan_shadow_profile(
                    scope.resume_plan_shadow_profile(routed_yield_handler),
                    EvidenceResumePlanShadowProfile {
                        catch_boundaries: boundaries.len(),
                        ..EvidenceResumePlanShadowProfile::default()
                    },
    ...
    fn append_resume_plan_trace(
        ...
        routed_yield_handler: Option<ExprId>,
        root_marker_scope: bool,
    ) {
        match self {
            Self::Then { first, second } => {
                if let Some(frame) = first.frame() {
                    frame.append_resume_plan_trace(
                        steps,
                        cursor,
            ...
            Self::ProviderEnv { next, .. } => {
                ...
                cursor.provider += 1;
                if let Some(frame) = next.frame() {
                    // ProviderEnv opens a child scope. Its inner markers stay in the scoped
                    // trace, while the flat marker lane tracks only the root marker delta.
                    frame.append_resume_plan_trace(steps, cursor, routed_yield_handler, false);
                }
            }
            Self::RefSetHandleResult { .. } | Self::RefSetHandleValueResult { .. } => {}
            _ => {
    ...
    fn direct_tail_segment_profile(&self) -> EvidenceDirectTailSegmentProfile {
        match self {
            ...
            Self::ScopePlanForeignCatchBoundary {
                ...
                let mut profile = scope.direct_tail_segment_profile();
                profile.request_boundaries += boundaries.len();
                merge_direct_tail_segment_profile(profile, next.direct_tail_segment_profile())
            }
            Self::RefSetHandleResult { .. } | Self::RefSetHandleValueResult { .. } => {
                EvidenceDirectTailSegmentProfile {
    ...
    fn request_boundary_profile(
        ...
        match self {
            ...
            EvidenceContinuationFrame::ScopePlanForeignCatchBoundary {
                ...
                let scope_profile = scope.request_boundary_profile(routed_yield_handler, stats);
                if scope_profile.has_boundary() {
    ...
    fn append_scope_blocker_info(&self) -> Option<EvidenceAppendScopeBlockerInfo> {
        match self.append_scope_blocker() {
            ...
            None => match self {
                Self::ScopePlanForeignCatchBoundary { next, .. } => {
                    next.append_scope_blocker_info()
                }
                _ => self
                    .tail_ref()
    ...
    fn provider_env_boundary_status(&self, handler_id: u32) -> EvidenceProviderEnvBoundaryStatus {
        match self {
            ...
            Self::ScopePlanForeignCatchBoundary { scope, next, .. } => merge_provider_env_presence(
                scope.provider_env_boundary_status(handler_id),
                next.provider_env_boundary_status(handler_id),
            ),
            _ => self
                .tail_ref()
    ...
    fn any_provider_env_status(&self, handler_id: u32) -> EvidenceProviderEnvBoundaryStatus {
        match self {
            ...
            Self::ScopePlanForeignCatchBoundary { scope, next, .. } => merge_provider_env_presence(
                scope.any_provider_env_status(handler_id),
                next.any_provider_env_status(handler_id),
            ),
            _ => self
                .tail_ref()
    ...
    fn provider_env_placement_at(
        ...
        match self {
            ...
            Self::ScopePlanForeignCatchBoundary { scope, next, .. } => {
                let mut scope_placement =
                    scope.provider_env_placement_at(handler_id, depth + 1, true, under_then_second);
                if scope_placement.has_nearest_boundary() {
                    scope_placement
                        .merge_any_after_nearest(next.any_provider_env_status(handler_id));
                    return scope_placement;
                }
                next.provider_env_placement_at(handler_id, depth + 1, under_then_first, true)
            }
            _ => self
                .tail_ref()
                .map(|tail| {
    ...
    fn provider_env_later_grant_placement_at(
        ...
        match self {
            Self::ProviderEnv {
                ...
                next.provider_env_grant_placement_at(
                    handler_id,
                    depth + 1,
                    under_then_first,
                    under_then_second,
            ...
            Self::MarkerFrame { .. } => EvidenceProviderEnvLaterGrantPlacement::none(),
            Self::Then { first, second } => {
                let first_placement =
                    first.provider_env_placement_at(handler_id, depth + 1, true, under_then_second);
                if first_placement.has_nearest_boundary() {
                    let first_later = first.provider_env_later_grant_placement_at(
                        handler_id,
                        depth + 1,
                        true,
                        under_then_second,
                    ...
                    return second.provider_env_grant_placement_at(
                        handler_id,
                        depth + 1,
                        under_then_first,
                        true,
                ...
                second.provider_env_later_grant_placement_at(
                    handler_id,
                    depth + 1,
                    under_then_first,
                    true,
            ...
            Self::ScopePlanForeignCatchBoundary { scope, next, .. } => {
                let scope_placement =
                    scope.provider_env_placement_at(handler_id, depth + 1, true, under_then_second);
                if scope_placement.has_nearest_boundary() {
                    let scope_later = scope.provider_env_later_grant_placement_at(
                        handler_id,
                        depth + 1,
                        true,
                        under_then_second,
                    );
                    if scope_later.is_found() {
                        return scope_later;
                    }
                    return next.provider_env_grant_placement_at(
                        handler_id,
                        depth + 1,
                        under_then_first,
                        true,
                ...
                next.provider_env_later_grant_placement_at(
                    handler_id,
                    depth + 1,
                    under_then_first,
                    true,
            ...
            _ => self
                .tail_ref()
                .map(|tail| {
                    tail.provider_env_later_grant_placement_at(
                        handler_id,
                        depth + 1,
                        under_then_first,
                        under_then_second,
    ...
    fn provider_env_later_grant_placement_at_profiled(
        ...
        match self {
            ...
            Self::ScopePlanForeignCatchBoundary { scope, next, .. } => {
                stats.provider_env_later_grant_placement_then_steps += 1;
                let scope_placement = scope.provider_env_placement_at_profiled(
                    handler_id,
                    depth + 1,
                    true,
                    under_then_second,
                ...
                if scope_placement.has_nearest_boundary() {
                    let scope_later = scope.provider_env_later_grant_placement_at_profiled(
                        handler_id,
                        depth + 1,
                        true,
                        under_then_second,
                    ...
                    return next.provider_env_grant_placement_at_profiled(
                        handler_id,
                        depth + 1,
                        under_then_first,
                        true,
                ...
                next.provider_env_later_grant_placement_at_profiled(
                    handler_id,
                    depth + 1,
                    under_then_first,
                    true,
    ...
    fn provider_env_grant_placement_at(
        ...
        match self {
            ...
            Self::MarkerFrame { .. } => EvidenceProviderEnvLaterGrantPlacement::none(),
            Self::Then { first, second } => {
                let first_grant = first.provider_env_grant_placement_at(
                    handler_id,
                    depth + 1,
                    true,
                    under_then_second,
                ...
                second.provider_env_grant_placement_at(
                    handler_id,
                    depth + 1,
                    under_then_first,
                    true,
            ...
            Self::ScopePlanForeignCatchBoundary { scope, next, .. } => {
                let scope_grant = scope.provider_env_grant_placement_at(
                    handler_id,
                    depth + 1,
                    true,
                    under_then_second,
            ...
            _ => self
                .tail_ref()
                .map(|tail| {
                    tail.provider_env_grant_placement_at(
                        handler_id,
                        depth + 1,
                        under_then_first,
                        under_then_second,
    ...
    fn provider_env_grant_placement_at_profiled(
        ...
        match self {
            ...
            Self::ScopePlanForeignCatchBoundary { scope, next, .. } => {
                stats.provider_env_later_grant_placement_then_steps += 1;
                let scope_grant = scope.provider_env_grant_placement_at_profiled(
                    handler_id,
                    depth + 1,
                    true,
                    under_then_second,
    ...
    fn provider_env_placement_at_profiled(
        ...
        match self {
            ...
            Self::ScopePlanForeignCatchBoundary { scope, next, .. } => {
                stats.provider_env_later_grant_placement_then_steps += 1;
                let mut scope_placement = scope.provider_env_placement_at_profiled(
                    handler_id,
                    depth + 1,
                    true,
                    under_then_second,
                ...
                if scope_placement.has_nearest_boundary() {
                    scope_placement.merge_any_after_nearest(
                        next.any_provider_env_status_profiled(handler_id, stats),
                    );
                    return scope_placement;
                }
                next.provider_env_placement_at_profiled(
                    handler_id,
                    depth + 1,
                    under_then_first,
                    true,
    ...
    fn any_provider_env_status_profiled(
        ...
        match self {
            ...
            Self::ScopePlanForeignCatchBoundary { scope, next, .. } => {
                stats.provider_env_later_grant_placement_then_steps += 1;
                merge_provider_env_presence(
                    scope.any_provider_env_status_profiled(handler_id, stats),
                    next.any_provider_env_status_profiled(handler_id, stats),
                )
            }
            Self::MarkerFrame { .. } => {
    ...
    fn shape_profile_at(&self, depth: usize) -> EvidenceContinuationShapeProfile {
        ...
        match self {
            ...
            Self::ScopePlanForeignCatchBoundary { scope, next, .. } => {
                shape.record_then();
                shape = merge_continuation_shape(shape, scope.shape_profile_at(depth + 1));
...
struct RuntimeSuspendedHostContinuation {
    ...
    kind: RuntimeSuspendedHostContinuationKind,
    operation: Vec<String>,
    continuation: EvidenceContinuation,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum RuntimeSuspendedHostContinuationKind {
    OneShot,
    MultiShot,
}
...
impl<'a> RuntimeEvidenceRunner<'a> {
    ...
    fn with_current_host_branch<T>(
        &mut self,
        branch_id: RuntimeHostBranchId,
        run: impl FnOnce(&mut Self) -> Result<T, RuntimeEvidenceRunError>,
    ) -> Result<T, RuntimeEvidenceRunError> {
        let previous = self.current_host_branch;
        self.current_host_branch = branch_id;
        let result = run(self);
        self.current_host_branch = previous;
        result
    }
    ...
    fn append_direct_tail_foreign_catch_boundary_delta(
        ...
        if let Some(blocker) = call.continuation.append_scope_blocker_info() {
            ...
            if SCOPE_PLAN_FOREIGN_CATCH_EXECUTOR_ENABLED
                && boundary.is_signal_passthrough()
                && boundary.has_value_resume_payload()
            {
                if self.should_collect_runtime_breakdown_stats() {
                    self.stats.resume_plan_exec_attempts += 1;
                    self.stats.resume_plan_exec_hits += 1;
                    self.stats.resume_plan_exec_eval_steps_saved_estimate += call
                        .continuation
                        .resume_plan_shadow_profile(Some(call.handler))
                        .estimated_resume_steps();
                }
                call.continuation = EvidenceContinuation::scope_plan_foreign_catch_boundary(
                    call.continuation,
                ...
                if SCOPE_PLAN_FOREIGN_CATCH_EXECUTOR_ENABLED
                    && self.should_collect_runtime_breakdown_stats()
                {
                    self.stats.resume_plan_exec_attempts += 1;
                    self.stats.resume_plan_exec_fallbacks += 1;
                    self.stats.resume_plan_exec_reject_boundary += 1;
                    self.stats.resume_plan_exec_materialized_legacy += 1;
                }
                self.stats.scope_exec_tree_fallbacks += 1;
                if SCOPE_PLAN_FOREIGN_CATCH_EXECUTOR_ENABLED {
                    self.stats.scope_exec_reject_non_foreign_catch += 1;
                }
                record_continuation_append_then(
                    &mut self.stats,
                    EvidenceContinuationAppendSource::DirectTail,
    ...
    fn provider_marker_close_decision(
        ...
        if let Some(handler_path) = resume_plan.and_then(|plan| plan.handler_path.as_ref()) {
            match self.provider_env_foreign_boundary_close_decision(
                call,
                handler_path,
                markers,
                permission,
                ...
                EvidenceProviderEnvForeignBoundaryCloseDecision::Native(cert) => {
                    #[cfg(debug_assertions)]
                    call.hygiene.set_provider_env_foreign_boundary_shadow(cert);
                ...
                EvidenceProviderEnvForeignBoundaryCloseDecision::NotForeign => {}
            }
            if let Some(cert) =
    ...
    fn record_provider_marker_close_handler_path_reject(
        ...
        if permission_family.as_slice() == handler_path {
            self.stats
    ...
    fn provider_env_foreign_boundary_close_decision(
        &mut self,
        call: &mut EvidenceDirectTailResumptive,
        handler_path: &[String],
        markers: &[EvidenceValueMarker],
        ...
        if boundary.blocked || boundary.path.as_slice() != handler_path {
            return EvidenceProviderEnvForeignBoundaryCloseDecision::NotForeign;
        }
        let Some(permission_family) = self.context.provider_grant_permission_family(permission)
        else {
        ...
        match self.provider_env_foreign_boundary_close_cert(
            call,
            handler_path,
            markers,
            permission,
    ...
    fn provider_env_foreign_boundary_close_cert(
        &mut self,
        call: &EvidenceDirectTailResumptive,
        handler_path: &[String],
        markers: &[EvidenceValueMarker],
        ...
        if permission_family != call.path.as_ref() {
            self.record_provider_env_foreign_boundary_close_reject(
    ...
    fn record_provider_env_foreign_boundary_close_reject(
        &mut self,
    ...
    fn record_provider_env_foreign_later_grant_profile_candidate(
        &mut self,
        call: &mut EvidenceDirectTailResumptive,
        handler_path: &[String],
        markers: &[EvidenceValueMarker],
        ...
        if placement.depth != Some(1) || !placement.under_then_first || placement.under_then_second
        {
            return;
        }
        if marker_slice_has_carry_after_frame_add_id(markers) {
        ...
        if later_placement.under_then_second {
            self.stats
                .provider_env_foreign_later_grant_under_then_second += 1;
        }
        if later_placement.in_provider_env_next {
            self.stats
                .provider_env_foreign_later_grant_in_provider_env_next += 1;
        }
        if let Some(depth) = later_placement.depth {
            self.stats.provider_env_foreign_later_grant_depth_sum += depth;
            self.stats.provider_env_foreign_later_grant_max_depth = self
                .stats
    ...
    fn record_resume_plan_candidate(
        ...
        match candidate {
            EvidenceResumePlanCandidate::Planned(candidate) => {
                ...
                match candidate.boundary_lane {
                    EvidenceScopePlanBoundaryLane::None => {
                        self.stats.resume_plan_candidate_request_delta_none += 1;
                    }
                    EvidenceScopePlanBoundaryLane::Catch {
                        same_handler,
                        no_routed_handler,
                        foreign_handler,
                    } => {
                    ...
                    EvidenceScopePlanBoundaryLane::RefSet { .. } => {}
                }
            }
        ...
        match cert {
            EvidenceResumePlanExecutionCert::ProviderDirtyForeignCatchScope(cert) => {
                ...
                self.stats
                    ...
                    cert.catch.foreign_handler_count;
                self.stats
                    .resume_plan_cert_provider_dirty_foreign_scope_estimated_resume_steps +=
                    cert.provider.estimated_resume_steps;
            }
            EvidenceResumePlanExecutionCert::Rejected(reject) => {
                ...
                match reject {
                    ...
                    EvidenceResumePlanExecutionCertReject::MultiShot => {
                        ...
                        self.stats.scope_exec_reject_not_one_shot += 1;
                    }
                    EvidenceResumePlanExecutionCertReject::RequestNone => {
    ...
    fn record_resume_plan_from_cert(
        ...
        self.record_resume_plan_exec_shadow(
            plan,
            direct_tail,
            cert.provider.estimated_resume_steps,
        );
    ...
    fn record_resume_plan_scoped_trace_plan(
        ...
        let sidecar_input = self.resume_plan_sidecar_shadow_input(marker_delta, provider_delta);
        let marker_delta_frames = sidecar_input.marker_frames;
        self.record_resume_plan_sidecar_shadow(&plan, sidecar_input);
        let scope_plan =
    ...
    fn resume_plan_sidecar_shadow_input(
        &self,
        marker_delta: Option<EvidenceMarkerDeltaId>,
        provider_delta: Option<EvidenceProviderDeltaId>,
    ) -> EvidenceSidecarPlanShadowInput {
        let marker_delta =
            marker_delta.map(|marker_delta| &self.marker_delta_plans[marker_delta.0]);
        let provider_delta =
            provider_delta.map(|provider_delta| &self.provider_delta_plans[provider_delta.0]);
        EvidenceSidecarPlanShadowInput {
            marker_frames: marker_delta
                .map(EvidenceMarkerDeltaPlan::frame_count)
                .unwrap_or(0),
            marker_active_add_id_ops: marker_delta
                .map(EvidenceMarkerDeltaPlan::active_add_id_ops)
                .unwrap_or(0),
            marker_handler_boundary_ops: marker_delta
                .map(EvidenceMarkerDeltaPlan::handler_boundary_ops)
                .unwrap_or(0),
            provider_frames: provider_delta
                .map(EvidenceProviderDeltaPlan::frame_count)
                .unwrap_or(0),
            provider_slots: provider_delta
                .map(EvidenceProviderDeltaPlan::slot_count)
                .unwrap_or(0),
            provider_handler_candidates: provider_delta
                .map(EvidenceProviderDeltaPlan::handler_candidate_count)
                .unwrap_or(0),
        }
    }

    fn record_resume_plan_sidecar_shadow(
        &mut self,
        plan: &EvidenceResumeScopedTracePlan,
        input: EvidenceSidecarPlanShadowInput,
    ) {
        self.stats.resume_plan_sidecar_shadow_candidates += 1;
        match plan.sidecar_plan_shadow(input) {
            EvidenceSidecarPlanShadow::Planned(profile) => {
                self.stats.resume_plan_sidecar_shadow_ready += 1;
                self.stats.resume_plan_sidecar_shadow_marker_frames += profile.marker_frames;
                self.stats
                    .resume_plan_sidecar_shadow_marker_active_add_id_ops +=
                    profile.marker_active_add_id_ops;
                self.stats
                    .resume_plan_sidecar_shadow_marker_handler_boundary_ops +=
                    profile.marker_handler_boundary_ops;
                self.stats.resume_plan_sidecar_shadow_provider_frames += profile.provider_frames;
                self.stats.resume_plan_sidecar_shadow_provider_slots += profile.provider_slots;
                self.stats
                    .resume_plan_sidecar_shadow_provider_handler_candidates +=
                    profile.provider_handler_candidates;
                self.stats.resume_plan_sidecar_shadow_child_marker_frames +=
                    profile.child_marker_frames;
                self.stats
                    .resume_plan_sidecar_shadow_legacy_frame_avoid_estimate +=
                    profile.legacy_frame_avoid_estimate;
            }
            EvidenceSidecarPlanShadow::Rejected(
                EvidenceSidecarPlanShadowReject::RootMarkerMismatch,
            ) => {
                self.stats
                    .resume_plan_sidecar_shadow_reject_root_marker_mismatch += 1;
            }
            EvidenceSidecarPlanShadow::Rejected(
                EvidenceSidecarPlanShadowReject::ProviderScopeMismatch,
            ) => {
                self.stats
                    .resume_plan_sidecar_shadow_reject_provider_scope_mismatch += 1;
            }
        }
    }
    ...
    fn record_scope_plan_foreign_catch_shadow(
        ...
        let has_non_foreign_catch = request_delta.same_handler_count() > 0
            || request_delta.no_routed_handler_count() > 0
            || !request_delta.is_foreign_pass_through_only();
        ...
        if delta_shadow.signal_mismatch > 0
            || delta_shadow.generic_fallback_mismatch > 0
            || delta_shadow.materialized_shape_mismatch > 0
            || delta_shadow.value_resume_mismatch > 0
        {
            self.stats.scope_exec_reject_shape_mismatch += 1;
            return;
        }
        if has_non_foreign_catch {
    ...
    fn record_provider_env_foreign_boundary_shadow_candidate(
        ...
        call.hygiene.set_provider_env_foreign_boundary_shadow(
            EvidenceProviderEnvForeignBoundaryGrantCert::new(permission, depth),
        );
        #[cfg(not(debug_assertions))]
        {
            let _ = permission;
    ...
    fn handle_runtime_host_one_shot_operation(
        ...
        match outcome {
            HostOutcome::Suspended => {
                ...
                let parent = self.current_host_branch_id();
                let Some(spawn) = self
                    .host_scheduler
                ...
                self.suspended_host_continuations.insert(
                    ...
                    RuntimeSuspendedHostContinuation {
                        ...
                        kind: RuntimeSuspendedHostContinuationKind::OneShot,
                        operation: operation_path,
    ...
    fn handle_runtime_host_multi_shot_operation(
        &mut self,
        operation: host::RuntimeHostResolvedOperation,
        request: EvidenceRequest,
    ) -> Result<RuntimeHostRequestHandling, RuntimeEvidenceRunError> {
        ...
        let suspend_issuer = self.host_scheduler.multi_shot_suspend_issuer();
        let mut ctx = HostCtx::new_with_suspend_issuer(&mut self.host_state, suspend_issuer);
        let outcome = self
            .host_registry
            .call(operation, &mut ctx, &boundary_payload);
        ...
        match outcome {
            HostOutcome::Suspended => {
                let Some(token) = issued_token else {
                    return Err(RuntimeEvidenceRunError::HostAbiError {
                        operation: operation_path,
                        message: "suspend multi-shot host operation did not request a resume token"
                            .to_string(),
                    });
                };
                let parent = self.current_host_branch_id();
                if !self
                    .host_scheduler
                    .commit_multi_shot_suspend(parent, token.clone())
                {
                    return Err(RuntimeEvidenceRunError::HostAbiError {
                        operation: operation_path,
                        message:
                            "suspend multi-shot host operation could not park its parent branch"
                                .to_string(),
                    });
                }
                self.suspended_host_continuations.insert(
                    ...
                    RuntimeSuspendedHostContinuation {
                        branch_id: parent,
                        kind: RuntimeSuspendedHostContinuationKind::MultiShot,
                        operation: operation_path,
                        continuation: request.continuation,
                    },
                );
            ...
            HostOutcome::Return(_) => {
                if let Some(token) = issued_token {
                    self.host_scheduler.discard_multi_shot_suspend(token);
                }
                Err(RuntimeEvidenceRunError::HostAbiError {
                    operation: operation_path,
                    message: "suspend multi-shot host operation returned without suspending"
                        .to_string(),
                })
            }
            HostOutcome::HostError(message) => {
    ...
    fn process_next_host_scheduler_event(
        ...
        match event {
            ...
            scheduler::RuntimeHostSchedulerEvent::Close {
                branch_id,
                token_id,
            } => {
                if self
                    .suspended_host_continuations
                    .get(&token_id)
                    .is_some_and(|suspended| suspended.branch_id == branch_id)
                {
                    self.suspended_host_continuations.remove(&token_id);
                }
                Ok(None)
            }
    ...
    fn host_resume_continuation(
        &self,
        token_id: HostResumeTokenId,
        branch_id: RuntimeHostBranchId,
    ) -> Result<Option<(Vec<String>, EvidenceContinuation, bool)>, RuntimeEvidenceRunError> {
        let Some(suspended) = self.suspended_host_continuations.get(&token_id) else {
            return Ok(None);
        };
        match suspended.kind {
            RuntimeSuspendedHostContinuationKind::OneShot => {
                ...
                Ok(Some((
                    ...
                    suspended.continuation.clone(),
                    true,
                )))
            }
            RuntimeSuspendedHostContinuationKind::MultiShot => {
                if self.host_scheduler.branch_parent(branch_id) != Some(suspended.branch_id) {
                    return Err(RuntimeEvidenceRunError::HostAbiError {
                        operation: suspended.operation.clone(),
                        message: "multi-shot host resume child did not belong to suspended parent"
                            .to_string(),
                    });
                }
                Ok(Some((
    ...
    fn resume_scope_plan_foreign_catch_boundary(
        &mut self,
        scope: EvidenceContinuation,
        ...
        {
            if catch_boundaries_are_foreign_pass_through(boundaries.as_ref()) {
                self.stats.scope_sidecar_value_resume_build_attempts += 1;
                match EvidenceScopeSidecarPlan::from_continuation(&scope) {
                    Ok(plan) => {
                        self.stats.scope_exec_hits += 1;
                        self.stats.scope_exec_foreign_catch_hits += 1;
                        self.stats.scope_sidecar_value_resume_hits += 1;
                        let result = self.resume_scope_sidecar_plan(&plan, value)?;
                        return self
                            .continue_scope_plan_foreign_catch_result(result, boundaries, next);
                    }
                    Err(EvidenceScopeSidecarPlanReject::RequestBoundary) => {
                        self.stats
                            .scope_sidecar_value_resume_reject_request_boundary += 1;
                        self.stats.scope_exec_reject_shape_mismatch += 1;
                    }
                    Err(EvidenceScopeSidecarPlanReject::RefSetBoundary) => {
                        self.stats.scope_sidecar_value_resume_reject_ref_set += 1;
                        self.stats.scope_exec_reject_shape_mismatch += 1;
                    }
                }
            } else {
                self.stats
                    .scope_sidecar_value_resume_reject_non_foreign_boundary += 1;
                self.stats.scope_exec_reject_shape_mismatch += 1;
            }
        }
        if SCOPE_PLAN_FOREIGN_CATCH_EXECUTOR_ENABLED
        ...
        {
            match scope_local_plan {
                Some(plan) => {
                    self.stats.scope_exec_hits += 1;
                    self.stats.scope_exec_foreign_catch_hits += 1;
                    let result = self.resume_scope_local_plan(plan.as_ref(), value)?;
                    return self.continue_scope_plan_foreign_catch_result(result, boundaries, next);
                }
                None => {
                    self.stats.scope_exec_reject_shape_mismatch += 1;
                }
    ...
    fn resume_scope_sidecar_plan(
        &mut self,
        plan: &EvidenceScopeSidecarPlan,
        value: SharedValue,
    ) -> Result<EvidenceEvalResult, RuntimeEvidenceRunError> {
        self.resume_scope_sidecar_actions(plan.actions.as_ref(), value)
    }

    fn resume_scope_sidecar_actions(
        &mut self,
        actions: &[EvidenceScopeSidecarAction],
        mut value: SharedValue,
    ) -> Result<EvidenceEvalResult, RuntimeEvidenceRunError> {
        for (index, action) in actions.iter().enumerate() {
            let result = self.resume_scope_sidecar_action(action, value)?;
            match result {
                EvidenceEvalResult::Value(next_value) => {
                    value = next_value;
                }
                EvidenceEvalResult::Effect(signal) => {
                    let rest = EvidenceScopeSidecarPlan::materialize_actions(
                        &actions[index + 1..],
                        EvidenceContinuation::identity(),
                    );
                    return self.continue_signal(signal, rest);
                }
            }
        }
        Ok(EvidenceEvalResult::Value(value))
    ...
    fn resume_scope_sidecar_action(
        &mut self,
        action: &EvidenceScopeSidecarAction,
        value: SharedValue,
    ) -> Result<EvidenceEvalResult, RuntimeEvidenceRunError> {
        match action {
            EvidenceScopeSidecarAction::Eval(frame) => self.resume_eval_delta_frame(frame, value),
            EvidenceScopeSidecarAction::CatchForeign { boundaries } => self
                .continue_scope_plan_foreign_catch_result(
                    EvidenceEvalResult::Value(value),
                    boundaries.clone(),
                    EvidenceContinuation::identity(),
                ),
            EvidenceScopeSidecarAction::Marker { frame, child } => {
                self.resume_scope_sidecar_marker_frame(frame, child, value)
            }
            EvidenceScopeSidecarAction::Provider { frame, child } => {
                self.resume_scope_sidecar_provider_frame(frame, child, value)
            }
        }
    }

    fn resume_scope_sidecar_marker_frame(
        &mut self,
        frame: &EvidenceMarkerDeltaFramePlan,
        child: &EvidenceScopeSidecarPlan,
        value: SharedValue,
        ...
        self.with_marker_frame(
            ...
            frame.activate_add_ids,
            handler_path,
            |runner| runner.resume_scope_sidecar_plan(child, value),
        )
    }

    fn resume_scope_sidecar_provider_frame(
        &mut self,
        frame: &EvidenceProviderDeltaFramePlan,
        child: &EvidenceScopeSidecarPlan,
        value: SharedValue,
        ...
        let provider_env = frame.provider_env.clone();
        self.with_provider_env(provider_env, |runner| {
            runner.resume_scope_sidecar_plan(child, value)
        })
    }

    fn resume_scope_local_plan(
        &mut self,
        plan: &EvidenceScopeLocalPlan,
        value: SharedValue,
    ) -> Result<EvidenceEvalResult, RuntimeEvidenceRunError> {
        self.resume_scope_local_actions(plan.actions.as_ref(), value)
    }

    fn resume_scope_local_actions(
        &mut self,
        actions: &[EvidenceScopeLocalAction],
        mut value: SharedValue,
    ) -> Result<EvidenceEvalResult, RuntimeEvidenceRunError> {
        for (index, action) in actions.iter().enumerate() {
            let result = self.resume_scope_local_action(action, value)?;
            match result {
                EvidenceEvalResult::Value(next_value) => {
                    value = next_value;
                }
                EvidenceEvalResult::Effect(signal) => {
                    let rest = EvidenceScopeLocalPlan::materialize_actions(
                        &actions[index + 1..],
                        EvidenceContinuation::identity(),
                    );
                    return self.continue_signal(signal, rest);
    ...
    fn resume_scope_local_action(
        &mut self,
        action: &EvidenceScopeLocalAction,
        value: SharedValue,
    ) -> Result<EvidenceEvalResult, RuntimeEvidenceRunError> {
        match action {
            EvidenceScopeLocalAction::Eval(frame) => self.resume_eval_delta_frame(frame, value),
            EvidenceScopeLocalAction::CatchForeign { boundaries } => self
                .continue_scope_plan_foreign_catch_result(
                    EvidenceEvalResult::Value(value),
                    boundaries.clone(),
                    EvidenceContinuation::identity(),
                ),
            EvidenceScopeLocalAction::Marker { frame, child } => {
                self.resume_scope_local_marker_frame(frame, child, value)
            }
            EvidenceScopeLocalAction::Provider { frame, child } => {
                self.resume_scope_local_provider_frame(frame, child, value)
            }
        }
    }

    fn resume_scope_local_marker_frame(
        &mut self,
        frame: &EvidenceMarkerDeltaFramePlan,
        child: &EvidenceScopeLocalPlan,
        value: SharedValue,
        ...
        if frame.markers.is_empty() {
            return self.resume_scope_local_plan(child, value);
        }
    ...
    fn resume_scope_local_provider_frame(
        &mut self,
        frame: &EvidenceProviderDeltaFramePlan,
        child: &EvidenceScopeLocalPlan,
        value: SharedValue,
    ) -> Result<EvidenceEvalResult, RuntimeEvidenceRunError> {
        if frame.provider_env.is_empty() {
            return self.resume_scope_local_plan(child, value);
        }
        ...
        let active_frame = self
            .active_provider_envs
            .pop()
            .expect("provider frame must be active after provider-scoped scope plan run");
        debug_assert_eq!(self.active_provider_envs.len(), len);
        self.active_provider_envs.truncate(len);
        self.close_provider_env_result(result?, active_frame.provider_env)
    }

    fn resume_eval_delta_frame(
        &mut self,
        frame: &EvidenceEvalDeltaFramePlan,
        value: SharedValue,
    ) -> Result<EvidenceEvalResult, RuntimeEvidenceRunError> {
        match frame {
            EvidenceEvalDeltaFramePlan::ForceThunk {
                target_value_is_thunk,
            } => {
                let result = self.force_thunk_result(value)?;
                if *target_value_is_thunk {
            ...
            EvidenceEvalDeltaFramePlan::ForceValueIfThunk => {
                self.force_value_if_thunk_result(value)
            }
            EvidenceEvalDeltaFramePlan::ApplyCallee { site, arg, env } => {
                let mut env = self.clone_env(env);
                self.eval_apply_arg_result(*site, value, *arg, &mut env)
            }
            EvidenceEvalDeltaFramePlan::ApplyArg { site, callee } => {
                self.apply_scoped_value_result(*site, callee.clone(), value)
            }
            EvidenceEvalDeltaFramePlan::ApplyForcedCallee { site, arg } => {
                self.apply_scoped_value_result(*site, value, arg.clone())
            }
            ...
            EvidenceEvalDeltaFramePlan::ApplyAdapterArg {
                function,
                ret_markers,
                source_ret,
                target_ret,
            ...
            EvidenceEvalDeltaFramePlan::ApplyAdapterResult {
                ret_markers,
                source_ret,
                target_ret,
            } => {
            ...
            EvidenceEvalDeltaFramePlan::CaseScrutinee { arms, env } => {
                self.eval_case_scrutinee_result(value, arms.clone(), env)
            }
            EvidenceEvalDeltaFramePlan::TupleItems { values, rest, env } => {
                let mut values = values.clone();
                values.push(value);
                let mut env = self.clone_env(env);
                self.eval_tuple_items_result(values, rest, &mut env)
            }
            EvidenceEvalDeltaFramePlan::RecordSpread { fields, env } => {
                let values = record_fields(value.as_ref())?;
                let mut env = self.clone_env(env);
                self.eval_record_fields_result(values, fields, &mut env)
            }
            EvidenceEvalDeltaFramePlan::RecordFields { values, rest, env } => {
                let mut values = values.clone();
                let Some((field, remaining)) = rest.split_first() else {
                    return Err(RuntimeEvidenceRunError::UnsupportedExpr(
                        "empty record field continuation",
                ...
                let mut env = self.clone_env(env);
                self.eval_record_fields_result(values, remaining, &mut env)
            }
            EvidenceEvalDeltaFramePlan::PolyVariantPayloads {
                tag,
                values,
                rest,
                env,
            ...
            EvidenceEvalDeltaFramePlan::BlockStmt {
                ...
                self.eval_block_parts_result(rest, *tail, &mut env, last)
            }
            EvidenceEvalDeltaFramePlan::RefSetReference { value: expr, env } => {
                let result = self.force_value_if_thunk_result(value)?;
                let env = self.clone_env(env);
                self.continue_result(
                    result,
            ...
            EvidenceEvalDeltaFramePlan::RefSetForcedReference { value: expr, env } => {
                let mut env = self.clone_env(env);
                let result = self.eval_expr_result(*expr, &mut env)?;
                self.continue_result(
                    result,
                    EvidenceContinuation::ref_set_value(value, EvidenceContinuation::identity()),
                )
            }
            EvidenceEvalDeltaFramePlan::RefSetValue { reference } => {
                let result = self.force_value_if_thunk_result(value)?;
                self.continue_result(
                    result,
            ...
            EvidenceEvalDeltaFramePlan::RefSetForcedValue { reference } => {
                let update_effect = record_field(reference.as_ref(), "update_effect")?;
                let result = self.apply_value_result(
                    None,
                    update_effect,
            ...
            EvidenceEvalDeltaFramePlan::RefSetResolvedUnit => Ok(EvidenceEvalResult::Value(
                shared(RuntimeEvidenceValue::Unit),
            )),
            ...
            EvidenceEvalDeltaFramePlan::ResolveRefSetValues {
                values,
                assigned,
                out,
                index,
            ...
            EvidenceEvalDeltaFramePlan::ResolveRefSetFields {
                fields,
                assigned,
                out,
                index,
    ...
    fn continue_scope_plan_foreign_catch_result(
        &mut self,
        result: EvidenceEvalResult,
        boundaries: Rc<[EvidenceCatchBoundaryDeltaPlan]>,
        next: EvidenceContinuation,
        ...
        match result {
            EvidenceEvalResult::Value(value) => {
                self.stats.scope_exec_value_resume_hits += 1;
                self.resume_catch_foreign_boundary_segment(boundaries, next, value)
            }
            EvidenceEvalResult::Effect(signal) => {
                self.stats.scope_exec_signal_passthrough_hits += 1;
                self.continue_signal_through_catch_foreign_boundaries(signal, boundaries, next)
            }
        }
    }

    fn continue_signal_through_catch_foreign_boundaries(
        &mut self,
        signal: EvidenceEffectSignal,
        boundaries: Rc<[EvidenceCatchBoundaryDeltaPlan]>,
        next: EvidenceContinuation,
        ...
        match signal {
            EvidenceEffectSignal::GenericRequest(request) => self
                .continue_generic_request_through_catch_foreign_boundary_segment(
                    request, boundaries, next,
                ),
            EvidenceEffectSignal::DirectAbortive(call) => self
                .continue_direct_abortive_through_catch_foreign_boundary_segment(
                    call, boundaries, next,
                ),
            EvidenceEffectSignal::DirectTailResumptive(call) => self
                .continue_direct_tail_resumptive_through_catch_foreign_boundary_segment(
                    call, boundaries, next,
                ),
            EvidenceEffectSignal::RoutedYield(call) => self
                .continue_routed_yield_through_catch_foreign_boundary_segment(
                    call, boundaries, next,
                ),
        }
    }
    ...
    fn continue_direct_abortive_with_continuation(
        ...
        match frame {
            ...
            EvidenceContinuationFrame::ScopePlanForeignCatchBoundary {
                ...
                let result = self.continue_direct_abortive_with_continuation(call, scope)?;
                self.continue_scope_plan_foreign_catch_result(result, boundaries, next)
            }
            EvidenceContinuationFrame::ForceThunk { next, .. }
            | EvidenceContinuationFrame::ForceValueIfThunk { next }
    ...
    fn continue_direct_tail_resumptive_through_scope_plan_foreign_catch_boundary(
        &mut self,
        call: EvidenceDirectTailResumptive,
        scope: EvidenceContinuation,
        scope_direct_tail_boundary_free: bool,
        ...
        let record_resume_plan_exec = self.should_collect_runtime_breakdown_stats();
        if SCOPE_PLAN_FOREIGN_CATCH_EXECUTOR_ENABLED && record_resume_plan_exec {
            self.stats.resume_plan_exec_attempts += 1;
        }
        let compact_reject = if SCOPE_PLAN_FOREIGN_CATCH_EXECUTOR_ENABLED {
            scope_plan_direct_tail_signal_compact_reject(
                scope_direct_tail_boundary_free,
                boundaries.as_ref(),
                call.handler,
            )
        ...
        if let Some(reject) = compact_reject {
            if record_resume_plan_exec {
                self.stats.resume_plan_exec_fallbacks += 1;
                self.stats.resume_plan_exec_materialized_legacy += 1;
                match reject {
                    EvidenceScopePlanDirectTailSignalReject::ScopeHasRequestBoundary => {
                        self.stats.resume_plan_exec_reject_shape += 1;
                    }
    ...
    fn continue_direct_tail_resumptive_with_continuation(
            ...
            .is_none()
        {
            let Some(EvidenceContinuationFrame::ScopePlanForeignCatchBoundary {
                ...
                unreachable!("prechecked scope plan continuation frame");
            };
            return self.continue_direct_tail_resumptive_through_scope_plan_foreign_catch_boundary(
                call,
                scope,
                scope_direct_tail_boundary_free,
                boundaries,
        ...
        let (call, next) = match frame {
            ...
            EvidenceContinuationFrame::ScopePlanForeignCatchBoundary {
                ...
                return self
                    .continue_direct_tail_resumptive_through_scope_plan_foreign_catch_boundary(
                        call,
                        scope,
                        scope_direct_tail_boundary_free,
                        boundaries,
        ...
        if let Some(permission) = self.provider_guard_boundary_permission(&call.hygiene) {
            ...
            if self.should_verify_permission_visibility() {
                ...
                #[cfg(debug_assertions)]
                {
                    ...
                    self.record_resume_marker_provider_prefix_boundary_close_visible_shadow(
                        &call.hygiene,
                        legacy_visible,
                        native_result,
                    );
                    ...
                    self.record_provider_env_foreign_later_grant_visible_shadow(
                        &call.hygiene,
                        legacy_visible,
                        native_result,
                    );
                ...
                let provider_env_foreign_shadow = call
                    ...
                    || call
                        .hygiene
                        .provider_env_foreign_later_grant_shadow()
                        .is_some();
                #[cfg(not(debug_assertions))]
    ...
    #[cfg(debug_assertions)]
    fn record_resume_marker_provider_prefix_boundary_close_visible_shadow(
        &mut self,
        hygiene: &EvidenceSignalHygiene,
        legacy_visible: Option<bool>,
        permission_result: EvidencePermissionVisibleResult,
        ...
        self.stats.resume_marker_provider_prefix_boundary_shadow += 1;
        if legacy_visible.is_some() {
            self.stats
                .resume_marker_provider_prefix_boundary_shadow_legacy_visible += 1;
        }
        let permission_visible = permission_result.visible();
        if permission_visible.is_some() {
            self.stats
                .resume_marker_provider_prefix_boundary_shadow_permission_visible += 1;
        }
        if legacy_visible == permission_visible {
            self.stats
                .resume_marker_provider_prefix_boundary_shadow_match += 1;
        } else {
            self.stats
    ...
    #[cfg(debug_assertions)]
    fn record_provider_env_foreign_later_grant_visible_shadow(
        &mut self,
        hygiene: &EvidenceSignalHygiene,
        legacy_visible: Option<bool>,
        naive_result: EvidencePermissionVisibleResult,
    ) {
        let Some(shadow) = hygiene.provider_env_foreign_later_grant_shadow() else {
            return;
        };
        debug_assert_eq!(
            Some(shadow.permission()),
            self.provider_guard_boundary_permission(hygiene),
            "ProviderEnv later grant shadow lost provider permission"
        );
        debug_assert_ne!(
            shadow.later_grant_depth(),
            0,
            "ProviderEnv later grant shadow must keep the grant depth"
        );
        if legacy_visible.is_some() {
            self.stats.provider_env_foreign_later_grant_legacy_visible += 1;
        }
        let naive_visible = naive_result.visible();
        if naive_visible.is_some() {
            self.stats.provider_env_foreign_later_grant_naive_visible += 1;
        }
        if legacy_visible == naive_visible {
            self.stats.provider_env_foreign_later_grant_naive_match += 1;
        } else {
            self.stats.provider_env_foreign_later_grant_naive_mismatch += 1;
        }
    }
...
mod tests {
    ...
    #[test]
    fn host_abi_suspend_multi_shot_resumes_three_child_branches() {
        let program = Program::default();
        let mut runner = RuntimeEvidenceRunner::new(&program, RuntimeEvidenceRunContext::default());
        install_multi_shot_host(&mut runner);

        let token = start_multi_shot_suspend(&mut runner);

        for value in [1, 2, 3] {
            assert_eq!(token.resume(BoundaryValue::Int(value)), Ok(()));
            let result = runner
                .process_next_host_scheduler_event()
                .expect("queued multi-shot resume should be processed")
                .expect("queued multi-shot resume should produce a resumed result");
            assert_eq!(
                result,
                EvidenceEvalResult::Value(shared(RuntimeEvidenceValue::Int(value)))
            );
        }

        assert_eq!(runner.suspended_host_continuations.len(), 1);
        assert!(runner.host_scheduler.has_only_root_branch());
        ...
        assert!(
            runner
                .process_next_host_scheduler_event()
                .expect("queued close should be processed")
                .is_none()
        );
        assert!(runner.suspended_host_continuations.is_empty());
    }
    ...
    #[test]
    fn host_abi_suspend_multi_shot_parent_cancel_rejects_resume_and_cascades_children() {
        let program = Program::default();
        let mut runner = RuntimeEvidenceRunner::new(&program, RuntimeEvidenceRunContext::default());
        install_multi_shot_host(&mut runner);
        ...
        let first = spawn_multi_shot_child_without_running(&mut runner, &token, 1);
        let second = spawn_multi_shot_child_without_running(&mut runner, &token, 2);

        assert_eq!(runner.host_scheduler.branch_parent(first), Some(parent));
        assert_eq!(runner.host_scheduler.branch_parent(second), Some(parent));
        assert_eq!(runner.host_scheduler.branch_resume_ordinal(first), Some(0));
        assert_eq!(runner.host_scheduler.branch_resume_ordinal(second), Some(1));
        assert!(runner.host_scheduler.enqueue_cancel(
            parent,
            scheduler::RuntimeHostCancelReason::ParentExtentClosed,
        ));
        assert!(
            runner
                .process_next_host_scheduler_event()
                .expect("parent cancel should be processed")
                .is_none()
        );

        assert_eq!(
            token.resume(BoundaryValue::Int(3)),
            Err(HostResumeError::Dropped)
        );
        assert!(runner.suspended_host_continuations.is_empty());
        ...
        assert!(
            runner
                .process_next_host_scheduler_event()
                .expect("first child cancel should be processed")
                .is_none()
        );
        assert!(
            runner
                .process_next_host_scheduler_event()
                .expect("second child cancel should be processed")
                .is_none()
        );
        assert_eq!(
            runner.host_scheduler.branch_status(first),
            Some(scheduler::RuntimeHostBranchStatus::CancelPending)
        );
        assert_eq!(
            runner.host_scheduler.branch_status(second),
            Some(scheduler::RuntimeHostBranchStatus::CancelPending)
        );
        assert_eq!(runner.host_scheduler.next_operation_instance(first), None);
        assert_eq!(runner.host_scheduler.next_operation_instance(second), None);
        assert_eq!(
            runner.host_scheduler.branch_status(first),
            Some(scheduler::RuntimeHostBranchStatus::Dropped)
        );
        assert_eq!(
            runner.host_scheduler.branch_status(second),
            Some(scheduler::RuntimeHostBranchStatus::Dropped)
    ...
    #[test]
    fn host_abi_suspend_multi_shot_close_rejects_later_resume() {
        let program = Program::default();
        let mut runner = RuntimeEvidenceRunner::new(&program, RuntimeEvidenceRunContext::default());
        install_multi_shot_host(&mut runner);
        ...
        assert_eq!(token.close(), Ok(()));
        assert_eq!(
            token.resume(BoundaryValue::Int(1)),
            Err(HostResumeError::Closed)
        );
        assert!(
            runner
                .process_next_host_scheduler_event()
    ...
    #[test]
    fn host_abi_suspend_multi_shot_child_cancel_does_not_affect_sibling_or_parent() {
        let program = Program::default();
        let mut runner = RuntimeEvidenceRunner::new(&program, RuntimeEvidenceRunContext::default());
        install_multi_shot_host(&mut runner);
        ...
        assert!(
            runner
                .host_scheduler
                .enqueue_cancel(first, scheduler::RuntimeHostCancelReason::HostDisconnected,)
        );
        assert!(
            runner
                .process_next_host_scheduler_event()
                .expect("child cancel should be processed")
                .is_none()
        );
        assert_eq!(
            runner.host_scheduler.branch_status(first),
        ...
        assert_eq!(
            runner.host_scheduler.branch_status(parent),
            Some(scheduler::RuntimeHostBranchStatus::Running)
        );

        assert_eq!(token.resume(BoundaryValue::Int(3)), Ok(()));
        let result = runner
            .process_next_host_scheduler_event()
            .expect("sibling-independent resume should be processed")
            .expect("sibling-independent resume should produce a result");
        assert_eq!(
            result,
            EvidenceEvalResult::Value(shared(RuntimeEvidenceValue::Int(3)))
        );
    }
    ...
    fn multi_shot_suspending_host_call(
        ctx: &mut HostCtx<'_>,
        payload: &BoundaryValue,
    ) -> HostOutcome {
        if !matches!(payload, BoundaryValue::Unit) {
            return HostOutcome::HostError(format!(
                "multi-shot host expected unit, got {payload:?}"
            ));
        }
        let token = match ctx.suspend_multi_shot() {
            Ok(token) => token,
            Err(error) => {
                return HostOutcome::HostError(format!(
                    "multi-shot host failed to issue token: {error:?}"
                ));
            }
        };
        let Some(state) = ctx.state_mut::<RuntimeBuiltinHostState>() else {
            return HostOutcome::HostError("multi-shot host expected builtin state".to_string());
        };
        state.test_resume_tokens.push(token);
        HostOutcome::Suspended
    }
    ...
    fn install_multi_shot_host(runner: &mut RuntimeEvidenceRunner<'_>) {
        runner.host_registry = RuntimeHostRegistry::with_manifest_and_registrations(
            ...
            vec![HostOpRegistration {
                act_id: "test.host.bridge",
                operation_id: "call",
                f: multi_shot_suspending_host_call,
            }],
        );
    }

    fn start_multi_shot_suspend(runner: &mut RuntimeEvidenceRunner<'_>) -> HostResumeToken {
        let handling = runner
            .try_handle_runtime_host_request(custom_host_request())
            .expect("multi-shot host op should suspend without runtime error");
        assert!(matches!(handling, RuntimeHostRequestHandling::Suspended));
        assert_eq!(runner.host_state.test_resume_tokens.len(), 1);
        runner.host_state.test_resume_tokens[0].clone()
    }

    fn spawn_multi_shot_child_without_running(
        runner: &mut RuntimeEvidenceRunner<'_>,
        token: &HostResumeToken,
        value: i64,
    ) -> RuntimeHostBranchId {
        assert_eq!(token.resume(BoundaryValue::Int(value)), Ok(()));
        match runner
            .host_scheduler
            .process_next_event()
            .expect("multi-shot resume should produce a scheduler event")
        {
            scheduler::RuntimeHostSchedulerEvent::Resume { branch_id, .. } => branch_id,
            other => panic!("expected multi-shot resume event, got {other:?}"),
        }
    }
    ...
    fn resume_plan_candidate_keeps_request_route_and_multiplicity() {
        ...
        assert_eq!(
            ...
            EvidenceResumePlanCandidate::Planned(EvidenceResumePlanPlannedCandidate {
                ...
                trace: None,
                eval_delta: None,
                boundary_lane: EvidenceScopePlanBoundaryLane::Catch {
                    same_handler: 1,
                    no_routed_handler: 0,
                    foreign_handler: 1
                },
    ...
    #[test]
    fn provider_env_foreign_boundary_close_accepts_nearest_grant() {
        let program = Program::default();
        let mut runner = provider_prefix_boundary_runner(&program, &permission_test_path());
        let permission = provider_prefix_boundary_permission();
        let call = provider_env_foreign_boundary_call(permission_test_path());
        let markers = Vec::new();
        let handler_path = provider_env_foreign_handler_path();

        let cert = runner.provider_env_foreign_boundary_close_cert(
            &call,
            &handler_path,
            &markers,
            permission,
            &permission_test_path(),
            provider_env_foreign_grant_placement(),
        );
        ...
        assert_eq!(runner.stats.provider_env_foreign_boundary_native_hits, 1);
        assert_eq!(
            runner.stats.provider_env_foreign_boundary_shadow_candidates,
            1
        );
    }

    #[test]
    fn provider_env_foreign_boundary_close_rejects_nearest_miss() {
        let program = Program::default();
        let mut runner = provider_prefix_boundary_runner(&program, &permission_test_path());
        let permission = provider_prefix_boundary_permission();
        let call = provider_env_foreign_boundary_call(permission_test_path());
    ...
    #[test]
    fn provider_env_foreign_boundary_close_rejects_later_grant() {
        let program = Program::default();
        let mut runner = provider_prefix_boundary_runner(&program, &permission_test_path());
        let permission = provider_prefix_boundary_permission();
        let call = provider_env_foreign_boundary_call(permission_test_path());
    ...
    #[test]
    fn provider_env_foreign_boundary_close_rejects_depth_not_one() {
        let program = Program::default();
        let mut runner = provider_prefix_boundary_runner(&program, &permission_test_path());
        let permission = provider_prefix_boundary_permission();
        let call = provider_env_foreign_boundary_call(permission_test_path());
        ...
        assert_eq!(cert, None);
        assert_eq!(runner.stats.provider_env_foreign_boundary_reject_depth, 1);
    }

    #[test]
    fn provider_env_foreign_boundary_close_rejects_then_second() {
        let program = Program::default();
        let mut runner = provider_prefix_boundary_runner(&program, &permission_test_path());
        let permission = provider_prefix_boundary_permission();
        let call = provider_env_foreign_boundary_call(permission_test_path());
    ...
    #[test]
    fn provider_env_foreign_boundary_close_rejects_carry_after_frame() {
        let program = Program::default();
        let mut runner = provider_prefix_boundary_runner(&program, &permission_test_path());
        let permission = provider_prefix_boundary_permission();
        let call = provider_env_foreign_boundary_call(permission_test_path());
    ...
    #[test]
    fn provider_env_foreign_boundary_close_rejects_permission_family_request_mismatch() {
        let program = Program::default();
        let mut runner = provider_prefix_boundary_runner(&program, &permission_test_path());
        let permission = provider_prefix_boundary_permission();
        let call =
    ...
    #[test]
    fn provider_env_later_grant_placement_finds_provider_env_next() {
        let continuation = provider_env_later_grant_continuation();

        let placement = continuation.provider_env_later_grant_placement(7);

        assert_eq!(
            placement,
            EvidenceProviderEnvLaterGrantPlacement::found(2, true, false, true)
        );
    }

    #[test]
    fn provider_env_foreign_later_grant_profile_records_position() {
        let program = Program::default();
        let mut runner = provider_prefix_boundary_runner(&program, &permission_test_path());
        let permission = provider_prefix_boundary_permission();
        let mut call = provider_env_foreign_boundary_call(permission_test_path());
        call.continuation = provider_env_later_grant_continuation();
        let markers = Vec::new();
        let handler_path = provider_env_foreign_handler_path();

        runner.record_provider_env_foreign_later_grant_profile_candidate(
            &mut call,
            &handler_path,
            &markers,
            permission,
        ...
        assert_eq!(runner.stats.provider_env_foreign_later_grant_depth_sum, 2);
        assert_eq!(runner.stats.provider_env_foreign_later_grant_max_depth, 2);
        assert_eq!(
    ...
    #[test]
    fn scope_plan_foreign_catch_boundary_merges_boundary_lane_outside_scope() {
        ...
        let first = EvidenceContinuation::scope_plan_foreign_catch_boundary(
            scope,
            Rc::from(vec![catch_foreign_boundary_fixture(ExprId(1))].into_boxed_slice()),
            EvidenceContinuation::identity(),
        );
        ...
        assert!(matches!(
            scope.into_frame(),
            Some(EvidenceContinuationFrame::MarkerFrame { .. })
        ));
        assert_eq!(boundaries.len(), 2);
        assert_eq!(boundaries[0].catch_expr, ExprId(1));
        assert_eq!(boundaries[1].catch_expr, ExprId(2));
    ...
    #[test]
    fn scope_plan_foreign_catch_boundary_matches_legacy_tree_shadow_shape() {
        let scope = EvidenceContinuation::marker_frame(
            Rc::from([EvidenceValueMarker::Frame {
                id: EvidenceGuardId(1),
            }]),
        ...
        let boundaries: Rc<[EvidenceCatchBoundaryDeltaPlan]> = Rc::from(
            vec![
                catch_foreign_boundary_fixture(ExprId(1)),
                catch_foreign_boundary_fixture(ExprId(2)),
            ]
        ...
        let routed_handler = Some(ExprId(99));

        let planned_profile = planned.resume_plan_shadow_profile(routed_handler);
        let legacy_profile = legacy.resume_plan_shadow_profile(routed_handler);
        assert_eq!(planned_profile.then_frames, 0);
        assert_eq!(legacy_profile.then_frames, 1);
        assert_eq!(
            EvidenceResumePlanShadowProfile {
        ...
        assert_eq!(
            EvidenceResumePlanTracePlan::from_continuation(&planned, routed_handler),
            EvidenceResumePlanTracePlan::from_continuation(&legacy, routed_handler)
        );
        assert_eq!(
            EvidenceResumeScopedTracePlan::from_continuation(&planned, routed_handler),
            EvidenceResumeScopedTracePlan::from_continuation(&legacy, routed_handler)
        );
        assert_eq!(
            EvidenceEvalDeltaPlan::from_continuation(&planned),
            EvidenceEvalDeltaPlan::from_continuation(&legacy)
        );
        assert_eq!(
            EvidenceRequestDeltaPlan::from_continuation(&planned, routed_handler),
            EvidenceRequestDeltaPlan::from_continuation(&legacy, routed_handler)
        );
        assert_eq!(
            planned.marker_delta_shadow_key(),
            legacy.marker_delta_shadow_key()
        );
        assert_eq!(
            planned.provider_delta_shadow_key(),
            legacy.provider_delta_shadow_key()
        );

        assert_eq!(planned_profile.catch_foreign_handler, 2);
        assert_eq!(planned_profile.marker_frames, 1);
        assert_eq!(planned_profile.provider_env_deltas, 1);
        assert_eq!(planned_profile.eval_frames, 1);
    }

    #[test]
    fn scope_plan_foreign_catch_boundary_tracks_direct_tail_boundary_free_scope() {
        let boundary: Rc<[EvidenceCatchBoundaryDeltaPlan]> =
            Rc::from(vec![catch_foreign_boundary_fixture(ExprId(3))].into_boxed_slice());
        let boundary_free_scope = EvidenceContinuation::marker_frame(
            Rc::from([EvidenceValueMarker::Frame {
                id: EvidenceGuardId(1),
            }]),
            true,
        ...
        let scoped_request_boundary = EvidenceContinuation::catch_body(
            ExprId(4),
            empty_catch_arms(),
            Env::new(),
            EvidenceContinuation::identity(),
    ...
    #[test]
    fn scope_plan_direct_tail_signal_compact_rejects_same_handler_boundary() {
        let boundaries: Rc<[EvidenceCatchBoundaryDeltaPlan]> = Rc::from(
            vec![
                catch_foreign_boundary_fixture(ExprId(3)),
                catch_foreign_boundary_fixture(ExprId(4)),
            ]
            .into_boxed_slice(),
        );

        assert_eq!(
            scope_plan_direct_tail_signal_compact_reject(true, boundaries.as_ref(), ExprId(99)),
            None
        );
        assert_eq!(
            scope_plan_direct_tail_signal_compact_reject(true, boundaries.as_ref(), ExprId(3)),
            Some(EvidenceScopePlanDirectTailSignalReject::SameHandlerBoundary)
        );
        assert_eq!(
    ...
    #[test]
    fn scope_local_plan_materializes_marker_provider_siblings_without_flattening_scope() {
        let continuation = EvidenceContinuation::Frame(Rc::new(EvidenceContinuationFrame::Then {
            first: EvidenceContinuation::marker_frame(
                Rc::from([EvidenceValueMarker::Frame {
                    id: EvidenceGuardId(1),
        ...
        let plan = EvidenceScopeLocalPlan::from_continuation(&continuation)
            .expect("scope should be representable as a local plan");
        let materialized = plan.materialize(EvidenceContinuation::identity());
    ...
    #[test]
    fn scope_local_plan_rejects_request_and_ref_set_boundaries() {
        let request_boundary = EvidenceContinuation::catch_body(
            ExprId(1),
            empty_catch_arms(),
            Env::new(),
        ...
        assert_eq!(
            EvidenceScopeLocalPlan::from_continuation(&request_boundary),
            Err(EvidenceScopeLocalPlanReject::RequestBoundary)
        );
        ...
        assert_eq!(
            EvidenceScopeLocalPlan::from_continuation(&ref_set_boundary),
            Err(EvidenceScopeLocalPlanReject::RefSetBoundary)
        );
    }

    #[test]
    fn scope_sidecar_plan_materializes_marker_provider_siblings_without_flattening_scope() {
        let continuation = EvidenceContinuation::Frame(Rc::new(EvidenceContinuationFrame::Then {
            first: EvidenceContinuation::marker_frame(
                Rc::from([EvidenceValueMarker::Frame {
                    id: EvidenceGuardId(1),
        ...
        let plan = EvidenceScopeSidecarPlan::from_continuation(&continuation)
            .expect("scope should be representable as a sidecar plan");
        let materialized = plan.materialize(EvidenceContinuation::identity());
    ...
    #[test]
    fn scope_sidecar_plan_rejects_request_and_ref_set_boundaries() {
        let request_boundary = EvidenceContinuation::catch_body(
            ExprId(1),
            empty_catch_arms(),
            Env::new(),
        ...
        assert_eq!(
            EvidenceScopeSidecarPlan::from_continuation(&request_boundary),
            Err(EvidenceScopeSidecarPlanReject::RequestBoundary)
        );
        ...
        assert_eq!(
            EvidenceScopeSidecarPlan::from_continuation(&ref_set_boundary),
            Err(EvidenceScopeSidecarPlanReject::RefSetBoundary)
        );
    }

    #[test]
    fn scope_local_plan_value_resume_preserves_marker_provider_scope() {
        let program = Program::default();
        let mut runner = RuntimeEvidenceRunner::new(&program, RuntimeEvidenceRunContext::default());
        let scope = EvidenceContinuation::marker_frame(
            Rc::from([EvidenceValueMarker::Frame {
        ...
        let scope_local_plan = EvidenceScopeLocalPlan::from_continuation(&scope)
            .expect("scope should be representable as a local plan");

        let result = runner
            .resume_scope_local_plan(&scope_local_plan, shared(RuntimeEvidenceValue::Unit))
            .expect("scope local value resume should succeed");
    ...
    #[test]
    fn scope_sidecar_plan_value_resume_preserves_marker_provider_scope() {
        let program = Program::default();
        let mut runner = RuntimeEvidenceRunner::new(&program, RuntimeEvidenceRunContext::default());
        let scope = EvidenceContinuation::marker_frame(
            Rc::from([EvidenceValueMarker::Frame {
        ...
        let scope_sidecar_plan = EvidenceScopeSidecarPlan::from_continuation(&scope)
            .expect("scope should be representable as a sidecar plan");

        let result = runner
            .resume_scope_sidecar_plan(&scope_sidecar_plan, shared(RuntimeEvidenceValue::Unit))
            .expect("scope sidecar value resume should succeed");
    ...
    fn resume_plan_scoped_trace_places_provider_inner_marker_in_child_scope() {
        ...
        assert_eq!(
            scoped.sidecar_plan_shadow(sidecar_input),
            EvidenceSidecarPlanShadow::Planned(EvidenceSidecarPlanShadowProfile {
                marker_frames: 1,
                marker_active_add_id_ops: 0,
                marker_handler_boundary_ops: 0,
                provider_frames: 1,
                provider_slots: provider_delta.slot_count(),
                provider_handler_candidates: provider_delta.handler_candidate_count(),
                child_marker_frames: 1,
                legacy_frame_avoid_estimate: 2,
            })
        );
        assert_eq!(
            scoped.sidecar_plan_shadow(EvidenceSidecarPlanShadowInput {
                provider_frames: provider_delta.frame_count() + 1,
                ..sidecar_input
            }),
            EvidenceSidecarPlanShadow::Rejected(
                EvidenceSidecarPlanShadowReject::ProviderScopeMismatch
        ...
        assert_eq!(
            scoped.sidecar_plan_shadow(EvidenceSidecarPlanShadowInput {
                marker_frames: marker_delta.frame_count() + 1,
                ..sidecar_input
            }),
            EvidenceSidecarPlanShadow::Rejected(
                EvidenceSidecarPlanShadowReject::RootMarkerMismatch
    ...
    #[test]
    fn resume_plan_trace_keeps_provider_child_marker_out_of_root_marker_lane() {
        let continuation = EvidenceContinuation::Frame(Rc::new(EvidenceContinuationFrame::Then {
            ...
            second: EvidenceContinuation::marker_frame(
                Vec::new().into(),
                false,
                None,
                EvidenceContinuation::identity(),
        ...
        let trace = EvidenceResumePlanTracePlan::from_continuation(&continuation, None);
        let scoped = EvidenceResumeScopedTracePlan::from_continuation(&continuation, None);
        let eval_delta = EvidenceEvalDeltaPlan::from_continuation(&continuation);
        let request_delta = EvidenceRequestDeltaPlan::from_continuation(&continuation, None);
        let marker_delta =
            EvidenceMarkerDeltaPlan::from_shadow_key(&continuation.marker_delta_shadow_key());
        let provider_delta =
            EvidenceProviderDeltaPlan::from_shadow_key(&continuation.provider_delta_shadow_key());
        ...
        let profile = trace
            .exec_shadow_profile(&eval_delta, &request_delta, &marker_delta, &provider_delta)
            .expect("root marker lane should match the marker delta sidecar");
    ...
    fn provider_env_foreign_boundary_call(
        request_path: Vec<String>,
    ) -> EvidenceDirectTailResumptive {
        provider_prefix_boundary_call_with_boundary(
            false,
            request_path,
            provider_env_foreign_handler_path(),
        )
    }

    fn provider_env_foreign_handler_path() -> Vec<String> {
        vec!["amount".to_string(), "coin".to_string()]
    }

    fn provider_env_foreign_grant_placement() -> EvidenceProviderEnvPlacement {
        EvidenceProviderEnvPlacement::nearest(
            EvidenceProviderEnvBoundaryStatus::GrantsPermission,
            EvidenceProviderEnvBoundaryStatus::None,
            1,
    ...
    fn provider_env_foreign_miss_placement() -> EvidenceProviderEnvPlacement {
        EvidenceProviderEnvPlacement::nearest(
            EvidenceProviderEnvBoundaryStatus::MissesPermission,
            EvidenceProviderEnvBoundaryStatus::None,
            1,
    ...
    fn provider_env_later_grant_continuation() -> EvidenceContinuation {
        EvidenceContinuation::Frame(Rc::new(EvidenceContinuationFrame::Then {
            first: EvidenceContinuation::provider_env(
            ...
            second: EvidenceContinuation::identity(),
        }))
    }
    ...
    fn provider_env_fixture_for_handler(handler_id: u32) -> RuntimeEvidenceProviderEnv {
        let apply = ExprId(10);
