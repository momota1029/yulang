...
- `YULANG_SUBST_SPECIALIZE=1` でも apply substitution evidence を export するようにした。`YULANG_COALESCE_APPLY_EVIDENCE=1` の早期 return でも principal evidence を落とさない。ただし `examples/07_junction.yu` の `std::range::fold_ints` はなお `apply-evidence-missing-principal-callee` なので、次は callee scheme を引けない apply 形状を追う。
