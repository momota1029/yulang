...
empty-boundaryの意味とrouteを変えない。対称 / mutually-recursive graphの完全なcanonical SCC方式は2c / 2dより
前に設計承認が必要であり、それまでは`None`によるcache missを維持する。
...
曖昧なrecursive rootは`None`で拒否した。次は2bのcache format bump / old-format missであり、typed/runtime exact
agreementとproduction接続は2c / 2dへ残す。Oracle A2 / Bはその後であり、Markdownのwarm成功は既存の
