...
- `yulang-runtime-finalize` の新 type graph path は、`Apply(Var(poly), arg)` の root use-site を
  ...
  `twice id 1` / `flip pair 1 2` では、apply spine の引数に現れる多相 `Var` も
  apply evidence / expression local type から得た expected type で別 graph として解き、
  `id.mono0` / `pair.mono0` のような値引数 alias へ差し替える。
  evidence より閉じた expression/local type を優先し、未閉じの場合だけ evidence の bounds を使う。
  `Never` lower は実際の制約として来た場合は保持するため、`pair <: f` から effect variable も
  `Never` へ解ける。
  既存の compiled-unit 永続 cache へ載せる前段として、runtime-finalize 内に
  `FinalizeInstanceCache` / `FinalizeInstanceCacheSurface` を追加した。
  key は `binding + type substitutions`、value は monomorphic scheme/body と runtime result/callee type。
  cache は advisory で、miss 時は従来どおり graph solver が動く。
  肥大化対策として max entry / max body node policy と skip profile を持つ。
  `FinalizeInstanceArtifactCache` も追加し、既存 compiled-unit manifest set を key にして
  `compiled-units/v*/parser-v*/runtime-finalize-v*/instances-*.bin` の sidecar を読める。
  `read_cache_for_manifests` / `write_cache_for_manifests` で `FinalizeInstanceCache` を直接
  load/store できる。
  infer artifact 本体へ runtime-ir body を混ぜず、同じ std/dependency manifest identity で
  finalize instance surface を永続化する前提にした。
  実際に finalize して作った cache を sidecar へ書き、manifest key から読み戻した cache で
  solver が同じ instance を hit する regression も追加した。
  一方で `"hello".println` を std ありで通常スタックの `runtime-finalize` test にするには、
  finalize instance cache だけでは足りない。compiled std artifact から user core へ merge
  するところまでは通常スタックで進むが、`yulang_runtime::lower_core_program` が stack overflow
  するため、core -> runtime IR lowering まで含む cache が別途必要。
