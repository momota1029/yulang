...
- `skip-no-complete-substitution` の原因を `no_complete_causes` として分けた。`examples/07_junction.yu` では `std::list::fold_impl` / `std::range::fold_from` が `instantiation-missing-vars`、`std::range::fold_ints` が `no-apply-evidence`。したがって fold 系でも、instantiation の欠落補完と apply evidence 自体の欠落は別問題として扱う。
