...
- Diagnostics:
  ...
  - 現在の `check` が lowering error を末尾に出すだけの状態を、source range と
    related information 付きで「直せる」表示へ移す。
- API fixed:
  - filesystem は `spec/2026-07-01-file-resource-api.md` を現行 anchor とする。
  - server API は filesystem と同じく resource lifetime / scope exit / host capability
    の言葉で固定する。先に薄い関数群を増やさない。
