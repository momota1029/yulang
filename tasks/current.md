...
initial/retry, indented strict-dedent initial/recovery-continuation, and
nested-If inner initial/recovery-continuation; query-only indented/dedent tests
are not a substitute.  Its immediate next action is to establish one concrete
accepted-own-Else direct source/result and receive a pre-write specification
audit before adding that distinct slice; its status does not authorize
