...
- parser/lowering の特例削減を続ける。
  ...
  - 末尾名 `add` / `fold` / `fold_impl` / `find` だけで associated demand signature closure を入れる特例は撤去した。`fold` は role impl graph 由来の `Fold` member として判定する。
  - 残っている `std` path / `ends_with` / 末尾名判定は `notes/design/std-special-case-audit.md` に洗い出した。sugar 入口以外の挙動特例は `DemandSemantics` / primitive family table / canonical id 比較へ寄せて消す。
  - associated demand signature closure の target 判定は `DemandSemantics::associated_signature_kind` に集約した。bootstrap classifier は撤去し、binding body/type、primitive op、role impl graph、local synthetic ref metadata から tag を作る。
  - monomorphize の effect path matching から任意 suffix match を外した。effect parent / child operation の prefix 関係だけ残す。
  - `Fold::item` は `std::list::list` / `std::range::range` の path 判定ではなく、role impl graph の associated type projection から閉じる。
  - `Fold::item` projection は `std::fold::Fold` path 固定ではなく、`item` associated type が一意に決まる role impl graph から閉じる。
  ...
  - `for` / `sub` / labelled loop sugar 入口の `std::flow` path 構築は `flow_capability` へ集約した。
  ...
  - infer primitive bootstrap の型/value path は `builtin_types` に集約した。`LowerState` は `PrimitivePathTable` を持ち、list literal / list pattern / string concat / export-time list index raw は state table 経由で path を読む。
  - `$x` fallback と var synthetic act source は `ref_capability` 経由にした。
  - 残る本丸は primitive family table を compiled-unit artifact metadata から渡すこと、sugar 入口の `for` / `sub` / labelled loop path 構築の semantic table 化。
