...
- Diagnostics:
  ...
  - `check` の lowering / syntax diagnostic は compact summary、source frame、
    ...
    - 2026-07-02 時点では role/method の unresolved / ambiguous failure は
      `run` の user-facing compile error として manifest に載っているが、
      `check` / LSP / playground の `SourceDiagnostic` にはまだ載っていない。
      `compile-error` manifest tag はこの移行対象を見つけるための印として使う。
- API fixed:
