...
canonical empty成功と構造failure fallbackのどちらかはまだ判別できない。次はvalidator cost / handoff outcomeの
shadow telemetryでこの理由を特定する。program-sensitive fallbackの退役判断はStage 7まで行わない。
