...
`std::core::fmt::Debug`の2-tuple impl candidate normalizationが生成する未適用merge constraint一件である。これは
別根なので今回修正しない。次はこのcandidate merge failureを独立に診断し、その後にcold/warm role-resolution counterへ
戻る。program-sensitive fallbackの退役判断はStage 7まで行わない。
