...
native/EOF behavior.  Direct PV ambient evidence presently covers only root
visible/no-stop initial/retry; query-only indented/dedent tests are not a
substitute.  Its immediate next action is to establish one concrete indented
strict-dedent direct PV observation, then pre-write specification audit that
remaining slice; its status does not authorize rewrite-source changes or alter
