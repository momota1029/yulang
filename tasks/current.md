...
nested-If inner identity; query-only indented/dedent tests are not a
substitute.  Its immediate next action is to establish one concrete nested-If
recovery-continuation direct source/result and receive a pre-write
specification audit before adding that distinct slice; its status does not
