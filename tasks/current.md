...
Direct PV ambient evidence pins root visible/no-stop initial/retry, indented
strict-dedent initial/recovery-continuation, nested-If inner initial/recovery-
continuation, accepted-own-Else initial/recovery-continuation, and
post-completed-If outer-tail initial/recovery-continuation; query-only
indented/dedent tests are not a substitute.  The ordinary ambient-provenance
row's named cells are complete.  Its immediate next action is to map one
concrete remaining fence-qualified scalar source/result before selecting
another direct-evidence slice, then receive a pre-write specification audit
before adding it; its status does not authorize rewrite-source changes or alter
the two-file delimiter integration block.
...
contiguous malformed ArrowRhs Error run.  The latter uses the fifth sealed
Error-run terminal operation only for its permitted same-line retry leading.
The literal `A ->@ B` now has successor Error CST `@` at `4..5`, retry-owned
space `5..6`, RHS `B` at `6..7`, and the unchanged Error record/unexpected
range `4..6`; the shifted local run maps that record to `19..21`.  The actual
legacy Yumark baseline remains intentionally distinct with Error CST `@ ` at
`19..21`.

M3 compiler/recovery, specification, and regression implementation-delta
review are clean.  The focused TypeExpression/output/legacy-baseline checks,
package check, formatting, and diff check passed; timing usage is zero.  O4
may certify only this local subproof.  The aggregate T2 matrix row remains
...
- 2026-09-05: fenced current-Itemの旧N0候補は、physical prefix/carrierを保持した
  ...
  L5aはcomplete-Item/`Deferred` ownerへ分岐する。N0c full candidateで露見した独立旧`grammar/`
  failure 3件も修復済み: named-fieldの空slotはcomma消費前に共有driverでemitし、`if`の初期condition
  不全はframeを正しい寿命で閉じた後にopaque idだけを戻す。Call argumentは失敗probeを常にsink rollbackし、
  accepted retryのlegacy expectationはcanonical Statement内だけに再publish、active Yumarkではfactだけへ
  流す。M2 compiler/recovery・specification delta reviewを通過し、指定final
  `cargo test -p yu-syntax`は906 passed, 0 failed, 2 ignored。N0c full certificationは完了。
...
2026-09-06: selected Struct ownerのvisibility/header、bodyless・named brace・tuple・strict-deeper indent body、
field/close/retry、final successorを一つのordinary/fenced normalized bodyへ移行し、Struct frontier一行を削除した。
raw Struct / field nameはdynamic operator tableより先にidentifierとして読み、`:{`はStruct colonへsplitせずType
polymorphic-variant entryへ渡す。TypeApply直前のStruct next-field probeはimmediate item/origin/line/fenceだけを受ける
fence-aware observerへし、close/transition外を観測しない。independent reviewでordinary malformed EOF recoveryが
owner-local whitespaceをError外へ落とす四経路を発見し、ordinary EOFだけをError内でemitする修正とCST topology controlsを
