...
現在の Yulang は alpha 段階の研究言語です。現行実装は `yulang2` pipeline にあり、
...
手元で CLI を使う場合は、`yulang2` を build します。ユーザー cache に標準ライブラリを
...
`yulang2 run`、`yulang2 check`、`yulang2 server` は、`YULANG_STD` や近くの
...
language server は `yulang2 server` で起動します。
...
directory を指定します。extension 側にはまだ旧 `yulang server` command を参照する
箇所が残っている可能性があり、`yulang2 server` へ更新する必要があります。

`yulang-ls` binary は現在、`yulang2` language server に委譲します。
...
- `crates/yulang2`: 現行 CLI と language server の入口。
...
- `crates/yulang-lsp`: `yulang-ls` binary。`yulang2` server に委譲する。
- `crates/yulang-parser`: parser と syntax tree。
- `crates/yulang-list-tree`: 共有 persistent list 実装。
