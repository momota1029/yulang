...
The following `grammar/**` location is historical legacy-parser evidence, not a current implementation path. The matching syntax owner is `crates/yu-syntax/src/pattern/delimited.rs`; public parsing enters through `crates/yu-syntax/src/lib.rs::{scan_header, parse_file}`.
