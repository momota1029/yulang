...
Yulang は compiler の artifact をキャッシュします。
...
local ファイルでは、`realm.toml` が explicit editable realm を示します。
`realm.toml`が見つからない場合、entry ファイルの親 directory が implicit editable realm になります。
entry ファイルは root module のままですが、`main.yu` なら `main` のように、source path由来の band path も持ちます。

## artifact の種類

| Artifact | 保存するもの | 効く場面 |
| --- | --- | --- |
| `.yucu` | 構文、名前空間、型、runtime surface | 標準ライブラリや変更していない依存 module の再利用 |
| `.yuir` | 推論済み principal poly IR | 完全に同じ source set を、推論なしで再実行する |
| `.yumo` | specialize 済み mono IR | mono command の再実行や、同じ source set から control IR を作り直す |
| `.yuvm` | lower 済み control-IR プログラム | 完全に同じ source set を、specialize / VM lowering なしで再実行する |
...
`.yures` は解決結果の記録で、compiled コードではありません。
...
| Route | 意味 |
| --- | --- |
| `disabled` | この run ではキャッシュの read と write を無効化 |
...
古いキャッシュや壊れたキャッシュは、言語エラーではありません。
Yulang はその artifact を飛ばし、source から compile し直します。

`--no-cache` は source collection 中の checked realm-resolution キャッシュ lookup も無効にします。

release installer は embedded std source を install したあと、最終 install path を使って標準ライブラリの `.yucu` prefix を一度 seed します。
これにより、その後の CLI run とキャッシュ key が一致します。
install 時のキャッシュ warmup を省く場合は`YULANG_NO_SEED_CACHE=1` を設定します。
...
remote provider、version-family solving、`yulang.lock`、registry/git source policy は stable なユーザー workflow ではありません。
