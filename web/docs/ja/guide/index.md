...
Yulang は、ライブラリで制御フローを定義しながら、短く直線的なプログラムを書ける言語です。
このページでは、その設計から集合的な比較と非決定的探索がどのように生まれるかを示し、続きのガイドへ案内します。
...
3 つの `each` が、無限に広がる 3 次元の格子を探索します。
`guard` で枝刈り、`.once` で「最初に見つかった 1 件」を取り出す。
