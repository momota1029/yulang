...
- `fs_err::wrap` — companion module 内のヘルパー。error effect を `result`
  値に閉じる。
- `fs_err::up` — companion module 内の handler。他の error 型が
  `from fs_err` を宣言している場合、narrower error を `fs_err` の effect に
  持ち上げる。
...
- variant `io_err::fs fs_err` と `io_err::parse parse_err`
