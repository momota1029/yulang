...
`std::io::file` は、テキストファイル操作、メタデータ、buffer を使ったテキスト編集を提供する。
...
`read_text` は UTF-8 のテキストファイル全体を読む。
`write_text` はテキストファイル全体を作成または置換する。
...
handler の終了処理で起きた書き戻し失敗は、catch 可能な `io_err` ではなく `yulang.host-io-error` runtime error になる。
...
`io_err::wrap` は error effect を `result` 値へ閉じる。
...
host capability がない場合は、`io_err` の操作失敗ではなく runtime の capability error になる。
...
| 操作 | シグネチャ | 役割 |
|---|---|---|
| `file::load path` | `path -> [file] result str io_err` | テキストファイル全体を読む。 |
| `file::store (path, text)` | `(path, str) -> [file] result unit io_err` | テキストファイル全体を作成または置換する。 |
...
この場合に native host で起きた失敗は、`yulang.host-io-error` runtime error になる。
