...
`open path` は text file を `ref '[fs] str` として開く。この参照は `.get()` で
host-backed file buffer を読む。参照経由で書くと buffer が更新され dirty に
なり、現在の host は underlying handle state が drop されたときに dirty な
file handle を flush する。

`open_in path f` は local wrapper 形である。

```yulang
open_in "data.txt": \file -> file.get()
```

handle を 1 つの callback-shaped operation に閉じ込めたいときに使う。
