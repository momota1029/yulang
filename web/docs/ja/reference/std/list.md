...
`list 'a` は `Index` を `int` と `range` の両方に実装する。
範囲外の `int` index と、末尾を超える `range` スライスは実行時に失敗する。
...
`map` / `filter` / `rev` / `sort` / `append` は新しい list を返す。
元の list は変わらない。
...
`first` と `last` は空の list に対して `nil` を返す。
`uncons` も先頭と残りの組がなければ `nil` を返す。
...
`xs` が参照に入っているとき（`my $xs = ...`）、`ref _ (list _)` 用の `Index` impl で `&xs[i] = value` が書ける。
`&xs.push v` も対応する ref method を通る。
...
`each` は要素を 1 つ非決定的に選ぶ。
`.list` はすべての分岐を新しい list に集める、`.once` は最初の 1 つを `opt` で返す。
