...
これを実装に写すため、ambient act を touch / get / set の三操作に割る:
...
pub text(path: path): ref '[file_buffer] str =
    unwrap_io (file_buffer::ambient_touch path)
    ref {
...
1. `lib/std/io/file.yu`: `file_buffer` act を D1 の三操作形に変更し、
   `text` を D1 の本体に書き換える。
2. `crates/evidence-vm/src/runtime/host.rs`:
...
- `text_with` の 4 行 protocol と `file` act（spec §1）の形を触らない。
...
新規決定である。2026-07-03 にユーザが D1〜D3 を承認した。
