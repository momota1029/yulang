...
6. `check` では compact な binding type を表示する。
7. `dump --core-ir` では core program を export して表示する。
8. `dump --runtime-ir` / `run` では runtime lowering を行う。
9. `run` では runtime IR を monomorphize し、VM で評価する。

ユーザーから見ると、`yulang check file.yu`、`yulang dump --core-ir file.yu`、`yulang run file.yu` が主な確認手段になる。

現在の workspace で `examples/showcase.yu` に対して確認した限り、`check` は成功する。
...
source loader は `crates/yulang-sources/src/lib.rs` にある。`SourceOptions` は次の情報を持つ。
...
runtime IR は literal pattern として string を扱える。`crates/yulang-runtime/src/lower/patterns.rs` は `typed_ir::Pattern::Lit(lit)` を受け取り、`lit_type` による型照合を行う。
