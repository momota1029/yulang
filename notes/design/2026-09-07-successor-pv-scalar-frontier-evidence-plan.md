...
post-completed-If outer-tail initial and recovery-continuation cells, below are
now direct evidence.
...
| ordinary ambient provenance | root visible/no-stop initial and retry, indented strict-dedent initial/recovery-continuation, nested-If initial/recovery-continuation, accepted-own-Else initial/recovery-continuation, and outer-tail initial/recovery-continuation are already pinned; retain the no-stop `else` distinction rather than encoding it in stops |
...
That initial row alone did not establish own-Else recovery continuation or an
outer tail.

The tenth bounded slice pins accepted-own-Else recovery continuation.  Under
the same root, preexisting outer companion, no-stop, and actual If entry,
`if condition:\n  type T = :{A::\nelse: value` consumes through byte 42.  The
PV `25..30` has complete `A`, then an incomplete second tag slot whose `::`
is the sole tag Error at `28..30`, followed by the Missing brace at `30..30`.
...
The twelfth bounded slice pins post-completed-If outer-tail recovery
continuation.  Under a root scope only,
`if outer:\n  if inner:\n    value\n  type T = :{A::\nelse: value` consumes
through byte 60.  Its completed inner `If` remains `12..31` without an Else;
the sibling Type RHS PV `43..48` has complete `A` `45..46`, an incomplete
second tag slot whose `::` is the Error at `46..48`, then the Missing Brace at
`48..48`.  The actual outer `ElseArm` `49..60` is accepted.  Exact AST/direct
...
The fourteenth bounded slice pins that distinct wrong-head CRLF outer-EOF
path.  `:{123::\r\n:{B}` has three outer tag-loop Errors at `2..5`, `5..7`,
and `9..13`; the final `}` is inside the final Error, while the only close
recovery is the enclosing PV Brace Missing at `13..13`.  Exact AST/direct full
topology, four ordered records, source reconstruction, and episode/frame
balance distinguish this from an admitted payload retry.

The fifteenth bounded slice pins the unspaced wrong-head comment no-retry
surface.  With native `:{123::/*c*/}`, `::` remains the second outer tag Error,
`/*c*/` is PV-local BlockComment trivia, and the actual RBrace closes the PV.
At EOF `:{123::/*c*/`, the AST/direct prefix ends at byte 7 with outer Missing
Brace and exact `/*c*/` remainder; the prefix has no comment or RBrace node.
AST/direct topology, ordered recoveries, source reconstruction, and
episode/frame balance establish neither a payload retry nor spacing/comment
generalization.

The sixteenth bounded slice pins only the active-newline fence termination
pairs for `+` and `@@`.  Its harness explicitly installs both the outer
`StopKind::Newline` and an incoming caller-boundary fence before each AST and
direct parse: `:{A+\n:{B}}` / `:{123+\n:{B}}` preserve bytes 4 / 6, and
`:{A@@\n:{B}}` / `:{123@@\n:{B}}` preserve bytes 5 / 7.  Both pairs consume
their full sources; the PV locally owns its Newline and actual RBrace, emits
no payload node, and has the exact valid/wrong tag Error sequences.  The outer
state remains unchanged through each episode, then the harness alone clears
the fence and pops the outer stop frame.  This establishes neither another
spelling, retry/decline classification, CRLF behavior, nor any rewrite
behavior.

The seventeenth bounded cell records the distinct comment-trivia overwrite,
not another preservation result.  With the same outer `StopKind::Newline`,
the harness installs incoming fences at the physical newline for
`:{A::/*c*/\n:{B}}` / `:{123::/*c*/\n:{B}}` (bytes 10 / 12), but both AST and
direct parsing leave the outer stop intact while replacing the fence with the
start of the complete comment/newline trivia run (bytes 5 / 7).  Both fully
...
The eighteenth bounded cell pins a separate CRLF preservation pair for `+`.
With the outer `StopKind::Newline` and incoming fences at bytes 4 / 6,
`:{A+\r\n:{B}}` / `:{123+\r\n:{B}}` fully consume and preserve the exact
outer fence and stop.  Their PVs locally own the two-byte Newline ranges
`4..6` / `6..8`, the final malformed tags, and actual RBraces, with no payload
node.  This establishes only the direct CRLF pair; it neither parameterizes
the LF cells nor determines another spelling, retry/decline, or rewrite rule.
