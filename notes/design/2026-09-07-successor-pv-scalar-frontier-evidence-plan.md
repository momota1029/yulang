...
`:{A@@\n:{B}}` / `:{123@@\n:{B}}` and `:{A->\n:{B}}` / `:{123->\n:{B}}`
preserve bytes 5 / 7.  Each pair consumes its full sources; the PV locally
owns its Newline and actual RBrace, emits no payload node, and has the exact
valid/wrong tag Error sequences.  The outer state remains unchanged through
each episode, then the harness alone clears the fence and pops the outer stop
frame.  This establishes neither a retry/decline classification, CRLF
behavior, nor any rewrite behavior.
