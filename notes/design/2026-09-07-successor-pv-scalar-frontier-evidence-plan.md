...
| scalar ownership | `legacy_polymorphic_variant_payload_scalar_run_boundaries_are_execution_pinned` and `legacy_polymorphic_variant_payload_scalar_fence_termination_is_execution_pinned` | valid-name `->`, `+`, `@@`, atomic `::/*c*/`, and CRLF boundary ownership, plus wrong-head inline nested-PV retry at native close/outer EOF, distinct CRLF tag-loop outer EOF, unspaced comment native/prefix no-retry ownership, and the exact valid/wrong `+` active-newline caller-fence pair |
...
pair for `+`.  Its harness explicitly installs both the outer
`StopKind::Newline` and an incoming caller-boundary fence before each AST and
direct parse: `:{A+\n:{B}}` preserves the fence at byte 4, and
`:{123+\n:{B}}` preserves it at byte 6.  Both parses consume the full source;
the PV locally owns its Newline and actual RBrace, emits no payload node, and
has the exact valid/wrong tag Error sequences.  The outer state remains
unchanged through each episode, then the harness alone clears the fence and
pops the outer stop frame.  This establishes neither another spelling,
retry/decline classification, CRLF behavior, nor any rewrite behavior.
