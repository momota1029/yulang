...
**Amendment, 2026-07-23 session (SUBP-H-1 verification finding).** While independently verifying uncommitted SUBP-H-1 work (public `UnsatisfiedSubtypeProvenance` field, `general_subtype_diagnostic_context` rendering), Claude found the real CLI produces no related-information note at all when std is loaded, even though the unit test (std-free fixture) passes. Root cause (confirmed by Codex with real measurements): `build_subtype_provenance_sidecar` exports the ENTIRE session's occurrence set in one call before the session is dropped (per Section 6.5's "populated sidecar" requirement for fresh full lowering), sorted deterministically but without regard to source origin. With std loaded this is ~14,467 roots against a `PortableProvenanceExportBudget::default()` global `max_anchors` of 64 -- std's own occurrences (sorted first) exhaust the entire anchor budget before the user's fixture occurrence is ever reached, so its exported anchor set is empty and `record_shadow_failure` attaches `Incomplete`/empty groups. This is starvation from an ordering choice, not a bug in any downstream layer (query, rendering, cache) -- all of SUBP-F/I-3/G-1 behave correctly on the (empty) data they receive. Naively raising `max_anchors` alone shifts the bottleneck to the node/edge budgets and, pushed further, costs 60+ seconds of export time -- not an acceptable fix. Section 9.3's "anchors per occurrence" budget dimension was sized from SUBP-A/B single-failure measurements and was never validated against a whole-session, std-scale export; that gap is the actual root cause, not an implementation defect in any single slice.

**Decided fix direction (Claude, this session):** change occurrence export ORDER, not budget size. Occurrences whose `TypeOccurrenceOwner` originates in the user's own compiled source (the fresh-lowering unit currently being built) must sort and export before occurrences originating in std/imported/prefix-cached material, within the existing deterministic-ordering discipline (Section 4.4/8.2's stable-order requirement is preserved -- this changes the ordering key, not determinism itself). This keeps the existing global budgets as a real, bounded ceiling (no scaling risk, no export-time regression) while ensuring the anchors that actually matter for a covered failure -- the user's own mismatched expression/pattern/definition sites -- are exported first and thus survive `max_anchors` exhaustion. std-internal occurrences may legitimately end up `Incomplete` under budget pressure; per Section 9.3 this is the designed degradation, not a new failure mode. This is a scoped fix to `build_subtype_provenance_sidecar`'s occurrence export call, not a re-architecture of the export mechanism.
