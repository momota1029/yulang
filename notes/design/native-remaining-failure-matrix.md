...
| N11 | Open-range nondet `.once` routing | Open: VM / CPS eval / CPS repr eval reach `:just 3` for the reduced guard case, but CPS repr JIT displays `1`. CLI `examples/06_undet_once.yu` likewise returns an early tuple instead of VM's `just (3, 4, 5)`. | The guard boolean is leaking to the root on the JIT path, so the post-guard / `.once` continuation chain is not being resumed in the same way as CPS eval. | Compare JIT `YULANG_CPS_JIT_TRACE=1` against CPS eval frame trace around open-range `guard:` and `.once`; fix the runtime frame routing, not the display side. | `runs_undet_once_open_range_guard_through_cps_repr` returns `:just 3`, and `examples/06_undet_once.yu` matches VM. |
...
| N13 | Combined finite nondet + junction + method result | Open: CPS eval reaches the expected result, but CPS repr eval currently errors with `CPS function root_0 expected plain value CpsValueId(usize::MAX)`. CLI `examples/showcase.yu` returns `1` for the final root instead of VM's `just 18`. | A sentinel / missing value is crossing the CPS repr root boundary in the junction + finite nondet + method-call shape. This is deeper than pretty-printing and must be treated as a continuation/value routing invariant failure. | First fix the CPS repr eval sentinel leak, then re-check Cranelift JIT frame routing for the same source shape. | `runs_junction_method_undet_once_through_cps_repr` returns `:just 18`, and `examples/showcase.yu` final root matches VM. |
...
6. Fix N11 before native release; it prevents guard booleans from leaking out of
   open-range `.once` on the JIT path.
7. Keep N12 as a regression for nested mutable reference updates.
8. Fix N13 before native release; it currently exposes a CPS repr sentinel at
   the root boundary before the JIT comparison can be trusted.
