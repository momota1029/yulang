...
| N11 | Open-range nondet `.once` routing | Fixed: VM / CPS eval / CPS repr eval / CPS repr JIT reach `:just 3` for the reduced guard case, and CLI `examples/06_undet_once.yu` returns `just (3, 4, 5)`. | Cached handler `return_frame_threshold` must remain the native JIT truncation source; deriving the boundary from a prompt_exit marker can drop the caller continuation after the frame slice has already been routed. | Keep the release-gate regression around when changing ScopeReturn routing, routed jumps, prompt_exit frames, or handler return-frame thresholds. | `runs_undet_once_open_range_guard_through_cps_repr` returns `:just 3`, and `examples/06_undet_once.yu` matches VM. |
...
| N13 | Combined finite nondet + junction + method result | Fixed: CPS eval / CPS repr eval / CPS repr JIT return `:just 18`, and CLI `examples/showcase.yu` ends with `just 18`. | Effectful direct-call reification must skip calls whose result is immediately forced, and native ScopeReturn routing must use cached handler thresholds rather than prompt_exit marker search after frame slicing. | Keep the release-gate regression around when changing direct-call reification, prompt_exit routing, or nondet/junction helper lowering. | `runs_junction_method_undet_once_through_cps_repr` returns `:just 18`, and `examples/showcase.yu` final root matches VM. |
...
6. Keep N11 as a release-gate regression for open-range `.once`.
7. Keep N12 as a regression for nested mutable reference updates.
8. Keep N13 as a release-gate regression for combined junction + finite nondet
   + method-call roots.
