...
| N8 | Handler escape through fold / for | Outer `sub` catches a callback `return` thrown inside `for`, but CPS repr Cranelift then keeps evaluating the inner fold/for chain and overwrites the result. | Public examples `03_for_last.yu` and `04_sub_return.yu` still hang or mis-return on forced CPS repr when the iterator is an open range; finite list reproduction returns `0` instead of VM's `2`. | Make handler catch completion abort the current force/apply chain instead of letting the skipped continuation keep producing a normal fallback value. | `notes/bugs/native_for_loop_escape_keeps_running.yu`, `examples/03_for_last.yu`, and `examples/04_sub_return.yu` match VM on forced CPS repr. |
...
4. Fix N8 before claiming for-loop / open-range effect control parity.
