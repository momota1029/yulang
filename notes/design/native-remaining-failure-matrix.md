...
| N10 | Heap values returned from handler/resumption chains | Direct tuple/string heap values print on forced CPS repr, and the previous `thunk@0x...` root leak is gone. `examples/10_effect_handler.yu` now returns `(9, "6\n")`, missing the first recursive log segment. | Recursive handler/resumption force propagation now installs the forced tuple, but the handler env/log carried through nested `listen(k (), ...)` is still wrong. | Trace selected handler env propagation across nested `Resume`/`ResumeWithHandler` and make recursive handler arms retain the outer accumulated log. | `notes/bugs/native_effect_handler_tuple_result_prints_pointer.yu` and `examples/10_effect_handler.yu` match VM on forced CPS repr. |
