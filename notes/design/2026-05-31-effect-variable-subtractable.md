...
# 新規導入変数について
1. 返りエフェクトが注釈されていない引数の`f`に対して`f x`を推論したときに出てくる`f: α [β] → [γ] δ`について: `γ`は「何も引いてはならない型変数」であるので、新しく`#a`を立て、`Empty-subtract(γ, #a)`を登録する。これの登場する関数の述部には新しく変数`ε`を立て、`non-subtract(ε, #a)`を登録する。
...
3. `my f(g: α [β] -> [handled] γ)`と注釈された関数について: `f: α [β] -> [δ] γ`として登録し、新しく`#c`を立て、`δ`には`handled-subtract(δ,#c)`として登録する。`β`に`handled2; ε`などと書いてあった場合はそのまま登録してよく、`ε`はなんの情報も付けないとする。`f`の述部には新しく変数`ζ`を立て、`non-subtract(ζ, #c)`を登録する。
...
instantiateのときは型変数と同じように`#a`もfreshにする。また、`non-subtract(_,#a)`を呼んだ関数の出力型に付与する。
