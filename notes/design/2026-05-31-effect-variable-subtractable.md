...
# 実際の表現について
compact collect型については、通常の共変型が`変数 + record + ...`であるように、こちらも共変部分は`変数 + 行`であることが望ましい。また、反変部分は`A = 変数 + [効果; A] + [効果2; A] + ...`であるのが望ましい。実際は`Subtract = Map<Path, (Args, Subtract)>`と表現されるべきである。
