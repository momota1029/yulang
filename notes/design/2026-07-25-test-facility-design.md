...
`assert_eq` を `assert` と併せて std に置く。（ユーザー決定、2026-07-25）

等値比較は表明の大半を占め、失敗時に「何を期待し、何が来たか」を出せる価値が大きい。
`assert_eq` も `assert` と同じく遅延であり、通常実行では両辺とも評価しない。

それ以上の派生（`assert_some` など）は、必要になった時点で追加する。
...
失敗報告には、表明が書かれた位置の絶対 source position を出す。
doc comment 内の表明も、`text_range` から元ファイルの絶対位置へ写像する（4.5 参照）。
...
**test module 内の束縛はすべて評価する。**表明を含まない束縛は自明に成功する。
（Claude 判断、2026-07-25）

効果ベースの設計なので、「テストを走らせる」と「束縛を評価する」が自然に一致する。
専用の命名規約も属性も要らない。

```yu
mod test parser:
    our roundtrips = assert parse(render(x)) == x
```

同じ module にヘルパーを置いてもよい。関数値の束縛は評価しても何も起きない。

型で絞る案（`() -> [test] ()` の形の束縛だけを実行する）は採らない。値の束縛で書けなくなり、
上の簡潔さが失われる。

入れ子の扱いと、単一 module 内で複数の表明をどう報告単位に切るかは実装時に確定する。
`notes/todo/property-testing.md` は `test "名前":` ブロックの導入を将来課題として
残しており、本設計はそれと矛盾しない。当面は test module + 通常の束縛で始め、
専用構文が必要になってから足す。
...
fence の中身は**文の並び**として扱う。これは新たな決定ではなく、parser の既存挙動である。
info が `yulang` の fence は既に statement parser で Yulang CST になっている（1.6）。

`assert` / `assert_eq` は prelude に入る（2.6）ため、doc comment 内でもそのまま書ける。

失敗位置は、保持されている `text_range` から元ファイルの絶対位置へ写像して報告する。
doc comment 内の相対位置のまま出さない。（ユーザー決定、2026-07-25）
...
テスト中に実行時エラーが起きた場合、**そのテストの失敗として記録し、残りのテストは続行する**。
全体を中断しない。（Claude 判断、2026-07-25）
