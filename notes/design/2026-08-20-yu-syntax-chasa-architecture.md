...
| local mismatched closer | maximal non-empty `Error(ClosingDelimiter(PolymorphicVariantType))` | matching `}`へclose-slot retry。actual closeに到達すればComplete、safe pointならIncomplete。同episodeへMissing closeなし |
