...
`RecordPattern`でもなく、type shapeを記述するown primary grammarである。

本追補が定める範囲は次だけである。

- `TypePrimary::Record(NamedRecordType)`のCST / AST / recognition。
- `Identifier : TypeExpression`だけを持つ`TypeRecordField`。
- comma / implicit newlineだけをseparatorとするbrace-delimited field sequence。
- named record type内部のtyped recoveryとowner-safe delimiter / layout scope。

struct field annotation、enum / error payload、cast / role / where / act signature、pattern type annotationへの
wiringは行わない。forall、polymorphic variant、effect row、bracket-rowもreserved-but-unspecifiedのままである。
original core addendumのdeferred forms listではnamed record typeの項目だけを本追補がsupersedeし、
残る四形式とall use-site integration boundaryは変更しない。

### Yulang2 oracle facts

Yulang2では`OpenBrace`がtype NUDとして`parse_type_record_group`へdispatchされ、`TypeExpr > TypeRecord`
を作った（`yulang2-oracle:crates/parser/src/typ/parse.rs:36-48,368-377`）。field machineは
headをplain `Ident`に限定し、literal colonとfull type RHSをmandatoryにした
（同`:443-494`）。したがってsurface fieldは一形式だけである。

```text
TypeRecordField := Identifier Colon TypeExpression
```

`{a: A\nb: B}`はvalid newline-separated record typeとしてfixture化され、各fieldは
`TypeField > Ident / Colon / TypeExpr`を持つ
（`yulang2-oracle:crates/parser/tests/type_grammar.rs:102-129`）。一方`{a: A, b}`は
`InvalidToken`を含むことが明示検証され、shorthand fieldは存在しない
（同`:155-161`）。field-specific machineのliteral separatorはcommaだけであり
（`typ/parse.rs:443-450`）、shared delimited machineが`indent <= base_indent`のnewlineを
implicit separatorにした（`crates/parser/src/parse/mod.rs:21-23,42-49`）。semicolonはvalid
record separatorではない。

shared delimited machineはopener直後またはcomma直後のmatching closeをendとして受け取るため、
empty `{}`とmatching closeが実在するtrailing comma `{a: A,}`はvalidである
（`parse/mod.rs:35-77`）。`TypeRecordFieldMachine`にはspread / rest分岐がなく、`..Type`に
record-type authorityはない（`typ/parse.rs:452-494`）。default value、nested-pattern field、
spreadを持つ`RecordPattern`のfield familyをこのgrammarへ移植しない。

Yulang2はnamed record専用recovery tableを持たない。invalid head、missing colon、invalid RHSは
generic `InvalidToken`へ落ち、close / separator recoveryもshared untyped machineへ委ねた
（`typ/parse.rs:458-486`, `parse/mod.rs:35-77`）。本追補のtyped recovery tableは
Yulang3 architecture-localな新規契約である。

### Authoritative surface grammar

original core grammarの`TypePrimary`を次の一項だけ拡張する。
...
NamedRecordType :=
    ...
    [
        TypeRecordField
        { RecordTypeSeparator TypeRecordField }
        [ RecordTypeSeparator ]
    ]
    RBrace
...
TypeRecordField :=
    Identifier
    TypeRecordFieldTrivia
    Colon
    TypeRecordFieldTrivia
    TypeExpression

RecordTypeSeparator :=
    ExplicitRecordCommaBoundary
  | ImplicitNewlineBoundary(named_record_base)

ExplicitRecordCommaBoundary := CommaBoundary

TypeRecordFieldTrivia :=
    EmptyTrivia
  | SameLineTrivia
  | TriviaWithDeeperFollowingIndent(named_record_base)
```

`OpeningTrivia`はaccepted `{`直後のcurrent lexical-depth maximal triviaである。これはfield continuationの
unbounded `G*`ではない。opener authorityへcutした後、recordのlayout baseをcaptureするためにrecord ownerが
一度だけconsumeする。matching brace内へ入った後のopening newlineはouter statement boundaryではない。

name-to-colonおよびcolon-to-typeの`TypeRecordFieldTrivia`は一つのmaximal trivia clusterをprobeし、
same-lineまたは`named_record_base`よりstrictly deeperなnewlineだけをfieldへcommitする。
equal-or-shallower newlineはfield continuationとしてconsumeせず、record listのimplicit separator判定へ返す。
したがってproduction中にowner boundaryを越えるunbounded `G*`は存在しない。

field nameはplain unsigiled `Identifier`一個である。path-qualified name、SigilIdentifier、number、shorthand、
default、spreadをacceptしない。colonはliteral fixed punctuationであり、dynamic operatorでも
`ColonApplicationTail`でもない。RHSはstandalone canonical `TypeExpression`をrecursiveに呼ぶため、
path / call / apply / arrow / parenthesized group / nested named recordをすべて利用できる。

### Primary recognition and adjacency

sink-free `TypePrimary` candidate judgeを次の順序へ拡張する。各starterはlexically disjointなので、
この順序はprecedenceを導入しない。

1. active owner stop / matching delimiter / equal-or-shallower caller newlineを先に判定する。
2. Identifier / SigilIdentifier candidateをprobeする。
3. Number candidateをprobeする。
4. exact `(`を`ParenthesizedTypeGroup` candidateとしてprobeする。
5. exact `{`を`NamedRecordType` candidateとしてprobeする。
6. candidateなしならinput / sink / local stateを変えずcallerへreturnする。

`{`はprimaryが必要な位置でだけrecord authorityを得る。root type slot、parenthesized / call item、arrow RHS、
record field RHS、accepted `TypeApplyBoundary`後のnested primaryが該当する。`{` accept後はcutし、missing closeや
malformed fieldを理由にexpression record / block / future polymorphic variantへreinterpretしない。

record opener自体には「直前triviaがemptyであること」というadjacency gateを置かない。primary entryは
candidate開始位置から呼ばれ、直前triviaの所有権を持たないためである。既存TypeApply judgeは引き続き
non-empty `TypeApplyBoundary`を必要とするので、次を固定する。

| source | classification |
| --- | --- |
| `{a: A}` | root / nested slotのone `NamedRecordType` primary |
| `F {a: A}` | one `TypeApplyArgument` whose argument primary is `NamedRecordType` |
| `F/*c*/{a: A}` | comment-owned `TypeApplyBoundary` + record argument |
| `F{a: A}` | no TypeApply / record-tail authority; `F`でcurrent type終了し`{`をcallerへ返す |
| `({a: A})` | `ParenthesizedTypeGroup` itemのrecord primary |

Yulang2のgeneric type LED scannerは`OpenBrace`を`MlNud(OpenBrace)`へfall throughでき、leading triviaが
emptyの隣接braceにも機械的にapply authorityを与え得た
（`yulang2-oracle:crates/parser/src/typ/scan.rs:106-120`）。専用fixtureはない。Yulang3はすでにapprovedな
「adjacent callは`(`だけ、TypeApplyはnon-empty boundary」というcore invariantを優先し、`F{...}`を
新しい隠れたapplication spellingにしない。このsource acceptance差は後述のexplicit divergenceである。

accepted record primaryの後はexisting fixed type tail judgeへ戻る。したがってrecord primaryも他primaryと同様、
path / adjacent call / spaced apply / arrowを後続できる。record内部のcolonやcommaはdelimiter depthと
record ownerにより外側tail judgeへ漏れない。

### Record field layout and separator authority

`NamedRecordType`は`{` accept後にopening triviaを一度consumeし、existing
`LayoutDelimitedFrame::after_opening_trivia`と同じ式でbaseをcaptureする。
...
separator classificationは次のexhaustive orderである。
...
raw triviaである。implicit newlineへsynthetic separator nodeをemitしない。

record fieldはcomma / implicit newlineだけをacceptする。TypeCallTail / ParenthesizedTypeGroupの
`TypeDelimitedSeparator`はcomma / semicolon / implicit newlineであるが、NamedRecordTypeはそのseparator policyを
そのまま再利用しない。shared primitiveは`LayoutDelimitedFrame` / boundary classifierであり、allowed literal
...
`trailing_comma`へ保持する。implicit newlineはAST flagを作らずraw CST triviaだけを保持する。
comma後にmatching `}`より先にEOF / active owner boundaryが来た場合はvalid trailing commaではなく、
one Missing fieldとdistinct Missing closeの二slotである。この二rowを同じseparator episodeへ重ねない。

### CST vocabulary and byte ownership

次のkindをtype grammar vocabularyへ追加する。
...
`TypeRecordFieldList`やgeneric `DelimitedTypeGroup` wrapperは作らない。`NamedRecordType`は
`TypeExpression`直下のprimary child、fieldはrecord直下のsource-order childである。literal comma、opening trivia、
field間trivia、trailing triviaは`NamedRecordType`直下へexactly once emitする。name-to-colon / colon-to-RHS triviaは
`TypeRecordField`直下に置く。field RHSのown triviaはnested `TypeExpression` ownerの既存規則に従う。

explicit-comma exampleのcomplete CST shapeを次で固定する。
...
implicit-newline exampleではall opening / separator / trailing triviaを次の位置へ置く。
...
上の二例はsourceの全byteを一度だけ示す。implicit newline位置にempty `Separator`、synthetic comma、
source-absent list wrapperを置かない。zero-width `Missing`とnon-empty `Error`だけがsource byteを持たない / wrapする
recovery nodeであり、one node = one committed recovery recordを維持する。
...
pub struct NamedRecordType<'source> {
    pub open: Range<usize>,
    pub fields: Vec<Recovered<TypeRecordField<'source>>>,
    pub trailing_comma: Option<Range<usize>>,
    pub close: Recovered<Range<usize>>,
    pub range: Range<usize>,
}

pub struct TypeRecordField<'source> {
    pub name: Recovered<WordSpan<'source>>,
    pub colon: Recovered<Range<usize>>,
    pub type_expr: Recovered<Box<TypeExpression<'source>>>,
    pub range: Range<usize>,
}
```

`NamedRecordType.open / close`はexisting delimited AST conventionと同じmandatory delimiter slotである。
close recovery tableの`ClosingDelimiter(NamedRecordType)`は実在する
`close: Recovered<Range<usize>>`へ対応する。`trailing_comma`はmatching closeを実際にprobeしてvalid trailingと
classifyしたliteral commaだけを保持し、separator-before-EOF recoveryでは`None`のままである。

fieldはname / colon / typeの三mandatory slotを個別に保持する。colonをplain `Range`にせず
`Recovered<Range<usize>>`にするのは、missing / malformed colon recoveryとAST shapeを一致させるためである。
whole fieldがleading / repeated comma位置で欠けた場合は`fields`へ`Recovered::Incomplete`を追加する。
field authorityを得た後のinternal missingは`Recovered::Complete(TypeRecordField { ... })`内の該当slotだけを
`Incomplete`にし、whole field missingと混同しない。

### Type-delimited owner and typed recovery vocabulary

type-delimited owner stackへrecord ownerを追加する。

```text
TypeDelimitedOwner := Call | ParenthesizedGroup | NamedRecord
```

`NamedRecord` frameは`Delimiter::Brace`、record-local comma / implicit newline stop、
`named_record_base`を同期してpushし、normal / recovery / rollbackの全exit pathでexact restoreする。
...
`RecoveryKind::{Missing, Error}`、`Delimiter::Brace`、existing unexpected-token categoriesを再利用する。
RecordPattern roles / Expression rolesを流用しない。両者は同じbrace punctuationを使うが、item grammar、RHS family、
separator policy、recovery ownerが異なる。

recovery scannerのsafe-point predicateをAST / direct-CSTで共有する。各scanはcurrent lexical depthのEOF、
record-local comma、matching `}`、outer-owned mismatched close、active caller stop、qualifying newline、valid field / slot
...
| input state | authority / recovery | retry / ownership |
| --- | --- | --- |
| `{}` | valid empty record | matching `}`をrecord ownerがconsume |
| `{a: A}` / `{a: A, b: B}` / qualifying newline list | valid | fieldsはone canonical formのみ |
| literal comma後のpost-boundary trivia位置にmatching `}`が実在 | valid trailing comma | no empty field、`trailing_comma = Some` |
| qualifying trailing implicit newline後にmatching `}`が実在 | valid trailing boundary | no empty field、`trailing_comma = None` |
...
trailing-comma rowsはactual matching close probeの成否でmutually exclusiveである。`{a: A,}`だけがvalid trailing、
`{a: A,` + EOFや`{a: A,]`（`]`がouter-owned）はMissing field + Missing record closeになる。
leading / repeated comma rowはcommaが存在するたびにfield slotを一件作るが、valid trailing comma rowへ入った
最後のcommaにはMissing fieldを作らない。

equal-or-shallower newlineはrecovery Error runへ取り込まない。current record depthでseparator条件を満たすなら
record ownerがimplicit boundaryとしてraw triviaを一度所有し、active enclosing ownerへ属するnewlineなら
rollbackして返す。「Error scannerがconsumeしない」と「record-local implicit separatorがlistに属する」を
混同しない。

### TypeRecordField recovery table
...
| accepted name後、malformed colon bytes後にvalid TypePrimary | maximal non-empty `Error(Type::RecordFieldColon)` | recovered-missing colonとしてsame-position type retry、additional Missing colonなし |
| accepted name後、colonなしでcomma / matching close / EOF / owner boundary | zero-width `Missing(Type::RecordFieldColon, Colon)` one record | boundaryをconsumeせずfieldをclose。same causeのMissing typeをcascadeしない |
| accepted literal colon後にvalid TypePrimary | canonical nested TypeExpression | full RHS recovery belongs to nested type roles |
| accepted literal colon後にEOF / comma / matching close / owner boundary | zero-width `Missing(Type::RecordFieldType, TypeExpression)` one record | boundaryをconsumeしない |
...
- field nameをacceptし、colonとtypeが同じEOF / boundary原因で欠けた場合、Missing colon一件だけをcommitする。
  ASTの`colon`と`type_expr`はともにIncompleteでも、second diagnosticを同じ原因から作らない。
- literal colonをaccept済みならcolon slotはcompleteで、missing / malformed RHSについてType role一件だけをcommitする。
- malformed bytesをErrorとしてconsumeしてboundaryへ達した場合、そのErrorがmandatory slotの一件であり、
  同じslotへ追加のzero-width Missingを積まない。
- separator後のwhole missing fieldとrecord closeは異なるAST slotなので、tableが明記する場合だけ二件を持つ。
...
outer TypeExpression
  primary = Record
    field a type = A -> B
    field b type = List + TypeApplyArgument(Int)

F {a: A} -> Out

outer TypeExpression
  primary = F
  postfix = TypeApplyArgument(record primary)
  arrow = TypeArrowTail(Out)
```

record field colonはfuture pattern annotation / where predicate / declaration colonと共有ownerにしない。
record delimiter depthと`RecordFieldColon` roleだけがfield colonをclaimする。future use-site callerが
outer stop / missing-role overrideを渡しても、record internal rolesをcaller roleへremapしない。

### Explicit Yulang2 divergences and architecture-local decisions

1. Yulang2はimplicit newline separatorへempty `Separator` CST nodeをemitした。Yulang3はapproved
   `LayoutDelimitedFrame` contractに従い、raw triviaだけをrecord直下へ保持する。
2. Yulang2はgeneric `TypeRecord` / `TypeField` event shapeとgeneric `InvalidToken` recoveryを使った。
   Yulang3は`NamedRecordType` / `TypeRecordField`とtyped role別 Missing / Errorを使う。
...
1. `{}`、`{a: A}`、multiple comma、implicit newline、valid trailing comma、valid trailing implicit newlineを
   AST / direct-CST両方で固定する。
2. semicolonがvalid separatorにならずtyped Error recoveryになることを、call/groupのsemicolon-valid fixtureと
   対比して固定する。
3. shorthand、default、spread、sigil / numeric field nameをacceptしない。
4. nested field typeがpath / call / apply / arrow / parenthesized / nested recordをfull canonical grammarでparseする。
5. `F {a: A}`対`F{a: A}`、comment / deeper newline TypeApply boundary、record primary後のexisting tailsを固定する。
...
13. existing TypeExpression core fixturesをbyte-identical / equal ASTのまま保ち、OperatorTable dependencyを追加しない。
14. forall / polymorphic variant / effect row / bracket rowとall use-site wiringを実装しない。

### Closed decisions and internal consistency review

本追補でimplementationをblockするopen questionはない。次を確定する。

- NamedRecordTypeはTypePrimaryであり、postfix record tailではない。
- field formはplain Identifier + mandatory colon + full TypeExpressionだけである。
- valid separatorはcomma / implicit newlineだけで、semicolonはinvalidである。
- opener / field gaps / separatorのtrivia ownershipはそれぞれ明記したbounded classifierを用いる。
- ASTはopen / recovered close、recovered name / colon / type、trailing commaを実在fieldとして持つ。
...
本追補はauthoritativeなstandalone `TypeExpression` grammarへpolymorphic variant type
`:{A Int, B}`を追加する。これはexpression-side variant literalでも、pattern-side poly-variantでもなく、
tag nameとzero-or-more payload typeを持つown type primaryである。

本追補が定める範囲は次だけである。

- `TypePrimary::PolymorphicVariant(PolymorphicVariantType)`のrecognition、CST、AST。
- comma / qualifying implicit newlineだけを受けるouter tag list。
- plain `Identifier` tag nameと、same-line boundaryで連なるzero-or-more
  `TypeExpressionInTypeMlScope` inner payload sequence（AST valueはfull `TypeExpression`）。
- outer tag list / inner payload sequence間のowner-safe typed recovery、AST / direct-CST parity、byte ownership。

struct / enum / error payload、cast / role / where / act signature、pattern type annotationへのwiringは行わない。
bracket-row typeは引き続きreserved-but-unspecifiedである。original core addendumのdeferred forms listでは
polymorphic variantの項目だけを本追補がsupersedeし、remaining bracket-rowとall use-site integration boundaryを
変更しない。

### Yulang2 oracle facts

Yulang2のtype NUD scannerはcolonを`TypNudTag::PolyVariantStart`へ分類し、`parse_type_from_nud`は
`TypeExpr`を開始して`parse_type_poly_variant_group`へdispatchした
（`yulang2-oracle:crates/parser/src/typ/scan.rs:35-63,94-103`、
`crates/parser/src/typ/parse.rs:19-52`）。parserはcolonをemitし、そのtrailing triviaを渡して次のNUDをscanし、
`OpenBrace`だけをgroup openerとしてacceptした（`typ/parse.rs:379-397`）。したがってhistorical parserは
colonとbraceの間にdedicated adjacency guardを持たず、triviaを許し得た。

outer `TypePolyVariantMachine`はexplicit separatorをcommaだけに限定した
（`typ/parse.rs:507-514`）。shared `DelimitedListMachine`は`indent <= base_indent`のnewlineをimplicit
separatorにし、matching closeをopener直後またはcomma後にacceptした
（`crates/parser/src/parse/mod.rs:21-23,35-77`）。したがってempty `:{}`、actual closeが実在するtrailing comma、
comma / qualifying implicit newlineで区切るouter tag listはvalidであり、semicolonはvalid separatorではない。

tag parserはheadを`TypNudTag::Atom`かつplain `SyntaxKind::Ident`へ限定した。uppercase判定やconstructor専用tokenはなく、
SigilIdent、Number、groupはtag headにならない（`typ/parse.rs:522-534`）。accepted name後はpayload loopへ入り、
each payloadをfull `parse_type_from_nud`でparseした。ただしpayload parse中は`ml_arg = true`であり、non-empty triviaで
nested type tailを止め、次payload候補をtag ownerへ返した（同`:535-560`、`typ/parse.rs:194-205`）。

load-bearingなnewline ruleはcode上明白である。tag parserはpayload NUDをscanする前に
`TriviaInfo::Newline`をindentに関係なく検出し、そのnewlineをconsumeせずouter machineへ返した
（`typ/parse.rs:536-540`）。outer machineだけがそのnewlineを`indent <= base_indent`ならimplicit separatorにし、
deeper newlineならgroup continuationにせずcallerへ返した（`parse/mod.rs:42-49`）。forall binderのapproved
`strictly-deeper newline continues` ruleとは同じではない。

dedicated fixtureは次の三件である。

- `:{A Int, B}`はtag `A` + one payload `Int`、comma、zero-payload tag `B`を作る
  （`crates/parser/tests/type_grammar.rs:165-188`）。
- `:{A Int Bool}`はone tag `A`直下にtwo sibling `TypeExpr` payloadを作り、payload間separator nodeを作らない
  （同`:192-212`）。
- `:{A Int\nB}`はnewlineでinner payload sequenceを終え、outer machineがempty `Separator` nodeを置いてtag `B`へ進む
  （同`:216-238`）。

type LED scannerはcolonを`MlNud(PolyVariantStart)`へfall throughでき、outer tailはそれをordinary `TypeApply` argument
としてparseした（`typ/scan.rs:106-121`、`typ/parse.rs:320-337`）。polymorphic variant自体はtailではなく、
TypeApply slotへ現れ得るprimaryである。

Yulang2はpolymorphic-variant-specific recovery tableを持たない。non-brace after colon、invalid tag head、unknown
separatorはgeneric `InvalidToken`へ落ち、delimited closeもshared untyped machineへ委ねた
（`typ/parse.rs:385-394,527-531`、`parse/mod.rs:35-77`）。以下のtyped recoveryはYulang3-local contractである。

### Architectural decisions

1. polymorphic variantはcanonical `TypePrimary`であり、postfix tailやdynamic operatorではない。
2. introducerはexact adjacent `Colon LBrace` pair `":{"`である。complete pairのsink-free probe成功後だけcutする。
3. payload boundaryはnon-empty same-line triviaだけである。physical newlineはindentに関係なくinner payload sequenceを終了する。
4. outer tag listだけがcomma / qualifying implicit newlineをseparatorとして所有する。semicolonをacceptしない。
5. tag nameはplain `Identifier`であり、uppercase conventionをparser constraintへ昇格しない。
6. payloadはfull fixed-precedence `TypeExpression`だが、one payload parse中はexisting `type_ml_arg` modeをpushし、
   次のnon-empty triviaをcurrent payloadのTypeApplyへ吸収させない。
7. outer tag listとinner payload sequenceはdistinct owner phaseを持ち、同じtrivia / token / recovery siteを二重にclaimしない。
8. optional probeはstate-neutralであり、accepted pair後のnormal / recovery / rollbackはdelimiter、stop、layout、
   type-delimited owner、type-ML stateをexact restoreする。

本追補でYulang3 grammar mechanismを指すcanonical termsは`outer tag list`と`inner payload sequence`である。oracle facts内の
`TypePolyVariantMachine` / outer machine / payload loopはYulang2 implementation固有名、synthetic wrapperはCST node shapeの語であり、
canonical mechanism名の別表記ではない。

### Introducer adjacency decision

authoritative introducerをexact adjacent `":{"`とする。

```text
AdjacentLBrace :=
    LBrace whose first byte is exactly Colon.end
```

これはEffectRowTypeの`Apostrophe AdjacentLBracket` / `AdjacentLBracket := LBracket whose first byte is exactly Apostrophe.end`
と同じBNF patternである。pair probeはatomicだが、`Colon`と`AdjacentLBrace`は引き続き別terminalなので、ASTの`colon` / `open`
fieldとCSTのtwo-token ownershipをそのまま表せる。

colonはapostropheと違いSigilIdentifierとlexically競合しない。それでもadjacent pairを選ぶ理由は、colon単体がfuture
type annotation / predicate / declaration ownerのstructural stopになり得るためである。colonだけでprematurely cutし、その後の
comment、newline、caller tokenをpolymorphic-variant recoveryへ移すより、complete two-token spellingをatomic candidateにする方が
owner-safeである。またalready-approved EffectRowのcompound introducerと同じ「pair全体をprobeしてからcut」という規則になり、
optional primary judgeがbare punctuationを奪わない。

| source | classification |
| --- | --- |
| `:{A}` | one `PolymorphicVariantType` primary |
| `: {A}` / `:/*c*/{A}` / `:\n{A}` | polymorphic variant candidateではない |
| bare `:` / `:` + EOF | candidateではなくexisting missing / malformed TypePrimary recoveryへ戻る |
| `F :{A}` | non-empty TypeApply boundary後のpolymorphic-variant argument |
| `F:{A}` | TypeApplyでもtype postfixでもない。`F`でcurrent typeを終えcolonをcallerへ返す |

これはtriviaを許したYulang2からのintentional narrowingである。
...
PolymorphicVariantType :=
    Colon AdjacentLBrace PolyVariantOpeningTrivia
    [
        PolymorphicVariantTag
        { PolyVariantTagBoundary PolymorphicVariantTag }
        [ PolyVariantTagBoundary ]
    ]
    RBrace
...
PolyVariantTagBoundary :=
    ExplicitPolyVariantCommaBoundary
  | ImplicitNewlineBoundary(poly_variant_base)
...
`poly_variant_base`を一度だけcaptureする。

```text
incoming_base := current lexical-depth indentation baseline, or 0
poly_variant_base :=
    if opening trivia contains a physical newline
       and following_line_indent > incoming_base
    then following_line_indent
    else incoming_base
```

payload boundaryにはunbounded `G*`もdeeper-newline caseもない。one maximal trivia clusterがphysical newlineを
含まなければboundary、含めばinner payload sequenceはtrivia前へrollbackしてtagを終了する。outer tag listは同じtriviaをexactly once
...
continuationにすると`A Int\n  B`の`B`をpayloadかtagかsyntaxだけで区別できない。Yulang2のany-newline hard stopを保存し、
indentはouter tag list separator authorityだけに使う。

payloadはnon-empty same-line boundaryを要求する。Yulang2 loopにはnon-empty-trivia assertionがなく、adjacent `(`等を
payload NUDとして機械的にacceptし得たがfixture evidenceはない。Yulang3はinner payload sequenceをML-style source surfaceとして
明示し、`A(Int)`にはzero-width missing payload boundary recoveryを適用する。このintentional clarificationは後述する。

### Primary recognition and tail interaction

canonical type-primary judgeを次のexclusive orderへ更新する。

1. active stop、matching / outer-owned delimiter、caller-owned newlineを判定しboundaryをconsumeしない。
2. exact maximal contextual `for`をexisting Forall candidateとしてprobeする。
3. exact adjacent `Apostrophe LBracket`をexisting EffectRow candidateとしてprobeする。
4. exact adjacent `Colon LBrace`をPolymorphicVariant candidateとしてprobeする。
5. Identifier / SigilIdentifier、Number、`(`、`{`をexisting orderでprobeする。
6. no candidateならstate-neutralにreturnする。

all startersはpair completionまでsink-freeである。step 4がcolonだけでcutすることはない。accepted
PolymorphicVariantTypeはforallと異なりterminal primaryではなく、existing fixed tail loopへ戻る。したがって
`:{A}::Result`、`:{A}(X)`、`:{A} T`、`:{A} -> Out`はordinary path / call / apply / arrow rulesへ従う。
TypeApply argument candidateでもsame canonical pair probeを使い、variant-specific LED branchを追加しない。

### Parser-side AST and CST vocabulary
...
pub struct PolymorphicVariantType<'source> {
    pub colon: Range<usize>,
    pub open: Range<usize>,
    pub tags: Vec<Recovered<PolymorphicVariantTag<'source>>>,
    pub trailing_comma: Option<Range<usize>>,
    pub close: Recovered<Range<usize>>,
    pub range: Range<usize>,
}

pub struct PolymorphicVariantTag<'source> {
    pub name: Recovered<WordSpan<'source>>,
    pub payloads: Vec<Recovered<PolymorphicVariantPayload<'source>>>,
    pub range: Range<usize>,
}

pub struct PolymorphicVariantPayload<'source> {
    pub boundary: Recovered<Range<usize>>,
    pub type_expr: Recovered<Box<TypeExpression<'source>>>,
    pub range: Range<usize>,
}
...
payload wrapperはinner payload sequence全体を囲むsynthetic wrapperではない。each payloadがmandatory leading same-line boundaryとnested typeという
二つのrecoverable slotをsource orderで所有するためのsource-bearing nodeである。valid boundary rangeはtrivia cluster全体、
payload rangeはboundary startからtype endまでである。adjacent payload recoveryではboundaryだけIncomplete、typeはCompleteに
できる。malformed payloadがtag boundaryまで達した場合はcomplete payload skeleton内のtype slotをIncompleteにする。
...
TypeDelimitedOwner::PolymorphicVariant
...
`ExpectedSyntax::{Identifier, TypeExpression, DelimitedSequenceSeparator}`、comma / close punctuation evidence、
`RecoveryKind::{Missing, Error}`を再利用する。tag / payload / close recoveryはそれぞれ実在する
`tags` slot、`name` slot、`payloads` slot、`boundary / type_expr` slot、`close` slotへ対応する。
`PolymorphicVariantTagSeparator`だけはexisting delimited-list separator roleと同じcommitted diagnostic roleであり、
source-absent separator AST fieldを作らない。literal / malformed punctuationとraw triviaはlossless CSTに残る。

### Outer tag list machinery

`":{"` accept後、次をone outer frame episodeとしてpushする。

```text
delimiter          = Delimiter::Brace
type owner         = TypeDelimitedOwner::PolymorphicVariant
stops              = active stops + Comma + RightBrace
layout             = LayoutDelimitedFrame(poly_variant_base)
explicit separator = Comma only
```

これはNamedRecordTypeのcomma / qualifying implicit newline frameをparameterizeして再利用する。outerのtoken authority、safe point、
...
各probeはsink-freeに同じ位置から比較する。outer tag list stateはleading / repeated / trailing separator時のAST slot数だけを決め、
...
用語上、`active caller stop`はenclosing ownerが登録したboundary tokenのgeneral categoryである。そのcurrent tokenがsemicolon
であるspecific instanceだけを`active caller-owned semicolon`と呼び、local authorityを持つinverse caseを
`non-caller-owned semicolon`と呼ぶ。この三語はsynonymではなくcategory / instance / complementの関係である。

`NT`のauthoritative orderは次だけである。

1. **`NT-1 MatchingClose`:** actual current-owner `}`。opener直後ならvalid empty、complete / recovered tag直後ならnormal close、
   comma / qualifying implicit newline後のunfilled tag-required stateならactual-close-present trailing boundaryとしてacceptする。
   filled stateならexisting Incomplete tagを保持してcloseし、second tag slotを作らない。
2. **`NT-2 ClosingDelimiter`:** other closing-delimiter token。active callerがownするouter closeならconsumeせず`NT-7`へ渡す。
   ...
   `NT`へretryする。
3. **`NT-3 Comma`:** literal comma。complete / recovered tag後ならexplicit boundaryとしてunfilled tag-required stateへ入る。すでに
   tag-required stateならone Missing tag slotをcomma位置で満たすleading / repeated boundaryとしてconsumeし、filled stateで
   `NT`へre-enterする。
4. **`NT-4 Semicolon`:** literal semicolon。active caller-owned semicolonならlocal authorityを得ず`NT-7`へ渡す。
   non-caller-owned semicolonの場合だけexact tokenへ`Error(PolymorphicVariantTagSeparator)`をcommitし、tag slotを作らず`NT`へ
   re-enterする。このbranchはopener直後にも同じように適用するため、`:{;A}`を取りこぼさない。
5. **`NT-5 PhysicalNewline`:** physical newline。`indent <= poly_variant_base`ならqualifying implicit newlineとして
   exactly once consumeし、complete / recovered tag後ならunfilled tag-required stateへ入り、すでにtag-requiredならそのfilled / unfilled
   statusを保って`NT`へre-enterする。strictly deeperならconsumeせず`NT-7`へ渡す。
...
7. **`NT-7 OwnerBoundary`:** EOF、active caller stop（semicolon instanceはactive caller-owned semicolon）、outer-owned close、または`NT-5`の
   non-qualifying physical newline。zero-width `Missing(ClosingDelimiter(PolymorphicVariantType))`を置き、boundary / leading gapを
   consumeせずcallerへ返す。commaまたはqualifying implicit newline後のtag-required stateがunfilledなら、distinct one
   Missing tag slotを先に満たす。leading / repeated `NT-3`が同じslotをすでにfilledにした場合はsecond Missingを作らない。
...
canonical ownership filtering、valid-trailing-vs-missing-tag等のoutcomeは`NT-1..NT-7`が決め、scanner側で再判定しない。

complete / recovered tag後の`NT-3` commaと`NT-5` qualifying implicit newlineはともにBNFの
`PolyVariantTagBoundary`を実現し、consume後は同じunfilled tag-required stateを持つ。差はsource ownershipだけであり、
commaだけがactual `NT-1` close時に`trailing_comma` fieldへ入り、qualifying implicit newlineは常にraw triviaとして残る。
...
```text
TagHead
  := plain Identifierをmandatory nameとしてacceptする

PayloadOrOuterBoundary
...
したがって`:{A Int B}`を`A Int` + missing tag separator + tag `B`へsplitしない。outer
`Missing(PolymorphicVariantTagSeparator)`を発火できるstronger same-line tag-head spellingは現grammarに存在しない。
このroleはmalformed separator Errorとfuture extensionのtyped vocabularyには残すが、valid Identifierだけを根拠にzero-width
...
| --- | --- | --- |
| `:{;A}`、non-caller-owned semicolon | opener直後の`NT-4`が`;`へone separator Error、`NT-6`が`A`をaccept、`IT-2`から`NT-1` close | tagsはComplete zero-payload `A`一件、close Complete。leading semicolon用tag slot / Missing tagなし |
...
| active caller-owned semicolon contextの`:{A; ...` | `IT-2`から`NT-4`へhandoffし、canonical ownership filterが`NT-7`へroute | Complete zero-payload `A`、close Incomplete。gap / `;`は未消費、separator Errorなし |

#### Round-9 separator-state fixtures

| source / context | canonical trace | resulting shape |
| --- | --- | --- |
| `:{A,` + EOF | `A`終了後の`NT-3`がcommaをconsumeしてtag-required state、EOFで`NT-7` | Complete `A` + one Incomplete missing tag、close Incomplete。commaはraw CST separatorで`trailing_comma = None` |
| `:{A\n` + EOF、newlineはqualifying implicit newline | `A`終了後の`NT-5` qualifying implicit newlineがtriviaをconsumeしてtag-required state、EOFで`NT-7` | Complete `A` + one Incomplete missing tag、close Incomplete。newlineはraw CST trivia、`trailing_comma = None` |
| `:{A` + EOF | `IT-2`がzero-payload `A`をfinishし、separator episodeなしで`NT-7` | Complete `A`だけ、tag Missingなし、close Incomplete |
| `:{A Int, B}` | `IT-3`がpayload `Int`、`NT-3`がcomma boundary、`NT-6` / `IT-2`がzero-payload `B`、`NT-1` | Complete tags `A(Int)` / `B`、close Complete。既存oracle shape不変 |
...
| `:{A Int\nB}` | `IT-1`がnewlineを返し、`NT-5` qualifying implicit newlineでtag boundary。tags `A(Int)` / `B` |
| `:{A;B}` / `:{A ; B}` | non-caller-owned semicolonの`NT-4` Error一件、Complete zero-payload tags `A` / `B`。space有無でshape不変 |
| `:{123}` / `:{123 Int}` | `NT-6` wrong-kind name Errorのone tag。前者zero payload、後者payload `Int` |
| root `:{@\n  B}` | `NT-8`が`@`だけをIncomplete tag Error。`NT-5` non-qualifying physical newlineから`NT-7`へ進みclose Incomplete、newline / `B`はcallerへ返す |
...
`PolymorphicVariantType`はcolon、brace、opening / outer-boundary trivia、comma、tags、closeをflat source orderで持つ。
`PolymorphicVariantTag`はnameとpayload nodes、`PolymorphicVariantPayload`はleading boundary triviaとnested
`TypeExpression`を持つ。implicit separatorにsynthetic nodeを作らない。
...
nested delimiter内のnewlineとtag-level newlineの区別を次で固定する。
...
`Pair(...)`内のnewlinesはnested `TypeCallTail` frameがownしone payloadの内部に留まる。`)`後のnewlineだけがcurrent
tag depthへ戻りimplicit tag separatorになる。四例は全space / newline / punctuation byteをexactly once示す。
...
#### Outer tag list table

| input state | authority / recovery | AST / retry / ownership |
| --- | --- | --- |
| `NT-1` at opener | valid empty | `tags = []`、close Complete |
| `NT-1` after complete / recovered tag | valid close | each tagはsource-order Complete、close Complete |
| `NT-1` after unfilled tag-required state entered by complete / recovered tag後の`NT-3` / `NT-5` qualifying implicit newline | valid actual-close-present trailing boundary | empty tagを作らず、literal commaだけ`trailing_comma`へ保持 |
| `NT-1` after filled tag-required state | valid close after recovered leading / repeated boundary | existing Incomplete tagだけを保持し、second tag slotを作らずclose Complete |
| `NT-3` in already tag-required state（leading / repeated comma） | each comma位置へzero-width `Missing(PolymorphicVariantTag, Identifier)` | commaごとにone Incomplete tag slotを作ってstateをfilledにし、consume後`NT`へre-enter |
| complete / recovered tag後の`NT-3`、`NT-1`より先に`NT-7` | valid trailingではなくMissing tag | unfilled slotを`NT-7`位置でone Incomplete tagとして満たし、distinct missing close。same tagへsecond Missingなし |
| complete / recovered tag後の`NT-5` qualifying implicit newline、`NT-1`より先に`NT-7` | valid trailingではなくMissing tag | unfilled slotを`NT-7`位置でone Incomplete tagとして満たし、distinct missing close。qualifying implicit newlineはraw triviaのままで`trailing_comma`なし |
| `NT-7` in filled tag-required state | zero-width `Missing(ClosingDelimiter(PolymorphicVariantType))` only | existing Incomplete tagを保持し、close Incomplete。second tag Missingなし |
| `NT-4`、non-caller-owned semicolon（opener直後を含む） | exact token `Error(PolymorphicVariantTagSeparator, DelimitedSequenceSeparator)` | tag slotを作らずconsumeし、`NT`へre-enter。same siteへMissing separatorなし |
| `NT-4`、active caller-owned semicolon | `NT-7` outcome | close Incomplete、gap / semicolonをconsumeせずcallerへ返す。TagSeparator Errorなし |
...
| `NT-2` local mismatched closer | mismatched token own rangeへmaximal non-empty `Error(ClosingDelimiter(PolymorphicVariantType))` | mismatched token位置へMissing closeを重ねず、same close slotから`NT`へretryする。later `NT-1`ならclose Complete。actual closeを得ないままlater `NT-7 OwnerBoundary`へ達した場合は、そのNT-7位置へcanonical zero-width Missing closeを置き、close Incomplete。tag Errorなし |
| accepted tag内のsame-line Identifier | `IT-3` payload authority | Missing tag separatorなし。`A Int B`はone tag + two payloads |

unfilled tag-required stateではnext authorityが`NT-1`ならvalid actual-close-present trailing boundary、`NT-7`ならone Missing tag +
distinct Missing closeであり、二outcomeは排他的である。filled stateではexisting Incomplete tagを再利用し、`NT-1` / `NT-7`
いずれでもsecond tag Missingを作らない。semicolonはvalid separatorではないためこのstate machineへ含めない。
qualifying implicit newlineはraw triviaであり`trailing_comma`を設定しない。outer same-line Missing separatorはvalid syntaxに
reachableでないことをtable自身に明記し、multiple payload rowと競合させない。

#### Tag / payload table

| input state | authority / recovery | AST / retry / ownership |
| --- | --- | --- |
| `IT-1` after tag name / payload | valid inner payload sequence end | newline前でtagをfinishし、payload slotを作らず`NT`へhandoff |
...
inner payload sequenceはzero-or-moreなので、単なるabsenceへ`Missing(PolymorphicVariantPayload)`をemitしない。payload Missing相当の
mandatory slotは、accepted boundaryまたはpayload-intent Errorがpayload skeletonへcutした後のtype slotだけに存在する。
表のError rowがそのslotをIncompleteにし、同じcauseへzero-width Missingをcascadeしない。

AST pathはError node / recovery recordを持たないが同じrangeをadvanceし、same retry pointとComplete / Incomplete shapeを持つ。
direct-CSTはone Error or Missing nodeごとにone committed recordを作る。両pathはcandidate、phase、safe point predicateを共有し、
parse後のCST再走査でtag / payload境界を作らない。

### Explicit Yulang2 divergences and preserved behavior

1. Yulang2はcolon trailing trivia後のbraceをacceptし得た。Yulang3はowner-safe compound candidateのためexact adjacent
   `":{"`へ狭める。comments / newlineを挟むspellingはintentional rejectionである。
2. Yulang2 payload loopはnon-empty boundaryを明示検査せず、adjacent group NUDをpayloadにし得た。Yulang3は
   ML-style non-empty same-line boundaryをsurface grammarにし、adjacent primaryへtyped Missing boundary recoveryを行う。
3. any physical newlineがinner payload sequenceを終了する点、outer tag listのimplicit separatorが`indent <= base`だけである点は
   Yulang2を保存する。Yulang3はsynthetic `Separator`を作らずraw triviaだけをemitする。
4. Yulang2の`TypePolyVariantItem`直下にはpayload `TypeExpr`が並んだ。Yulang3はboundary recoveryとAST parityのため
   source-bearing `PolymorphicVariantPayload` nodeを置くが、inner payload sequence用synthetic wrapper / separator nodeは作らない。
5. plain Identifierだけがtag headで、uppercase restrictionがない点、comma-only outer policy、empty / trailing comma、
   zero-or-more payloadはYulang2を保存する。
6. Yulang2はgeneric InvalidToken recoveryだけを持った。Yulang3はphase-specific typed roles、maximal Error、zero-width Missing、
   safe-point non-consumption、same-position retry、no-cascadeを新設する。
7. polymorphic variantがordinary TypeApply argumentになり、primary accept後にfixed path / call / apply / arrow tailへ戻る点は
   Yulang2を保存する。special postfix ruleを追加しない。

### Implementation boundary and gates

implementationはstandalone `type_expr.rs`、必要なSyntaxKind / session vocabulary、同module testsだけを変更する。
use-site、expression / pattern grammar、HIR / lowering / inferenceは変更しない。

1. exact adjacent `":{"`だけがauthorityを得て、`: {` / comment-gap / newline-gap / bare colonは得ない。
...
5. outer separatorをcomma / qualifying implicit newlineに限定する。semicolonのsafe-point判定、inner-to-outer handoff、
   non-caller-owned / active caller-owned ownershipはcanonical `IT-2` / `NT-4` / `NT-7` outcomeを参照し、両caseとspace有無をfixture化する。
6. payload boundaryをnon-empty same-line triviaに限定し、any newlineでinner payload sequenceを終了する。
...
10. opener-immediate semicolon、leading / repeated separator、`NT-3` / `NT-5` trailing-before-EOF、non-caller-owned / active caller-owned semicolon、
    contextual `for` wrong-kind tag head、wrong-kind / malformed tag name、
    semicolon vs generic malformed byte、non-qualifying physical newline、malformed-prefix後のwrong-kind primary、every-reentry missing /
    local mismatched closer、innerからhandoffするtrivia-prefixed local mismatched closerをrole / range / retry / AST parityまでfixture化する。
...
15. existing type grammar fixturesを維持し、`OperatorTable` dependencyを追加しない。
16. bracket-row typeとall TypeExpression use-site wiringを実装しない。

### Closed decisions and internal consistency review

本追補でimplementationをblockするopen questionはない。

- introducerはexact adjacent `Colon LBrace`である。
- payload boundaryはnon-empty same-line triviaだけで、any physical newlineがinner payload sequenceを終了する。
...
final reviewでstandard four issue classにterminology identityを加え、both nesting levelsについて再点検した。
...
2. **CST byte completeness:** three oracle fixturesとnested-call newline exampleでcolon、braces、all spaces / newlines、comma、
   names、payload bytesへexactly one homeを示した。implicit separatorやinner payload sequence用synthetic nodeはない。
3. **AST / recovery-slot alignment:** tag、tag name、payload、payload boundary / type、trailing comma、close recoveryには
   ...
   slotだけ、`NT-8`はtag slotだけ、`IT-3..4`はpayload slotsだけをshapeし、`NT-7 OwnerBoundary`はclose Incomplete以外を
   implicitに作らない。ただし`NT-3`または`NT-5` qualifying implicit newline後のunfilled tag-required stateだけは、tableどおり
   distinct Missing tag slotを先に満たす。filled stateへsecond Missingは作らない。
...
5. **terminology identity:** compound introducer BNFはEffectRowTypeと同じ`Colon AdjacentLBrace` patternだけを使う。current grammarの
   repeated structuresは`outer tag list` / `inner payload sequence`、accepted layout separatorは`qualifying implicit newline`、
   close-boundary branchは`NT-7 OwnerBoundary`で統一する。`active caller stop`はgeneral category、
...
### Current yu-syntax implementation re-verification

本revisionで現行Rust実装とexisting testsを再検証した。この節はobserved evidenceだけを
記録し、新しいjudge / recovery orderは定義しない。

- AST `parse_type_expression_in_context`はForallまたはclose IncompleteのPolymorphicVariantを
  parseした直後にempty postfix / arrowでreturnする（`type_expr.rs:220-230`）。direct pathは
  `DirectTypePrimary`が`Ordinary`以外ならtail loop前にreturnし（`:350-360`）、Forallと
  unclosed variantをそれぞれterminal outcomeにする（`:592-625`）。variant ASTのcloseは
...
- `parse_type_delimited_items`（`:1774-1859`）と`commit_direct_type_delimited`
  ...
  malformed scanner、malformed run後のseparator / terminal boundary、deeper-newline後のcandidate / local mismatch、close retryに
...
- existing `effect_row_reuses_type_call_delimited_recovery_slots` testは`'[A)`のdirect pathに
  ...
  deeper-indented close、terminal item後のdeeper candidate、complete item + deeper newline + local mismatchはcoverしない。
  `effect_row_primary_is_adjacent_semantically_blind_and_composes_normally`
...
row content parsingはexisting implementationを共有する。現行
...
current direct-CST pathはaccepted arrow tailのleading triviaを`TypeArrowTail` start前にemitしている。
このexisting owner boundaryを保ち、row直前のboundary triviaもenclosing `TypeExpression`直下に置く。
row後のarrowまでのtriviaだけが`TypeArrowTail`直下である。
...
TypeExpression :=
    ...
    TypePrimary
    { TypeTightTail | TypeApplyArgument }
    [ TypeArrowBoundary TypeArrowTail ]

LeadingBracketRow := BracketRow

TypeArrowBoundary := TypeChainTrivia
...
`TypeArrowBoundary`はsynthetic nodeではなく、accepted tail直前のenclosing `TypeExpression`が持つ
raw trivia clusterのgrammar上の名前である。rowの有無でこのownerを変えない。
...
`BR-L1`はaccepted leading triviaをexisting no-row arrowと同じくenclosing `TypeExpression`直下へ
emitしてからone `TypeArrowTail`を開き、rowを`argument_effect`へ入る。row後のarrow /
RHSは`BR-A`を使い、current tail loopを必ず終了する。rowなし`T -> U`の
trivia emission order / node boundary / RHS recursionは一切変更しない。
...
| Δ6 | complete item後の`LayoutDelimitedBoundary::None`、またはmalformed run後のlocal mismatched close | first mismatchをconsumeしてclose Incompleteでreturn | each mismatchへclose Errorをemitしactual close / safe boundaryまでretry | `"'[A)"`はdirect Error + later MissingをassertするがAST remainderはassertしないためpartial | `BR-RP4` |
...
| Δ9 | complete item + deeper-newline trivia後のlocal mismatched close | `DeeperNewline` branchが後続tokenをprobeせずclose Incompleteでreturnし、mismatchをunconsumedにする（`type_expr.rs:1837-1840`） | post-trivia item loopをbreak後、close loopがmismatchをErrorとしてconsumeしactual close / safe boundaryまでretry（`:988-1013,1026-1045`） | **gap:** `"'[A\n  )]"`相当なし | `BR-RP3`から`BR-RP4`へhandoff |
...
complete item後にdeeper-newline triviaをconsumeした時点の全followerはここで閉じる。literal separatorは
layout判定前に両pathがconsumeするshared branch、actual close / next primaryはΔ5、local mismatched closeは
Δ9である。EOF / active・outer owner boundary / その他のmalformed byteでは両pathとも後続byteをconsumeせず、
AST close Incomplete / direct close Missingで止まる。したがってこのcross productに第10のprojection差はない。
...
3. **Deeper newline after a complete item (`BR-RP3`):** containerがdeeper-newline triviaを受け取ったら
   ...
   fieldはなく、directだけがzero-width Missing separatorをemitする。local mismatched closeならclose slotへ
   handoffして`BR-RP4`を適用する。EOF / active・outer owner boundary / other malformed byteならsourceを
   consumeせず、AST close Incomplete / direct close Missingで止まる。literal separatorはこのruleより先の
   shared branchが所有する。
...
| opener直後がEOF / owner boundaryでactual `]`なし | Missing `BracketRowItem`、次にMissing close | ASTは`items = [Incomplete]`, close Incomplete; directはzero-width 2 records | current shared helpersの実際 |
...
| complete item後のdeeper newline + local mismatched close | directはError `ClosingDelimiter(BracketRow)`、ASTはsource-free consume | `BR-RP3`から`BR-RP4`へhandoff。later actual `]`ならclose Complete、later safe boundaryならdirect Missing / AST Incomplete | Δ9; existing EffectRow fixtureなし |
...
| complete item直後（deeper newlineなし）のlocal mismatched close | directはError `ClosingDelimiter(BracketRow)`、ASTはsource-free consume | `BR-RP4`; later actual `]`ならComplete、later boundaryならdirect Missing / AST Incomplete | Δ6; current direct helper + bracket AST close-slot adapter |
...
| --- | --- | --- | --- |
| `T [` + EOF | row `items = [Incomplete]`, close Incomplete | Missing item + Missing close、その後`BR-A`のMissing arrow | helperどおり。3 recordsはitem / close / arrowの異なmandatory slots |
...
| `T [A\n  )] -> U` | item `A`、deeper-newline trivia、`)`をsource-free consume、close Complete | close Error on `)`後にactual `]`をconsume、arrow / RHS valid | Δ9。`BR-RP3`がmismatchをclose slotへ渡し、`BR-RP4`で両pathを収束させる |
...
`T [@] -> U`のAST item omissionと`T [e)] -> U`のgeneric AST early return、追加8 traceのlegacy差は
...
| --- | --- | --- | --- |
| `BR-N1..3` / `BR-N5` candidate | valid mandatory head | `primary = Complete`;そのheadが返すexisting dispositionをそのまま適用 | source-observed shape; fixtureなし |
...
| disabled `BR-N4` second row with matching `]` | delimiter-aware maximal Error `LeadingEffectTypeHead` | full nested `[...]`をone Error rangeとしてconsume、same primary slot retry | Yulang3-inferred recovery |
| other malformed non-empty bytes後にvalid allowed head | maximal Error `LeadingEffectTypeHead` | same primary slot retry; success時Complete | Yulang3-inferred recovery |
...
valid / recovered headのdispositionはexisting primary grammarが決める。AST pathは現行
`parse_type_expression_in_context`と同じく`TypePrimary::Forall(_)`を常にterminal、
`TypePrimary::PolymorphicVariant(PolymorphicVariantType { close: Incomplete, .. })`をterminal、
その他をordinaryとする。
direct pathは現行`DirectTypePrimary::{TerminalForall,
TerminalIncompletePolymorphicVariant, Ordinary}`の結果をそのまま使う。terminal headは
`postfix = [] / arrow = None`で直ちにenclosing `TypeExpression`をfinishし、headが返したtrivia /
owner boundaryをconsumeしない。leading rowはこのdispositionをordinaryへ上書きしない。

したがって`[e] :{A Int\n B}`のheadがunclosed polymorphic variantであるとき、
newline前でterminal returnし、newlineを`BR-L`へ渡さない。`[e] for 'a: T`もForall自身の
terminalityを保つ。closed polymorphic variant、EffectRowType、NamedRecordType、atom / groupだけが
ordinary dispositionでexisting fixed-tail loopへ進む。

disabled `BR-N4`のrecovery scannerはdelimiter-awareである。matching `]`があるsecond rowは
nested delimiter depthとopaque lexical regionを認識し、openerからそのopenerに対応するmatching close endまでを
one balanced unitとしてconsumeする。single `[`だけをErrorにしない。matching closeがなければ
owner-safe boundary直前までのmaximal local runとし、boundaryをconsumeしない。

`[e][f]T`のexact resultは、outer row `leading_effect_row = Some([e])`、
`primary = Complete(Identifier T)`、empty postfix / arrowである。direct-CSTはsource range `[f]`全体へ
one `Error(Type::LeadingEffectTypeHead, TypeExpression)`をemitし、second `BracketRow` nodeや
Incomplete primaryを追加しない。Error後の`T`はexact same primary slotのretryである。
...
deeper-newline triviaの2 componentsはrow内に一度ずつemitする。Δ9の`BR-RP3` → `BR-RP4`
handoffにより、directは`)`をclose Errorとして所有し、ASTは同じbyteをsource-free consumeする。
両pathともlater actual `]`でclose Complete、arrow / RHS validとなる。
...
TypeExpression
  BracketRow
    ...
    TypeExpression
      Identifier "e"
    RBracket "]"
  Error(Type::LeadingEffectTypeHead, TypeExpression) "[f]"
  Identifier "T"
```

second opener / item / closeの3 bytesは`BR-H`のone delimiter-aware Errorが所有し、
second `BracketRow`は作らない。
...
このno-row baselineはcurrent direct-CST emission orderそのものである。row-present exampleも
pre-tail whitespaceを同じenclosing ownerへ置くため、本追補は`T -> U`のnode boundaryを変えない。
...
8. **Inference:** second leading row rejectionとfull balanced `[f]` Error consumption、missing arrow後のvalid RHS retry、
   malformed head / arrowのmaximal ErrorはYulang3のtyped-recovery architectureから決めた。
   yulang2専用fixtureに根拠を持たない。
...
5. empty、comma / semicolon / newline、trailing separator、leading / repeated separator、same-line missing separator、
   ...
   `T [:] -> U`、`T [@ A] -> U`、`T [@\nA] -> U`、`T [@\n  A] -> U`、`T [A\n  ] -> U`、
   `T [A\n  )] -> U`、`T [)] -> U`、`T [@, A] -> U`、malformed run中のsame-line / multiline comment triviaと、
...
7. `[e] for 'a: T`と`[e] :{A Int\n B}`でhead-own terminal dispositionを保ち、後者のnewlineを
   unconsumedでcallerへ返すことをAST / direct-CSTの両方でassertする。
8. no-trivia / same-line / deeper newline / equal-or-shallower newlineをrow-to-headとrow-to-arrowの両方でfixture化する。
9. all worked examplesでlossless round trip、every trivia byte one home、balanced nodes、no synthetic
   Separatorを確認する。
10. `T -> U`のCST / AST / recoveryをbyte-identicalに保ち、row-present tailのleading triviaも
...
1. **bounded trivia / owner boundary:** `BR-G`のrow-to-head / row-to-arrow gapはone `TypeChainTrivia`だけで、
   ...
   complete item後のdeeper newlineに続くlocal mismatchは`BR-RP3`からclose slotへ渡し、`BR-RP4`だけが
   mismatchをconsumeする。active caller newlineはgeneric boundaryとしてconsumeしない。
2. **CST byte completeness:** eleven worked examplesは全bracket、identifier、arrow、space、newline、full `[f]` Error、
   malformed `:` / `@ ` / `@` / `)`を明示し、source-absent Missingはempty textで示した。`@ `のspaceはError内、
   Δ9 exampleのnewline / two-space indent / `)`は各one owner、pre-tail spaceはrowの有無にかかわらず
   enclosing `TypeExpression`にあり、one byteに二ownerはない。
...
4. **recovery-row exclusivity:** `BR-RP1`の6 outcomesはprimary、literal separator、implicit newline、
   ...
   complete item + deeper newline後はactual close / next primary / local mismatch / terminal-or-otherの
   `BR-RP3` partitionだけを通り、local mismatchだけが`BR-RP4`へhandoffする。one decision pointで二rowは
   発火しない。close Error後のlater boundary Missingはnext retry positionの
...
5. **single canonical statement:** grammarは`BR-G`、fresh / tail judgeは`BR-N` / `BR-L`、row / head / arrow
   recoveryは`BR-R` / `BR-H` / `BR-A`にそれぞれ1回だけ定義した。shared-helper差（Δ1..Δ9）は`BR-RΔ`、その裁定は
