...
- `StoredContinuation` と snapshot cursor は実装した。
- ただし `ContinuationFrames` を `SmallVec<[FrameSegment; 2]>` にすると、
  recursive VM の stack frame が太り、100 回程度の ref-update loop で
  CLI stack overflow が再発した。
  active continuation の storage は heap-backed `Vec<FrameSegment>` にする。
- `split_back` / `prepend` を完全な view 操作のままにすると、
  ref-update の request/marker-close workload で悪化した。
  partial snapshot split は suffix を materialize し、
  active continuation 同士の prepend は owned に materialize する。
- この形では 100x nondet cache-hit は改善傾向がある一方、
  ref-update 100 loop は Stage 1 より遅い。
  今後は snapshot cursor を一律適用するのではなく、
  request-heavy path と multi-shot-heavy path の差を見る必要がある。
