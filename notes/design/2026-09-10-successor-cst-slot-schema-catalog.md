...
| proof and status | Governing slice above; direct publishers: `crates/yu-syntax/src/rule/mod.rs:683–724, 864–878, 891–894`. Direct Rowan proof: the named-tail portions of `dedicated_rule_slots_are_directly_readable_from_the_rowan_tree` in `crates/yu-syntax/src/tests/rule_literal_recovery.rs`, covering `{a.}` direct childless Name Missing before native body close, `{a.\nnext}` Missing before native LF with `next` in the second RuleSequence, `{a.12 b}` one-item Error followed by a separate outer RuleItem and `{a.12?}` quantifier ownership outside the failed RuleField. Independent post-write delta audits were clean. Status: catalog-audited evidence-complete Draft for these witnessed Missing and Error/continuation forms only; accepted-name, CRLF/EOF/fence/interpolation callers and nested grammar remain open. No following-name retry is implied. |
...
| proof and status | Governing slice above; direct publishers: `crates/yu-syntax/src/rule/mod.rs:683–724, 864–878, 891–894`. Direct Rowan proof: the named-tail portions of `dedicated_rule_slots_are_directly_readable_from_the_rowan_tree` in `crates/yu-syntax/src/tests/rule_literal_recovery.rs`, covering `{a::}` direct childless Name Missing before native body close, `{a::\nnext}` Missing before native LF with `next` in the second RuleSequence, and `{a::💥 b}` one-item UTF-8 Error followed by a separate outer RuleItem with exact immediate ancestry, token/node identity, byte ranges, full source and native close. Independent post-write delta audits were clean. Status: catalog-audited evidence-complete Draft for these witnessed Missing and Error/continuation forms only; the parenthesized EOF UTF-8 pending-leading composition is mapped separately below. Accepted-name, other CRLF/parenthesis/EOF/fence/interpolation callers and nested grammar remain open. No following-name retry is implied. |
