...
| proof and status | Governing authority: [Mod current-Item typed recovery](2026-09-08-successor-mod-current-item-recovery.md), **Records, leading and identity**, plus the CST-derived diagnostics amendment. Publisher: `crates/yu-syntax/src/declaration/mod_decl.rs:76–328,760–894`. Direct Rowan proof: `mod_schema_identity_slots_have_direct_missing_error_and_identifier_retry`, `mod_schema_anonymous_test_marker_selects_braced_and_inline_bodies`, `mod_schema_completed_identity_selects_body_introducer_before_inline_body`, and `mod_schema_actual_colon_selects_inline_body_and_keeps_child_recovery_nested` in `crates/yu-syntax/src/tests/declaration/mod_decl.rs`. M2 specification audit and regression audit, including one marker-body test repair and delta audit, were clean. Status: catalog-audited evidence-complete Draft for these witnessed direct Mod rows only; visibility, braced/indented/nested Statement recovery, global collector, parser API and recovery-ledger retirement remain open. |
...
| proof and status | Governing authority: [Role current-Item typed recovery](2026-09-08-successor-role-current-item-recovery.md), **Records and ownership** and **Boundary, leading and retry**, plus the CST-derived diagnostics amendment. Publisher: `crates/yu-syntax/src/declaration/role_decl.rs:300–606,790–894`; Head missing construction: `crates/yu-syntax/src/type_expr/mod.rs:699–708`. Direct Rowan proof: `role_schema_completed_head_selects_body_introducer`, `role_schema_actual_colon_selects_inline_body`, `role_schema_incomplete_head_does_not_select_body_introducer`, and `role_schema_inline_binding_recovery_remains_in_child_body` in `crates/yu-syntax/src/tests/declaration/role_decl.rs`. M1 post-write specification audit was clean. Status: catalog-audited evidence-complete Draft for these witnessed direct Role rows only; Head Type schema, braced/indented/nested Statement recovery, caller/fence topology, global collector, parser API and recovery-ledger retirement remain open. |
...
| proof and status | Governing authority: [Impl current-Item typed recovery](2026-09-08-successor-impl-current-item-recovery.md), **Phase correction and records** and **Boundary, leading and retry**, plus the CST-derived diagnostics amendment. Publisher: `crates/yu-syntax/src/declaration/impl_tail.rs:130–460,760–875`. Direct Rowan proof: `impl_schema_completed_head_or_description_selects_body_introducer`, `impl_schema_second_colon_selects_inline_body`, `impl_schema_first_colon_absence_and_malformed_description_stay_upstream`, and `impl_schema_inline_binding_missing_remains_child_owned` in `crates/yu-syntax/src/tests/declaration/impl_decl.rs`. M1 post-write specification audit was clean. Status: catalog-audited evidence-complete Draft only for these witnessed direct Impl rows; Head/Description Type schema, braced/indented/nested Statement recovery, caller/fence topology, global collector, parser API and recovery-ledger retirement remain open. |

### Declaration Variant core Item, Name and Separator Draft

This bounded Draft maps only the shared Enum/Error variant-core sequence. It
excludes payload/FromType, fields, closes and caller/fence behavior.

| Fact | Candidate catalog row |
| --- | --- |
| identity | `Variant(Item)` is direct `EnumVariant(Missing)` or terminal `EnumVariant(Error+)`. `Variant(Name)` is direct `EnumVariant(Error+ Native* Identifier ...)`; its post-error direct Identifier distinguishes it from terminal Item. `Variant(Separator)` is a direct declaration-shell Missing between completed direct EnumVariant siblings, not a child of either Variant. EnumDeclaration versus ErrorDeclaration ancestor selects the role family. |
| ordered Rowan grammar | `EnumVariant := Missing \| Error+ \| Error+ Native* Identifier Payload? \| Identifier Payload?`. At shell level, `completed EnumVariant Native* Missing Native* next EnumVariant` is Separator. A required Item before comma remains `EnumVariant(Missing), Comma`, and is not Separator. Error+ is maximal adjacent direct Variant Error content; native retry leading stays outside it. No Invalid belongs to these rows. |
