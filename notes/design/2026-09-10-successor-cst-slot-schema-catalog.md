...
| proof and status | Governing slice above; direct publishers: `crates/yu-syntax/src/rule/mod.rs:545–568, 653–679, 864–878, 891–894`. Direct Rowan proof: the Capture portion of `dedicated_rule_slots_are_directly_readable_from_the_rowan_tree` in `crates/yu-syntax/src/tests/rule_literal_recovery.rs:428–569`, covering `{a=;}` Error-to-Missing and `{a=; b?}` Error-to-RuleItem with exact shell, ranges, native close, retry token/node structure and terminal ownership. The independent post-write audit was clean. Status: catalog-audited evidence-complete Draft for these two witnessed forms only; bare Missing/accepted-only, longer Error runs, deeper RHS certification, interpolation/fence/caller variants and the global interpreter remain open. Capture remains terminal and is not a general retry policy. |
...
| proof and status | Governing slice above; direct publishers: `crates/yu-syntax/src/rule/mod.rs:683–724, 864–878, 891–894`. Direct Rowan proof: the named-tail portion of `dedicated_rule_slots_are_directly_readable_from_the_rowan_tree` in `crates/yu-syntax/src/tests/rule_literal_recovery.rs:592–743`, covering `{a.12 b}` one-item Error followed by a separate outer RuleItem and `{a.12?}` quantifier ownership outside the failed RuleField. Independent post-write delta audit was clean. Status: catalog-audited evidence-complete Draft for these witnessed Error/continuation forms only; Missing/accepted-name, newline/fence/broader boundaries and nested grammar remain open. No following-name retry is implied. |
...
| proof and status | Governing slice above; direct publishers: `crates/yu-syntax/src/rule/mod.rs:683–724, 864–878, 891–894`. Direct Rowan proof: the named-tail portion of `dedicated_rule_slots_are_directly_readable_from_the_rowan_tree` in `crates/yu-syntax/src/tests/rule_literal_recovery.rs:592–722`, covering `{a::💥 b}` one-item UTF-8 Error followed by a separate outer RuleItem with exact immediate ancestry, token/node identity, byte ranges, full source and native close. Independent post-write delta audit was clean. Status: catalog-audited evidence-complete Draft for this witnessed Error/continuation form only; Missing/accepted-name, newline/fence/broader boundaries and nested grammar remain open. No following-name retry is implied. |
...
| RuleLiteral interpolation/lazy/terminator | bounded mapped row above; direct Rowan proof in `tests/literal/rule_literal.rs`; Rule literal current-Item authority | user promotion open; direct-CST UTF-8 ranges and Rule child schemas remain open |
