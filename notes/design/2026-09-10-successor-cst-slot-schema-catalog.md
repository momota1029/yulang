...
more accepted binder siblings, for the ordinary emitted `Colon` token. It
excludes `PolymorphicVariantColon`, a recovered binder after malformed colon
content, first/later binder phases, nested Type, caller continuation and
close/stop ownership. It adds no node: `Missing`, raw `Error` tokens, `Colon`
and `TypeExpression` remain direct ordered children of `ForallType`.

| Fact | Candidate catalog row |
| --- | --- |
| identity | `(ForallType, terminal ordinary Colon then canonical Type body after accepted ForallTypeBinder+, direct forall head context)` |
