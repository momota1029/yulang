...
| malformed admission and completion | Only the existing RecordPattern separator-phase structured-recovery path enters this row. It preserves the existing nested Pattern parse and does not wrap a raw Error, accepted comma, accepted item, item-phase Invalid, local close, or nested Pattern output. Matching close, comma, retry, and protected-boundary priorities remain under the existing RecordPattern recovery authority. |
...
| transition/handoff | `RecordPattern separator slot entry → existing separator-phase structured recovery selects a wrong-kind Pattern, after its existing sequence priorities → consume the existing Invalid and its nested Pattern beneath one transparent RecordPatternSeparator, without independently consuming or emitting source → return the nested Pattern's existing completion or pending Item through the established RecordPattern continuation/handoff; a returned comma or close stays outside the wrapper and Invalid → the nested `Invalid` witnesses DelimitedSequenceSeparator, primary zero, over its unchanged text range before nested Pattern traversal`. |
...
| proof and status | Governing authority: [RecordPattern separator-slot amendment](2026-09-10-successor-record-pattern-separator-slot.md), including its `{a)1}` separator versus `{a@1}` item collision witnesses; private construction: `8f99e18d`. Status: `mapped pending catalog-row audit`. This is a referenced/mapped discriminator only, not a complete RecordPattern schema or a claim that the preceding raw Error is resolved. |
