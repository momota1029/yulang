...
## 追記4（2026-07-05 夜、Claude 署名）: ES-1 landed——正しさ検証全緑・大幅高速化

commit `31dc3da3`（push 済み）。設計どおりのキー（path tag 区別＋`NoHandlers`／フルコンテキスト、cycle guard 優先）で実装。

**正しさの検証（全緑）**: evidence-vm test 132/132、named canary 全部、**release contract corpus 全 219 件**、そして `examples/*.yu`・`bench/*.yu` 全部＋今回の scanner-bloated ケースで **control evidence 出力がバイト単位で完全一致**（メモ化前後で差分ゼロ）。Claude 検品済み: cycle guard チェックがキャッシュ参照より必ず先に来ること、キャッシュに入るのは active マーカー解除後の完了済み結果のみであることをコード上で確認。

**性能（release 実測、`control_evidence` フェーズのみ）**:

| regime | workload | before | after | 倍率 |
|---|---|---:|---:|---:|
| baseline（現状の通常コード） | String Match | 21.5ms | 0.709ms | 約30倍 |
| baseline | Config | 9.0ms | 0.292ms | 約31倍 |
| baseline | nondet20 | 0.369ms | 0.148ms | 約2.5倍 |
| bloated（scanner 退行の再現ケース） | String Match | 573.4ms | 0.843ms | **約680倍** |
| bloated | Config | 27.8ms | 0.412ms | 約67倍 |

**重要な意味**: bloated ケース（今回の退行の元凶）は、メモ化後は**退行前の baseline（21.5ms）よりもさらに速い**（0.843ms）。つまり、根本原因（メモ化ゼロの callgraph 展開）を直したことで、**revert 済みの文字列スキャナ高速化（43f83284 系）を安全に再導入できる可能性が高い**——コンパイル時コスト問題そのものが解消したため。ただしこれは改めて検証してから判断する話であり、本ノートのスコープ外の次の意思決定点として記録する。

キャッシュヒット率は全サンプルで `NoHandlers` キーのみが使われ（handler-stack が空の経路が支配的）、フルコンテキストキーの実測データはまだ無い（安全性は網羅的な出力差分比較で担保、性能上の恩恵は別途要確認）。`EffectOp` の known callee は意図的に非キャッシュ（apply site 固有の evidence を含むため）——ここは将来の安全な追加最適化の余地として残る。
