...
この変更で `std::var::ref` 系の古い期待値
`std::var::ref<[std::var::var<α>;], α>` が崩れたように見えたが、これは compact / instantiate の
row surface 問題ではなかった。`ref '[var 't] 't` の第1引数は不変型引数内の effect row 注釈なので、
exact row として受け入れてはならない。関数の effect 注釈と同じく、`var 't` を差し引ける
fresh residual を導入して `var 't | β` と読むのが正しい。

修正は signature lowering に入れた。`lower_invariant_sig_arg_ids` で `SigType::Row` を
pure 型の正負 lowering に流さず、`lower_function_sig_ret_eff` と同じ subtractable-row 経路へ通す。
これで `var_ref` は `unit -> std::var::ref<std::var::var<β> | α, β>` と表示される。
`α` は residual tail、`β` は参照値型で、古い exact row 期待値より注釈の意味に近い。
cooccur には保護集合を足さず、`atom <: [atom;]` を row 側へ正規化する compact 側の過剰適応も削除した。
