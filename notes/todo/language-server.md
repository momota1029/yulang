...
- diagnostic hover は実装済み。残りは shared diagnostics fixture を使った coverage 追加と、
  role/method diagnostic hover の compact regression。
...
- token classification の regression を足す。
  - type name が `tok-type` に落ちる。
  - function value / call target が `tok-function` に落ちる。
  - record field / method property が `tok-property` に落ちる。
  - playground と LS が同じ source で同じ分類を返す。
- release binary を使った `yulang server` 起動 smoke を作る。
