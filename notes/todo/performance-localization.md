...
- Done: the wasm playground run path now embeds a build-time compiled-unit
  artifact for the compact playground std profile. The artifact is generated
  by the `wasm` crate build script, decoded at runtime, and fed into the same
  embedded playground prefix-lowering route. Invalid embedded artifacts fall
  back to the old source route.
...
- Not done: build-time full embedded std compiled-unit artifact for wasm
  fallback. Parser/text-heavy examples that miss the compact playground profile
  still compile full embedded std from source.
- Next: decide whether to add full-std wasm artifact embedding, or design
