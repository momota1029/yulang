...
- Done: `.yuunit` envelopes now include `infer::CompiledNamespaceSurface`,
...
- Done: `CompiledTypeImporter` remaps serialized type IDs, type variables, and
...
- Done: the wasm playground run path now embeds build-time compiled-unit
  artifacts for both embedded std profiles. The artifacts are generated by the
  `wasm` crate build script, decoded at runtime, and fed into the same embedded
  prefix-lowering routes. Invalid embedded artifacts fall back to the old source
  route.
...
- Next: design external references for true source-SCC artifacts.
...
5. restart loop は view 全体を捨てず、dirty な view だけを再構築する。
   - method-role rescan cost を対象にした owner-level dirty scheduling subproject は、
     2026-07-16 に Stage 0〜7 を完了し、production default として close 済み。
     `notes/design/2026-07-15-owner-level-dirty-scheduling-spec.md` を見る。
...
10. evidence-only replay frontier production path — **closed-negative / superseded (2026-07-16)**。
    safe な production win は見つからず、実装せず close した。履歴と再検討条件は
    `notes/design/2026-07-16-constraint-replay-dedup-investigation.md` を見る。
11. direct-style / pure island
12. native backend revisit

## What To Do Next

`scripts/performance-gate.sh` には 2026-06-27 時点で 100x nondet の
marker-heavy runtime workload も入った。
次の運用タスクは、この gate を full mode で回して current HEAD の baseline を保存すること。
...
- marker-heavy workload の `runtime_execute` / `marker_scope_frame_touches` /
  `path_prefix_checks` / `active_add_scans` を同じ summary で追える。
