...
2026-07-22 決定: 上の numbered 0〜8「既存の公開準備 track」を優先順位の正本とする。

- Yumark の残る command / injection、span-carrying diagnostics、playground preview は
  `yumark.md` で追跡し、track item 7 から特別に繰り上げない。
- REPL（対話環境）は `repl.md` へ分離した。未着手・要設計時間の将来項目であり、
  現行の numbered 0〜8 track には含めない。
