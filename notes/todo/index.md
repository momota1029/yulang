...
1. playground regression と recently-fixed bug のうち、まだ fixture 化されていないものを洗い出して fixture に落とす（多くは landed 済み、残りは網羅確認）。
2. `yulang-editor` の diagnostics / hover を playground と LS の共有面にする（token classification は共有済み）。
3. `scripts/performance-gate.sh` を release gate / CI から呼び出して実際に gate として機能させる（script 自体は実装済み、CI 配線が残り）。
4. Rowan cost の counter を追加し、intern cost の計測を設計段階から実装に進める（cache 側の timing は実装済み）。
5. Yumark の value model を決め、syntax から runtime までの最初の thin path を作る。
