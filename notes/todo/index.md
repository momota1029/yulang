...
7. `yumark.md`
   - static core / thin path は algebra-passing model、static lowering、runtime、LS lazy hover まで完了。
     残件は command / injection support、Yumark を通した span-carrying diagnostics、playground preview。
...
注記（2026-07-16）: この順序は、上の numbered 0〜8「既存の公開準備 track」とまだ整合していない。
どちらを正本とするかは human decision が必要であり、ここでは両方を残す。

1. Yumark の static core / thin path は完了。残る command / injection、span-carrying diagnostics、
   playground preview は `yumark.md` に明示し、ここでは優先順位を追加決定しない。
2. REPL(対話環境)を作る。（累積 script を毎回 recompile すれば effect row / nondet は現行モデルを流用できるが、副作用も毎回再実行する。未入力を effect handler 同様の continuation suspend/resume と見て compile/run 境界を溶かす案は未検証、要設計時間。）
