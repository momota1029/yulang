...
- release gate / hardening smoke:
  - `scripts/hardening-smoke.sh` は Rust 側の public contract manifest に加えて、
    ...
    Direct release smoke also runs focused state-protocol sugar cases: pure
    ...
    and focused `diagnostics` canaries for type mismatch, unresolved value
    names, missing local binding body, catch arm body syntax, and unhandled
    effect reporting,
