...
4. monomorphize の entrypoint を `SchemeTable + ErasedProgram + RefExportTable` に変える。
   - ここで API 境界を先に固定する。
   - 旧 typed IR を読む処理は entrypoint の外へ追い出す。
5. monomorphize 内の型参照を削る。
...
- first cut では `yulang-erased-ir` と `yulang-elaboration` を新 crate として追加する。
