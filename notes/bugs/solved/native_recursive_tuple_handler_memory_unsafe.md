...
この note で扱っていた別系統のバグも 2026-05-18 までに解消済み:
...
| ケース | handler 形 | 戻り値型 | interpreter | native (修正前) | native (3bad594) | native (2026-05-18 最終) |
| --- | --- | --- | --- | --- | --- | --- |
| u1 | `op,k -> rec(k(), n+1)`, `v -> n` | `int` | `3` | `3` ✓ | `3` ✓ | `3` ✓ |
| u3 | `op,k -> rec(k(), n+1)`, `v -> (v, n)` | `(int, int)` | `(99, 2)` | `810610592` 等（毎回変わる） | `(99, 2)` ✓ | `(99, 2)` ✓ |
| t1 | `op,k -> rec(k(), log + o)`, `v -> (v, log)` | `(int, str)` | `(99, "a\nb\n")` | `280001392` 等 | `(99, "ab")` ✓（注: log 連結 `+ "\n"` を入れた版なら `(99, "a\nb\n")`） | `(99, "ab")` ✓ |
| t3 | t1 + `for` body 内 `if x == 5: last` | `(int, str)` | `(5, "0\n1\n2\n3\n4\n")` | `5`（tuple が崩れて第一要素のみ） | `5`（companion bug 残存） | `(5, "0\n1\n2\n3\n4\n")` ✓ |
| t5 | t1 を 1 回だけ recurse | `(_, str)` | `((), "x")` | `474247824` 等 | `(0, "x")`（unit→0 表示だけ残る） | `((), "x")` ✓ |
...
`notes/bugs/solved/native_recursive_handler_for_last_tail_skips_value_arm.yu` は t3 と同形。
...
`notes/bugs/solved/native_recursive_tuple_handler_memory_unsafe.yu` を参照。
ここに inline で書いておくと:
