...
### D5 fixed: check 後の run が specialize2 で落ちていた

D3 修正後、`(&doc.lines).index 0` と `.get()` chain は `check` を通る。
ただし当時の `run` は specialize2 で次の別 conflict に当たっていた:

```text
conflicting type candidates for slot 11: [std::io::file::file] vs []
```

原因は二段だった。まず `ref_lines 'e` の nominal 引数だけを見て `'e` を value slot
として materialize し、後から現れる `ref 'e str` の structural effect 文脈に負けていた。
nominal 引数は default kind に留め、structural occurrence の hard kind を優先するようにした。

次に specialize2 の effect slot 候補が、value `unit` などの naked `Con` を effect item
として row 化していた。effect family として登録された `Con`、row tail、pure row だけを
effect slot 候補に正規化するようにした。

最後に、同じ ref residual が呼び出し文脈で `[]` と `[file]` の両方に具体化されると、
upper row の meet が `[]` へ潰れて lower `[file]` と衝突していた。lower item が各 upper
候補の top-level item か nested effect-row payload に構造的に現れる場合は、候補集合の
row join として lower row を保持する。これで `(&doc.lines).index 0` は native run まで通る。

`(&doc.lines.each).update \line -> ...` は specialize2 を抜け、handler なしの
`nondet::branch` runtime error まで進む。これは `each` の nondet handler 境界の別件。

固定した regression:

- `tests/yulang/regressions/runtime/file_ref_lines_index_chain_native.yu`
- `file_ref_lines_index_chain_native` contract case
- specialize2 / materializer unit tests:
  - `nominal_arg_default_value_kind_loses_to_structural_ref_effect_kind`
  - `nominal_arg_default_kind_stays_value_without_structural_effect_use`
