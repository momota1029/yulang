...
- [`native_recursive_handler_for_last_tail_skips_value_arm.yu`](native_recursive_handler_for_last_tail_skips_value_arm.yu)
  — self-recursive shallow handler の resumption 内で `for ... last` を踏み、
  その後に block tail `5` があると、VM は `(5, "0\n1\n2\n3\n4\n")` だが
  native は `5` を返す。`last` / `last::break` の handler selection と
  `route_scope_return` の frame-walk は成功している。`RoutedJump` を root-level
  `Global` abort として残す実験では `(0, "4\n")` で止まるため、単純な abort
  propagation ではなく、`for` 後続 continuation と外側 handler value arm の
  再開順序が CPS eval/repr とずれている。
